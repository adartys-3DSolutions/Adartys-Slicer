...
// BBS: remove snapshots function currently
    ...
    if (! boost::filesystem::is_directory(path_dst) && 
        ...
        throw Slic3r::RuntimeError(std::string("AdartysSlicer was unable to create a directory at ") + path_dst.string());
                                          ...
                                          const std::string& message)
{
    try {
        ...
        RichMessageDialog dlg(static_cast<wxWindow*>(wxGetApp().mainframe),
                              _L("AdartysSlicer has encountered an error while taking a configuration snapshot.") + "\n\n" +
                                  from_u8(err.what()) + "\n\n" + from_u8(message),
                              _L("AdartysSlicer error"), wxYES_NO);
