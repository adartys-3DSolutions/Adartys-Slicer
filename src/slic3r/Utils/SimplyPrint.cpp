...
static const std::string                    CLIENT_ID             = "simplyprintadartysslicer";
...
GUI::OAuthParams SimplyPrint::get_oauth_params() const
{
    ...
    const std::vector<std::pair<std::string, std::string>> query_parameters{
        {"client_id", CLIENT_ID}, {"redirect_uri", CALLBACK_URL},     {"scope", CLIENT_SCOPES},          {"response_type", RESPONSE_TYPE},
...
void SimplyPrint::log_out() const { boost::nowide::remove(cred_file.c_str()); }
                              ...
                              std::function<bool(std::string, std::string, unsigned)> on_error) const
{
    ...
    const auto create_request = [this, &build_request, &res, &on_complete](const std::string& access_token, bool is_retry) {
        ...
        http.header("User-Agent", "SimplyPrint Adartys Plugin").on_complete([&](std::string body, unsigned http_status) {
                                 ...
                                 ErrorFn                        error_fn) const
{
    ...
    return do_api_call(
        ...
        [this, &error_fn](std::string body, std::string error, unsigned status) {
            BOOST_LOG_TRIVIAL(error) << boost::format("SimplyPrint: Error uploading file : %1%, HTTP %2%, body: `%3%`") % error % status %
                                            body;
...
bool SimplyPrint::do_chunk_upload(const boost::filesystem::path& file_path,
                                  const std::string&             filename,
                                  ProgressFn                     prorgess_fn,
                                  ErrorFn                        error_fn) const
{
    ...
    for (size_t i = 0; i < chunk_amount; i++) {
        ...
        const bool succ = do_api_call(
            ...
            [&error_fn, i, chunk_amount, this, &chunk_id, &delete_token](std::string body, unsigned status) {
                BOOST_LOG_TRIVIAL(info) << boost::format("SimplyPrint: File chunk [%1%/%2%] uploaded: HTTP %3%: %4%") % (i + 1) %
                                               chunk_amount % status % body;
                if (i == 0) {
