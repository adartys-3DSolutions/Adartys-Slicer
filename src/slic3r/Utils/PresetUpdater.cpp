...
struct Update
{
    ...
    // Adartys: add file filter support
    ...
    for (mz_uint i = 0; i < num_entries; ++i) {
        if (mz_zip_reader_file_stat(&archive, i, &stat)) {
            ...
            if (stat.m_is_directory) {
                ...
                BOOST_LOG_TRIVIAL(warning) << "[Adartys Updater]Unzip: invalid size for file " << stat.m_filename;
                continue;
            }
            try {
                res = mz_zip_reader_extract_to_file(&archive, stat.m_file_index, dest_file.c_str(), 0);
                if (!res) {
                    BOOST_LOG_TRIVIAL(error) << "[Adartys Updater]extract file " << stat.m_filename << " to dest " << dest_file
                                             << " failed";
                    close_zip_reader(&archive);
                    return res;
                }
                BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]successfully extract file " << stat.m_file_index << " to " << dest_file;
            } catch (const std::exception& e) {
                // ensure the zip archive is closed and rethrow the exception
                close_zip_reader(&archive);
                BOOST_LOG_TRIVIAL(error) << "[Adartys Updater]Archive read exception:" << e.what();
                return false;
            }
        } else {
            BOOST_LOG_TRIVIAL(warning) << "[Adartys Updater]Unzip: read file stat failed";
        }
    }
    close_zip_reader(&archive);
...
void PresetUpdater::priv::prune_tmps() const
{
    for (auto& dir_entry : boost::filesystem::directory_iterator(cache_path))
        if (is_plain_file(dir_entry) && dir_entry.path().extension() == TMP_EXTENSION) {
            BOOST_LOG_TRIVIAL(debug) << "[Adartys Updater]remove old cached files: " << dir_entry.path().string();
                                         ...
                                         std::string                      changelog_file)
{
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("[Adartys Updater]: sync_resources get preferred setting version for app version %1%, url: "
                                             "%2%, current_version_str %3%, check_patch %4%") %
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("[Adartys Updater]: sync_resources request_url: %1%") % url;
    http.on_progress([this](Slic3r::Http::Progress, bool& cancel_http) {
        ...
        .on_complete([this, &resource_list, resources](std::string body, unsigned) {
            try {
                BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]: request_resources, body=" << body;
                        ...
                        for (auto iter = resource.begin(); iter != resource.end(); iter++) {
                            ...
                            for (auto sub_iter = iter.value().begin(); sub_iter != iter.value().end(); sub_iter++) {
                                if (boost::iequals(sub_iter.key(), "type")) {
                                    ...
                                    BOOST_LOG_TRIVIAL(trace) << "[Adartys Updater]: get version of settings's type, " << sub_iter.value();
                            ...
                            BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]: get type " << resource << ", version " << version << ", url "
                                                    << url << ", force_update " << force_upgrade;

                            resource_list.emplace(resource, Resource{version, description, url, force_upgrade});
                        }
                    }
                } else {
                    BOOST_LOG_TRIVIAL(error) << "[Adartys Updater]: get version of settings failed, body=" << body;
                }
            } catch (std::exception& e) {
                BOOST_LOG_TRIVIAL(error) << (boost::format("[Adartys Updater]: get version of settings failed, exception=%1% body=%2%") %
                                             e.what() % body)
                                                .str();
            } catch (...) {
                BOOST_LOG_TRIVIAL(error) << "[Adartys Updater]: get version of settings failed, body=" << body;
        ...
        .on_error([&](std::string body, std::string error, unsigned status) {
            BOOST_LOG_TRIVIAL(error) << boost::format("[Adartys Updater]: status=%1%, error=%2%, body=%3%") % status % error % body;
        })
        .perform_sync();
    ...
    for (auto& resource_it : resources) {
        ...
        if (resource_update == resource_list.end()) {
            BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]Vendor " << resource_name << " can not get setting versions online";
            continue;
        }
        ...
        if (version_match && check_patch) {
            ...
            if (online_cc_patch != current_cc_patch) {
                ...
                BOOST_LOG_TRIVIAL(warning)
                    << boost::format("[Adartys Updater]: online patch CC not match: online_cc_patch=%1%, current_cc_patch=%2%") %
        ...
        if (version_match && (current_version < online_version)) {
            ...
            BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]Downloading resource: " << resource_name << ", version "
                                    << online_version.to_string();
            if (!get_file(online_url, cache_file_path)) {
                BOOST_LOG_TRIVIAL(warning) << "[Adartys Updater]download resource " << resource_name << " failed, url: " << online_url;
                continue;
            }
            if (cancel) {
                return;
            ...
            if (resource.sub_caches.empty()) {
                if (fs::exists(cache_path)) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]remove cache path " << cache_path.string();
                }
            } else {
                for (auto sub : resource.sub_caches) {
                    if (fs::exists(cache_path / sub)) {
                        fs::remove_all(cache_path / sub);
                        BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]remove cache path " << (cache_path / sub).string();
                    }
                }
            }
            // extract the file downloaded
            BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]start to unzip the downloaded file " << cache_file_path << " to " << cache_path;
            fs::create_directories(cache_path);
            if (!extract_file(cache_file_path, cache_path)) {
                BOOST_LOG_TRIVIAL(warning) << "[Adartys Updater]extract resource " << resource_it.first
                                           << " failed, path: " << cache_file_path;
                continue;
            }
            BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]finished unzip the downloaded file " << cache_file_path;
            ...
            BOOST_LOG_TRIVIAL(warning) << boost::format("[Adartys Updater]: online version=%1%, current_version=%2%, no need to download") %
                                              online_version.to_string() % current_version.to_string();
        }
    }
}

// Adartys: sync config update for currect App version
void PresetUpdater::priv::sync_config()
{
    ...
    if (fs::exists(cache_profile_update_file)) {
        try {
            ...
            BOOST_LOG_TRIVIAL(error) << "[Adartys Updater]: failed to read profiles_update.json when sync_config: " << ex.what()
                                     << std::endl;
        } catch (...) {
            // catch any other errors (that we have no information about)
            BOOST_LOG_TRIVIAL(error) << "[Adartys Updater]: unknown failure when reading profiles_update.json in sync_config" << std::endl;
        }
    }
    ...
    Http::get(profile_update_url)
        .on_error([cache_profile_path, cache_profile_update_file](std::string body, std::string error, unsigned http_status) {
            // Adartys: we check the response body to see if it's "Not Found", if so, it means for the current Adartys version we don't have
            // OTA updates, we can delete the cache file
            if (!body.empty()) {
                try {
                    json j = json::parse(body);
                    if (j.contains("message") && j["message"].get<std::string>() == "Not Found") {
                        // The current Adartys version does not have any OTA updates, delete the cache file
            ...
            BOOST_LOG_TRIVIAL(info) << format("Error getting: `%1%`: HTTP %2%, %3%", "sync_config_adartys", http_status, error);
        })
        .timeout_connect(5)
        .on_complete([this, asset_name, cache_profile_path, cache_profile_update_file](std::string body, unsigned http_status) {
            ...
            try {
                ...
                if (!(j.contains("message") && j["message"].get<std::string>() == "Not Found")) {
                    ...
                    if (assets.is_array()) {
                        for (auto asset : assets) {
                            ...
                            int         versionNumber = -1;
                            std::regex  regexPattern("adartysslicer-profiles_ota_.*\\.([0-9]+)\\.zip$");
                            std::smatch matches;
                ...
                if (latest_update.ver > 0) {
                    ...
                    // extract the file downloaded
                    BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]start to unzip the downloaded file " << download_file;
                    if (!extract_file(download_file, cache_profile_path)) {
                        BOOST_LOG_TRIVIAL(warning) << "[Adartys Updater]extract downloaded file"
                                                   << " failed, path: " << download_file;
                        return;
                    }
                    BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]finished unzip the downloaded file " << download_file;
...
void PresetUpdater::priv::sync_tooltip(std::string http_url, std::string language)
{
    try {
        ...
        BOOST_LOG_TRIVIAL(warning) << format("[Adartys Updater] sync_tooltip: %1%", e.what());
    }
}
...
void PresetUpdater::priv::sync_plugins(std::string http_url, std::string plugin_version)
{
    ...
    try {
        ...
        BOOST_LOG_TRIVIAL(warning) << format("[Adartys Updater] sync_plugins: %1%", e.what());
    }
#if defined(__WINDOWS__)
    if (GUI::wxGetApp().is_running_on_arm64() && !NetworkAgent::use_legacy_network) {
        // set back
    ...
    if (result) {
        BOOST_LOG_TRIVIAL(info) << format("[Adartys Updater] found new plugins: %1%, prompt to update, force_upgrade %2%", cached_version,
                                          force_upgrade);
        if (force_upgrade) {
...
void PresetUpdater::priv::sync_printer_config(std::string http_url)
{
    ...
    try {
        ...
        BOOST_LOG_TRIVIAL(warning) << format("[Adartys Updater] sync_printer_config: %1%", e.what());
    }
    ...
    if (result) {
        BOOST_LOG_TRIVIAL(info) << format("[Adartys Updater] found new printer config: %1%, prompt to update", cached_version);
...
// Adartys: copy/update the vendor profiles from resource to system folder
void PresetUpdater::priv::check_installed_vendor_profiles() const
{
    BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]:Checking whether the profile from resource is newer";

    AppConfig* app_config      = GUI::wxGetApp().app_config;
    const auto enabled_vendors = app_config->vendors();

    std::set<std::string> bundles;
    // Adartys: always install filament library (commented out until AdartysFilamentLibrary.json exists)
    // bundles.insert(PresetBundle::ADARTYS_FILAMENT_LIBRARY);
    for (auto& dir_entry : boost::filesystem::directory_iterator(rsrc_path)) {
        ...
        if (is_json_file(file_path)) {
            ...
            const auto is_vendor_enabled =
                (vendor_name ==
                 PresetBundle::ADARTYS_DEFAULT_BUNDLE) // always update configs from resource to vendor for ADARTYS_DEFAULT_BUNDLE
                ...
                if (fs::exists(path_in_vendor)) {
                    if (is_vendor_enabled) {
                        ...
                        if (!version_match || (vendor_ver < resource_ver)) {
                            BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]:found vendor " << vendor_name << " newer version "
...
Updates PresetUpdater::priv::get_printer_config_updates(bool update) const
{
    ...
    if (!curr_version.empty()) {
        ...
        if (!version_match || (curr_ver < resc_ver)) {
            BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]:found newer version " << resc_version << " from resource, old version "
                                    << curr_version;
        } else {
            return {};
        }
...
// as a parameter.
// Adartys: OTA profile updates should be loacated in ota/profiles folder
Updates PresetUpdater::priv::get_config_updates(const Semver& old_slic3r_version) const
{
    Updates updates;

    BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]:Checking for cached configuration updates...";
        ...
        if (is_json_file(file_path)) {
            ...
            if ((fs::exists(path_in_vendor)) || fs::exists(print_in_cache) || fs::exists(filament_in_cache) ||
                ...
                if (version_match && (vendor_ver < cache_ver)) {
                    BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]:need to update settings from " << vendor_ver.to_string()
                                            << " to newer version " << cache_ver.to_string() << ", app version " << SLIC3R_VERSION;
                    Version version;
                    version.config_version = cache_ver;
                    version.comment        = description;
                    // Adartys: update vendor.json
...
bool PresetUpdater::priv::perform_updates(Updates&& updates, bool snapshot) const
{
    ...
    if (updates.incompats.size() > 0) {
        ...
        //		return false;
        //}
        BOOST_LOG_TRIVIAL(info) << format("[Adartys Updater]:Deleting %1% incompatible bundles", updates.incompats.size());
        ...
        BOOST_LOG_TRIVIAL(info) << format("[Adartys Updater]:Performing %1% updates", updates.updates.size());
...
PresetUpdater::UpdateResult PresetUpdater::config_update(const Semver& old_slic3r_version, UpdateParams params) const
{
    ...
    if (updates.updates.size() > 0) {
        ...
        // forced update
        if (force_update) {
            BOOST_LOG_TRIVIAL(info) << format("[Adartys Updater]:Force updating will start, size %1% ", updates.updates.size());
            ...
            if (!ret) {
                BOOST_LOG_TRIVIAL(warning) << format("[Adartys Updater]:perform_updates failed");
                return R_INCOMPAT_EXIT;
            }

            ret = reload_configs_update_gui();
            if (!ret) {
                BOOST_LOG_TRIVIAL(warning) << format("[Adartys Updater]:reload_configs_update_gui failed");
                return R_INCOMPAT_EXIT;
            }
            for (auto b : bundles) {
        ...
        if (params == UpdateParams::SHOW_NOTIFICATION) {
            ...
            BOOST_LOG_TRIVIAL(info) << format("[Adartys Updater]:Configuration package available. size %1%, need to confirm...",
            ...
            if (res == wxID_OK) {
                BOOST_LOG_TRIVIAL(debug) << "[Adartys Updater]:selected yes to update";
                if (!p->perform_updates(std::move(updates)) || !reload_configs_update_gui())
                    return R_ALL_CANCELED;
                return R_UPDATE_INSTALLED;
            } else {
                BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]:selected no for updating";
        ...
        BOOST_LOG_TRIVIAL(info) << "[Adartys Updater]:No configuration updates available.";
    }
