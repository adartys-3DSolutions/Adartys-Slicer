...
// For starting another AdartysSlicer instance on OSX.
                                            ...
                                            bool                        single_instance)
{
    ...
    path += "\\";
    path += (instance_type == NewSlicerInstanceType::Slicer) ? "adartys-slicer.exe" : "bambu-gcodeviewer.exe";
    ...
    // Don't call with wxEXEC_HIDE_CONSOLE, AdartysSlicer in GUI mode would just show the splash screen. It would not open the main window
...
#if defined(__APPLE__)
    {
        ...
        // bin_path = bin_path.parent_path() / "AdartysSlicer";
...
#else // Linux or Unix
    {
        ...
        if (args.empty()) {
            ...
            my_path = (bin_path.parent_path() / ((instance_type == NewSlicerInstanceType::Slicer) ? "adartys-slicer" : "bambu-gcodeviewer"))
                          .string();
