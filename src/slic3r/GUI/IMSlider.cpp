...
void IMSlider::SetSelectionSpan(const int lower_val, const int higher_val)
{
    ...
    // ADARTYS reset single layer position when min max values changed
...
bool IMSlider::switch_one_layer_mode()
{
    ...
    if (!m_is_one_layer) {                    // DEACTIVATE
        m_one_layer_value = GetHigherValue(); // ADARTYS Backup value on deactivate
        SetLowerValue(m_min_value);
        SetHigherValue(m_max_value); // Higher value resets on toggling off one layer mode to show whole model
    } else {                         // ACTIVATE
                                     // ADARTYS Ensure value fits range. value set in IMSlider::SetSelectionSpan but added this just in case
        ...
        } else if (GetHigherValue() == m_max_value) // ADARTYS Prefer backup value if higher value reseted
            SetHigherValue(m_one_layer_value);      // ADARTYS Restore value
        else // ADARTYS Prefer higher value if user changed higher value. so it will show section on same view
            SetHigherValue(GetHigherValue()); // ADARTYS use same position with higher value if user changed its position. visible section
...
void IMSlider::draw_ticks(const ImRect& slideable_region)
{
    ...
    tick_it = GetSelection() == ssHigher ?
                  m_ticks.ticks.find(TickCode{this->GetHigherValue()}) :
                  GetSelection() == ssLower ? m_ticks.ticks.find(TickCode{this->GetLowerValue()}) : m_ticks.ticks.end();
...
bool IMSlider::render(int canvas_width, int canvas_height)
{
    ...
    ImGui::PushStyleColor(ImGuiCol_::ImGuiCol_Text, ImGuiWrapper::COL_ADARTYS); // ADARTYS: Use adartys color for slider value text
...
void IMSlider::render_menu()
{
    ...
    auto tick_it = GetSelection() == ssHigher ?
                       m_ticks.ticks.find(TickCode{GetHigherValue()}) :
                       GetSelection() == ssLower ? m_ticks.ticks.find(TickCode{GetLowerValue()}) : m_ticks.ticks.end();
    ...
    if (is_horizontal()) {
        ...
        if (evt.GetPosition().x > layers_slider_window->Pos.x &&
            ...
            if (is_one_layer()) {
                const int new_pos = GetHigherValue() + wheel;
                SetHigherValue(new_pos);
                m_one_layer_value = new_pos; // ADARTYS backup value for single layer mode
            } else {
...
std::string IMSlider::get_label(int tick, LabelType label_type)
{
    ...
    if (m_draw_mode == dmSequentialGCodeView) {
        ...
        if (label_type == ltHeightWithLayer) {
            char   buffer[64];
            size_t layer_number;
            layer_number = m_draw_mode == dmSequentialFffPrint ?
                               (m_values.empty() ? value : value + 1) :
                               m_is_wipe_tower ? get_layer_number(value, label_type) + 1 : (m_values.empty() ? value : value + 1);
