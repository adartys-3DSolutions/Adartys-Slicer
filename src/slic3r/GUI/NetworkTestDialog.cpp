...
wxBoxSizer* NetworkTestDialog::create_info_sizer(wxWindow* parent)
{
    ...
    text_version_title        = new wxStaticText(this, wxID_ANY, _L("AdartysSlicer Version:"), wxDefaultPosition, wxDefaultSize, 0);
...
wxBoxSizer* NetworkTestDialog::create_content_sizer(wxWindow* parent)
{
    ...
    btn_link = new Button(this, _L("Test AdartysSlicer (GitHub)"));
    btn_link->SetStyle(ButtonStyle::Regular, ButtonType::Window);
    grid_sizer->Add(btn_link, 0, wxEXPAND | wxALL, 5);

    text_link_title = new wxStaticText(this, wxID_ANY, _L("Test AdartysSlicer (GitHub):"), wxDefaultPosition, wxDefaultSize, 0);
...
void NetworkTestDialog::init_bind()
{
    Bind(EVT_UPDATE_RESULT, [this](wxCommandEvent& evt) {
        if (evt.GetInt() == TEST_ADARTYS_JOB) {
...
void NetworkTestDialog::start_all_job_sequence()
{
    m_sequence_job = new boost::thread([this] {
        ...
        if (m_closing)
            return;
        start_test_url(TEST_ADARTYS_JOB, "AdartysSlicer(GitHub)", "https://github.com/SoftFever/AdartysSlicer");
        if (m_closing)
            return;
...
{
    if (m_in_testing[TEST_ADARTYS_JOB])
        return;
    test_job[TEST_ADARTYS_JOB] = new boost::thread(
        [this] { start_test_url(TEST_ADARTYS_JOB, "AdartysSlicer(GitHub)", "https://github.com/SoftFever/AdartysSlicer"); });
}
void NetworkTestDialog::start_test_bing_thread()
{
    test_job[TEST_BING_JOB] = new boost::thread([this] { start_test_url(TEST_BING_JOB, "Bing", "http://www.bing.com"); });
}
