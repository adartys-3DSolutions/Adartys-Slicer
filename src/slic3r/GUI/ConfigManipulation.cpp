...
void ConfigManipulation::toggle_print_fff_options(DynamicPrintConfig* config, const bool is_global_config)
{
    ...
    for (auto el : {"sparse_infill_pattern", "infill_combination", "minimum_sparse_infill_area", "sparse_infill_filament",
                    "infill_anchor_max", "infill_shift_step", "sparse_infill_rotate_template", "symmetric_infill_y_axis"})
    ...
    bool have_multiline_infill_pattern = pattern == ipGyroid || pattern == ipGrid || pattern == ipRectilinear || pattern == ipTpmsD ||
                                         ...
                                         pattern == ipLateralLattice || pattern == ipLateralHoneycomb || pattern == ipCubic ||
                                         pattern == ipStars || pattern == ipAlignedRectilinear || pattern == ipLightning ||
                                         pattern == ip3DHoneycomb || pattern == ipAdaptiveCubic || pattern == ipSupportCubic;
    ...
    toggle_line("single_loop_draft_shield", have_skirt); // ADARTYS: Display one wall if skirt enabled
    ...
    // Adartys: Force solid support interface when using support ironing
    ...
    // Adartys: Raft, grid, snug and organic supports use these two parameters to control the size & density of the "brim"/flange
    ...
    // Adartys: disable infill_direction/solid_infill_direction if sparse_infill_rotate_template/solid_infill_rotate_template is not empty value
...
{
    wxString msg_text = _(L("Spiral mode only works when wall loops is 1, support is disabled, top shell layers is 0, sparse infill "
    ...
    MessageDialog dialog(m_msg_dlg_parent, msg_text, "", wxICON_WARNING | (!is_object_config ? wxYES | wxNO : wxOK));
    is_msg_dlg_already_exist = true;
    auto answer              = dialog.ShowModal();
    is_msg_dlg_already_exist = false;
    if (is_object_config)
