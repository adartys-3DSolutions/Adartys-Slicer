...
static const int    PARTPLATE_EDIT_PLATE_NAME_ICON_SIZE = 9; // ADARTYS this also scales height of plate name
...
void PartPlate::render_exclude_area(bool force_default_color)
{
    ...
    ColorRGBA select_color{.9f, .86f, .82f, .7f}; // ADARTYS
    ColorRGBA unselect_color{.6f, .6f, .6f, .3f}; // ADARTYS
...
Vec3d PartPlate::estimate_wipe_tower_size(
    const DynamicPrintConfig& config, const double w, const double d, int plate_extruder_size, bool use_global_objects) const
{
...
void PartPlate::generate_plate_name_texture()
{
    ...
    // ADARTYS also scale font size to prevent low res texture
...
bool PartPlate::check_outside(int obj_id, int instance_id, BoundingBoxf3* bounding_box)
{
    ...
    if (instance_box.min.z() < SINKING_Z_THRESHOLD) {
        // Adartys: For sinking object, we use a more expensive algorithm so part below build plate won't be considered
...
void PartPlate::generate_exclude_polygon(ExPolygon& exclude_polygon)
{
    ...
    if (m_exclude_area.size() == 4) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            radius = 1.f; // ADARTYS use equal rounding for all corners
            switch (i) {
            case 0: // Left-Bottom
                center(0)   = p(0) + radius;
                center(1)   = p(1) + radius;
...
bool PartPlate::set_shape(const Pointfs& shape, const Pointfs& exclude_areas, Vec2d position, float height_to_lid, float height_to_rod)
{
    ...
    if ((m_shape != new_shape) || (m_exclude_area != new_exclude_areas)) {
        ...
        // ADARTYS also change bed_icon_count number in calc_vertex_for_icons() after adding or removing icons for circular shaped beds that
...
std::vector<PartPlate*> PartPlateList::get_nonempty_plate_list()
{
    ...
    for (auto plate : m_plate_list) {
        ...
        if (!plate->empty()) { // ADARTYS counts failed slices as non empty because they have model and should be calculated on total count
...
bool PartPlateList::set_shapes(
    const Pointfs& shape, const Pointfs& exclude_areas, const std::string& texture_filename, float height_to_lid, float height_to_rod)
{
    const std::lock_guard<std::mutex> local_lock(m_plates_mutex);
    m_shape         = shape;
