...
#define PRINTER_SPACE_SIZE wxSize(FromDIP(100), FromDIP(24)) // ADARTYS Match size with other components
...
#define SELECT_ALL_OPTION_COLOUR wxColour(ADARTYS_PRIMARY_COLOR)
...
static const std::unordered_map<std::string, std::vector<std::string>> printer_model_map =
    {{"Anker", {"Anker M5", "Anker M5 All-Metal Hot End", "Anker M5C"}},
     {"Anycubic",
      {"Anycubic i3 Mega S", "Anycubic Chiron", "Anycubic Vyper", "Anycubic Kobra", "Anycubic Kobra Max", "Anycubic Kobra Plus",
       "Anycubic 4Max Pro", "Anycubic 4Max Pro 2", "Anycubic Kobra 2", "Anycubic Kobra 2 Plus", "Anycubic Kobra 2 Max",
       "Anycubic Kobra 2 Pro", "Anycubic Kobra 2 Neo", "Anycubic Kobra 3", "Anycubic Kobra S1"}},
     {"Artillery",
     ...
     {"Mellow", {"M1"}},
     {"Adartys Arena Printer", {"Adartys Arena X1 Carbon"}},
     {"Peopoly", {"Peopoly Magneto X"}},
     {"Positron 3D", {"The Positron"}},
     {"Prusa",
      {"Prusa CORE One", "Prusa CORE One HF", "MK4IS", "MK4S", "MK4S HF", "Prusa XL", "Prusa XL 5T", "MK3.5", "MK3S", "MINI", "MINIIS"}},
     {"Qidi",
                ...
                wxCAPTION | wxCLOSE_BOX | wxCENTER)
{
    ...
    m_main_sizer->Add(page_sizer, 0, wxEXPAND | wxRIGHT | wxLEFT, FromDIP(5)); // ADARTYS use equal border for both sides
    select_curr_radiobox(m_create_type_btns, 0);
...
wxBoxSizer* CreatePrinterPresetDialog::create_bed_size_item(wxWindow* parent)
{
    ...
    // ADARTYS use icon on input box to match style with other Point fields
...
wxBoxSizer* CreatePrinterPresetDialog::create_max_print_height_item(wxWindow* parent)
{
    ...
    m_print_height_input          = new TextInput(parent, "200", "mm", wxEmptyString, wxDefaultPosition, PRINTER_SPACE_SIZE,
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    case FILAMENT:
        ...
        wxString sync_text   = sync_user_preset_need_enabled ?
                                 _L("\n\nAdartys has detected that your user presets synchronization function is not enabled, "
...
{
    m_exprot_type.preset_bundle   = _L("Printer config bundle(.adartys_printer)");
    m_exprot_type.filament_bundle = _L("Filament bundle(.adartys_filament)");
...
ExportConfigsDialog::ExportCase ExportConfigsDialog::archive_preset_bundle_to_file(const wxString& path)
{
    ...
    for (std::pair<::CheckBox*, Preset*> checkbox_preset : m_preset) {
        if (checkbox_preset.first->GetValue()) {
            ...
            mz_zip_archive zip_archive;
            mz_bool        status = initial_zip_archive(zip_archive, export_path + "/" + printer_preset->name + ".adartys_printer");
            if (MZ_FALSE == status) {
                BOOST_LOG_TRIVIAL(info) << "Failed to initialize ZIP archive";
                return ExportCase::INITIALIZE_FAIL;
            }
...
ExportConfigsDialog::ExportCase ExportConfigsDialog::archive_filament_bundle_to_file(const wxString& path)
{
    ...
    for (std::pair<::CheckBox*, std::string> checkbox_filament_name : m_printer_name) {
        if (checkbox_filament_name.first->GetValue()) {
            ...
            mz_zip_archive zip_archive;
            mz_bool        status = initial_zip_archive(zip_archive, export_path + "/" + filament_name + ".adartys_filament");
            if (MZ_FALSE == status) {
                BOOST_LOG_TRIVIAL(info) << "Failed to initialize ZIP archive";
                return ExportCase::INITIALIZE_FAIL;
            }
...
wxWindow* EditFilamentPresetDialog::create_dialog_buttons()
{
    ...
    dlg_btns->GetButtonFromID(wxID_DELETE)
