    ...
    : DPIDialog(parent, wxID_ANY, title, wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX), m_app_key(app_key)
{
    ...
    m_confirm->SetBorderColor(wxColour(ADARTYS_PRIMARY_RGB));
...
void TipsDialog::on_dpi_changed(const wxRect& suggested_rect)
{
    if (m_confirm)
        m_confirm->SetMinSize(TIPS_DIALOG_BUTTON_SIZE);
    if (m_cancel)
        m_cancel->SetMinSize(TIPS_DIALOG_BUTTON_SIZE);
    Fit();
    Refresh();
}
    ...
    : wxPanel(parent, id, pos, size, style, name)
{
    ...
    if (dynamic_cast<Notebook*>(parent)) {
        ...
        m_title_view = new Label(m_top_panel, Label::Body_12,
                                 _L("Advance")); // ADARTYS match size with advanced toggle on tab.cpp m_static_title
