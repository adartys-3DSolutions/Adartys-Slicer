...
Sidebar::Sidebar(Plater* parent) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
...

    {
        ...
        p->m_panel_filament_title->Bind(wxEVT_LEFT_UP, [this](wxMouseEvent& e) {
            if (e.GetPosition().x > (p->m_flushing_volume_btn->IsShown() ?
                                         ...
                                         (p->m_bpButton_add_filament->GetPosition().x -
                                          FromDIP(30)))) // ADARTYS exclude area of del button from titlebar collapse/expand feature to fix
        ...
        add_btn->Bind(wxEVT_BUTTON, [this, scrolled_sizer](wxCommandEvent& e) {
            // Adartys: limit filament choices to MAXIMUM_EXTRUDER_NUMBER
        ...
        // ADARTYS Moved add button after delete button to prevent add button position change when remove icon automatically hidden
        ...
        bSizer39->Add(add_btn, 0, wxALIGN_CENTER | wxLEFT,
                      FromDIP(SidebarProps::IconSpacing())); // ADARTYS Moved add button after delete button to prevent add button position
                                                             // change when remove icon automatically hidden
        bSizer39->AddSpacer(FromDIP(20));

        if (p->combos_filament.size() <=
            1) { // ADARTYS Fix Flushing button and Delete filament button not hidden on launch while only 1 filament exist
            bSizer39->Hide(p->m_flushing_volume_btn);
            bSizer39->Hide(p->m_bpButton_del_filament); // ADARTYS: Hide delete filament button if there is only one filament
        }

        ams_btn = new ScalableButton(p->m_panel_filament_title, wxID_ANY, "ams_fila_sync", wxEmptyString, wxDefaultSize, wxDefaultPosition,
                                     wxBU_EXACTFIT | wxNO_BORDER, false, 16); // ADARTYS match icon size with other icons as 16x16
...

    {
        ...
        // ADARTYS: Update search box to modern style
        ...
        p->m_search_item = new TextInput(p->m_search_bar, wxEmptyString, wxEmptyString, "", wxDefaultPosition, wxDefaultSize,
                                         0 | wxBORDER_NONE);
        p->m_search_item->SetIcon(*BitmapCache().load_svg("search", FromDIP(16), FromDIP(16))); // ADARTYS: Add search icon to search box
...
void Sidebar::update_all_preset_comboboxes()
{
    ...
    if (is_bbl_vendor || cfg.opt_bool("support_multi_bed_types")) {
        ...
        // Adartys: don't update bed type if loading project
    ...
    // Adartys:: show device tab based on vendor type
...
void Sidebar::on_filaments_change(size_t num_filaments)
{
    ...
    if (p->m_flushing_volume_btn != nullptr && sizer != nullptr) {
        if (num_filaments > 1) {
            ...
            sizer->Show(p->m_bpButton_del_filament); // ADARTYS: Show delete filament button if multiple filaments
        } else {
            sizer->Hide(p->m_flushing_volume_btn);
            sizer->Hide(p->m_bpButton_del_filament); // ADARTYS: Hide delete filament button if there is only one filament
        }
    }
...
void Sidebar::sync_ams_list()
{
    ...
    if (unknowns > 0) {
        MessageDialog
            dlg(this,
                _L("There are some unknown filaments mapped to generic preset. Please update Adartys Slicer or restart Adartys Slicer "
...
void Sidebar::show_SEMM_buttons(bool bshow)
{
    ...
    if (p->m_bpButton_del_filament && p->combos_filament.size() > 1) // ADARTYS add filament count as condition to prevent showing Flushing
                                                                     // volumes and Del Filament icon visible while only 1 filament exist
        p->m_bpButton_del_filament->Show(bshow);
    if (p->m_flushing_volume_btn && p->combos_filament.size() > 1) // ADARTYS add filament count as condition to prevent showing Flushing
                                                                   // volumes and Del Filament icon visible while only 1 filament exist
...
wxColour Plater::get_next_color_for_filament()
{
    ...
    wxColour colors[FILAMENT_SYSTEM_COLORS_NUM] = {
        // ADARTYS updated all color palette
                ...
                Semver             file_version;
                {
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if (load_type != LoadType::LoadGeometry)
                            show_info(q, _L("The 3mf is not supported by AdartysSlicer, load geometry data only."), _L("Load 3mf"));
                    }
                    ...
                    //             show_info(q, _L("The 3mf is generated by old Adartys Slicer, load geometry data only."), _L("Load 3mf"));
                    //         else
                    //             show_info(q, _L("The 3mf is not supported by AdartysSlicer, load geometry data only."), _L("Load 3mf"));
                    //     }
                    ...
                    //     }
                    // }
                    // Adartys: check if the project is created with AdartysSlicer 2.3.1-alpha and use the sparse infill rotation template
                    // for non-safe infill patterns
                        ...
                        if (!config_loaded.opt_string("sparse_infill_rotate_template").empty()) {
                            ...
                            if (!is_safe_to_rotate) {
                                wxString msg_text = _(
                                    L("This project was created with an AdartysSlicer 2.3.1-alpha and uses "
                                ...
                                msg_text += "\n\n" + _(L("Would you like AdartysSlicer to automatically fix this by clearing the rotation "
                                                         "template settings?"));
...
{
    m_is_dark = true; // Always dark mode on macOS
...
void Plater::priv::apply_color_mode()
{
    ...
    wxColour   adartys_color =
        wxColour(59, 68,
                 70); // wxColour(ColorRGBA::ADARTYS().r_uchar(), ColorRGBA::ADARTYS().g_uchar(), ColorRGBA::ADARTYS().b_uchar());
    adartys_color       = is_dark ? StateColor::darkModeColorFor(adartys_color) : StateColor::lightModeColorFor(adartys_color);
        ...
        if (printer_technology == ptSLA)
            ...
            if (current_panel == assemble_view) {
                ...
                if (is_some_full_instances)
                    ...
                    menu                            = (model_volume != nullptr && model_volume->is_text()) ?
                               menus.text_part_menu() :
                               (model_volume != nullptr && model_volume->is_svg()) ? menus.svg_part_menu() : menus.part_menu();
                } else
                    menu = menus.multi_selection_menu();
            }
        }
...
void Plater::priv::set_project_name(const wxString& project_name)
{
    ...
    // update topbar title
#ifdef __WINDOWS__
    wxGetApp().mainframe->SetTitle(m_project_name + " - AdartysSlicer");
...
{
    // Adartys: reduce resolution for large bed printer
...
void Plater::priv::undo_redo_to(std::vector<UndoRedo::Snapshot>::const_iterator it_snapshot)
{
    ...
    if (it_snapshot->timestamp < this->undo_redo_stack().active_snapshot_time() ?
        ...
        this->sidebar->obj_list()->set_selection_mode(new_selected_settings_on_sidebar ?
                                                          ObjectList::SELECTION_MODE::smSettings :
                                                          new_selected_layer_on_sidebar ?
                                                          ObjectList::SELECTION_MODE::smLayer :
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "Adartys Slicer window going forward";
...
void Plater::import_model_id(wxString download_info)
{
    ...
    boost::thread import_thread = Slic3r::create_thread(
        [&percent, &cont, &cancel, &retry_count, max_retries, &msg, &target_path, &download_ok, download_url, &filename] {
            // Adartys: NetworkAgent is not needed and only prevents this from running
                ...
                http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                    ...
                    .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                        ...
                        if (retry_count == max_retries) {
                            msg  = _L("Importing to Adartys Slicer failed. Please download the file and manually import it.");
                            cont = false;
                        }
                    })
    ...
    if (download_ok) {
        ...
        /* load project */
        // Adartys: If download is a zip file, treat it as if file has been drag and dropped on the plater
...
void Plater::_calib_pa_pattern(const Calib_Params& params)
{
    ...
    // Adartys: find acceleration to use in the test
    ...
    // Adartys: Set all accelerations except first layer, as the first layer accel doesnt affect the PA test since accel
    ...
    // Adartys: find jerk value to use in the test
    if (print_config.option<ConfigOptionFloat>("default_jerk")->value > 0) {          // we have set a jerk value
        ...
        // Adartys: Set jerk values. Again first layer jerk should not matter as it is reset to the travel jerk before the
    ...
    // Adartys: Set the outer wall speed to the optimal speed for the test, cap it with max volumetric speed
    if (speeds.empty()) {
...
void adjust_settings_for_flowrate_calib(ModelObjectPtrs& objects, bool linear, int pass)
{
    ...
    for (auto _obj : objects) {
        ...
        if (obj_name[0] == 'm')
            obj_name[0] = '-';
        // Adartys: force set locale to C to avoid parsing error
...
void Plater::calib_flowrate(bool is_linear, int pass)
{
    ...
    wxString calib_name;
    if (is_linear) {
        calib_name = L"Adartys YOLO Flow Calibration";
        if (pass == 2)
    ...
    if (is_linear) {
        if (pass == 1)
            add_model(false,
                      (boost::filesystem::path(Slic3r::resources_dir()) / "calib" / "filament_flow" / "Adartys-LinearFlow.3mf").string());
        else
            add_model(false, (boost::filesystem::path(Slic3r::resources_dir()) / "calib" / "filament_flow" / "Adartys-LinearFlow_fine.3mf")
                                 .string());
    } else {
        if (pass == 1)
            add_model(false,
...
void Plater::calib_input_shaping_freq(const Calib_Params& params)
{
    ...
    if (const auto option = printer_config->option<ConfigOptionFloats>("machine_max_jerk_x"); option != nullptr && !option->values.empty()) {
        machine_max_jerk_x = option->values.front();
    }
    float machine_max_jerk_y = 0.f;
    if (const auto option = printer_config->option<ConfigOptionFloats>("machine_max_jerk_y"); option != nullptr && !option->values.empty()) {
...
void Plater::load_gcode(const wxString& filename)
{
    ...
    *current_result = std::move(processor.extract_result());
    ...
    // Adartys: Fix crash when loading gcode file multiple times
    if (m_only_gcode) {
    ...
    , m_action(2)
{
    ...
    // icon
    std::string icon_path = (boost::format("%1%/images/AdartysSlicerTitle.ico") % resources_dir()).str();
    ...
    // Adartys: hide the "Don't show again" checkbox, people keeps accidentally checked this then forgot
...
bool Plater::load_files(const wxArrayString& filenames)
{
    ...
    // Adartys: Iters through given paths and imports files from zip then remove zip from paths
...
void Plater::convert_unit(ConversionType conv_type)
{
    ...
    TakeSnapshot snapshot(this, conv_type == ConversionType::CONV_FROM_INCH ?
                                    "Convert from imperial units" :
                                    conv_type == ConversionType::CONV_TO_INCH ?
                                    "Revert conversion from imperial units" :
...
void publish(Model& model, SaveStrategy strategy)
{
    ...
    // Adartys: don't show this in silence mode
...
void Plater::reslice()
{
    ...
    // Adartys: regenerate CalibPressureAdvancePattern custom G-code to apply changes
    if (model().calib_pa_pattern) {
...
void Plater::on_config_change(const DynamicPrintConfig& config)
{
    ...
    for (auto opt_key : diff_keys) {
        ...
        // Adartys: update when *_filament changed
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8(
            "\n" + _L("Tips:") + "\n" +
            _L("\"Fix Model\" feature is currently only on Windows. Please repair the model on Adartys Slicer(windows) or CAD softwares."));
    }
#endif // APPLE & LINUX
