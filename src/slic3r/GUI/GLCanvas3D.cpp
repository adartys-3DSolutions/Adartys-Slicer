...
void GLCanvas3D::set_config(const DynamicPrintConfig* config)
{
    ...
    // Adartys: Filament shrinkage compensation
...
void GLCanvas3D::enable_legend_texture(bool enable) { m_gcode_viewer.enable_legend(enable); }

void GLCanvas3D::enable_picking(bool enable)
{
    m_picking_enabled = enable;

    // Adartys: invalidate hover state when dragging is toggled, otherwise if we turned off dragging
...
void GLCanvas3D::on_size(wxSizeEvent& evt) { m_dirty = true; }
...
void GLCanvas3D::on_mouse(wxMouseEvent& evt)
{
    ...
    if (evt.Entering()) {
        ...
        // do not process the dragging if the left mouse was set down in another canvas
            ...
            // Adartys: Sphere rotation for painting view
        ...
        // BBS Select plate in this 3D canvas.
            ...
            // ADARTYS allow right click on empty space while an object selected
...
void GLCanvas3D::_render_3d_navigator()
{
    ...
    strcpy(style.AxisLabels[ImGuizmo::Axis::Axis_X], "Y"); // ADARTYS use uppercase to match text on tranform widgets
    strcpy(style.AxisLabels[ImGuizmo::Axis::Axis_Y], "Z"); // ADARTYS use uppercase to match text on tranform widgets
    strcpy(style.AxisLabels[ImGuizmo::Axis::Axis_Z], "X"); // ADARTYS use uppercase to match text on tranform widgets
...
bool GLCanvas3D::_init_main_toolbar()
{
    ...
    item.sprite_id++;
    item.left.toggable = true; // ADARTYS Closes popup if other toolbar icon clicked and it allows closing popup when clicked its button
    item.left.action_callback = [this]() {
        if (m_canvas != nullptr)
...
bool GLCanvas3D::_init_select_plate_toolbar()
{
    ...
    // ADARTYS add dark mode support and load images with 2x resolution to prevent blurry image on hi-dpi screens
...
void GLCanvas3D::_picking_pass()
{
    ...
    // Adartys: ignore clipping plane if not applying
...
void GLCanvas3D::_render_imgui_select_plate_toolbar()
{
    ...
    PartPlateList& plate_list        = wxGetApp().plater()->get_partplate_list();
    size_t         sliced_plates_cnt = 0; // ADARTYS make it accesable for other conditions
    ...
    // ADARTYS simplify and correct window size and margin calculations and get values from style
    ImGuiWrapper& imgui = *wxGetApp().imgui();
    ...
    ImVec4 button_active = ImGuiWrapper::COL_ADARTYS; // ADARTYS: Use adartys color for selected sliced plate border
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, m_is_dark ? ImVec4(.9f, .9f, .9f, 1) :
                                                     ImVec4(.3f, .3f, .3f, 1)); // ADARTYS Plate number text > Add support for dark mode
    ImGui::PushStyleColor(ImGuiCol_WindowBg, window_bg);
    ImGui::PushStyleColor(ImGuiCol_ScrollbarBg,
                          ImVec4(0.f, 0.f, 0.f,
                                 0.f)); // ADARTYS using background color with opacity creates a second color. This prevents secondary color
    ...
    imgui.set_next_window_pos(canvas_w * 0 + 5.0f * f_scale, canvas_h * 0 + y_offset, ImGuiCond_Always, 0,
                              0); // ADARTYS Add slight gap on left edge so toolbar looks like floating and it creates separation with sidebar
    ...
    if (m_sel_plate_toolbar.show_stats_item) {
        ...
        // ADARTYS add more useful information about slicing process and use progress bar on stats button
        std::string text_top;
    ...
    for (int i = 0; i < m_sel_plate_toolbar.m_items.size(); i++) {
        ...
        // draw text
        GImGui->FontSize      = 18.0f * f_scale; // ADARTYS fix font scaling
        ImVec2 text_start_pos = ImVec2(start_pos.x + 4.0f * f_scale,
                                       start_pos.y + 2.0f * f_scale); // ADARTYS move close to corner to prevent overlapping with preview
        ImGui::RenderText(text_start_pos, std::to_string(i + 1).c_str());
        ImGui::SetWindowFontScale(1.2f); // ADARTYS fix font scaling
...
void GLCanvas3D::_render_return_toolbar() const
{
    ...
    ImVec2 button_icon_size = ImVec2(font_size * .55, font_size * 1.1); // ADARTYS
    ...
    ImGui::PushStyleColor(ImGuiCol_Button, bg_color); // ADARTYS Toolbar color
...
void GLCanvas3D::_render_paint_toolbar() const
{
    ...
    ImGui::PushStyleColor(ImGuiCol_WindowBg,
                          m_is_dark ? ImGuiWrapper::COL_TOOLBAR_BG_DARK : ImGuiWrapper::COL_TOOLBAR_BG); // ADARTYS Toolbar color
...
void GLCanvas3D::_render_assemble_info() const
{
    ...
    ImGuiWrapper::push_common_window_style(get_scale()); // ADARTYS use window style for popups with title
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, ctxt.layers.size(), grain_size), [&ctxt, &new_volume, is_selected_separate_extruder,
        ...
        auto select_geometry = [&ctxt, &geometries](size_t layer_idx, int extruder, int feature) -> GLModel::Geometry& {
            return geometries[ctxt.color_by_color_print() ?
                                  ...
                                  ctxt.color_by_tool() ? std::min<int>(ctxt.number_tools() - 1, std::max<int>(extruder - 1, 0)) : feature];
        };
...
void GLCanvas3D::_set_warning_notification(EWarning warning, bool state)
{
    ...
    case EWarning::GCodeConflict: {
        ...
        text = (boost::format(_u8L("Conflicts of G-code paths have been found at layer %d, z = %.2lf mm. Please separate the conflicted "
                                   "objects farther (%s <-> %s).")) %
