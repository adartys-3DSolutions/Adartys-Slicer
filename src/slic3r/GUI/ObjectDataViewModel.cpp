...
void ObjectDataViewModelNode::set_action_icon(bool enable)
{
    ...
    m_action_icon_name = m_type & itPlate ?
                             undo :
                             m_type & itObject ? undo : m_type & (itVolume | itLayer) ? undo : /*m_type & itInstance*/ "set_separate_obj";
...
void ObjectDataViewModel::UpdateBitmapForNode(ObjectDataViewModelNode* node)
{
    ...
    if (!node->has_warning_icon() && !node->has_lock()) {
        node->SetBitmap(is_volume_node ?
                            (node->is_text_volume() ? m_text_volume_bmps.at(vol_type) :
                                                      node->is_svg_volume() ? m_svg_volume_bmps.at(vol_type) : m_volume_bmps.at(vol_type)) :
                            m_empty_bmp);
        return;
    }
    ...
    if (bmp == nullptr) {
        ...
        if (node->has_lock()) {
            if (!bmps.empty()) // ADARTYS: Add spacing between icons if there are multiple
        ...
        if (is_volume_node) {
            ...
            bmps.emplace_back(node->is_text_volume() ? m_text_volume_bmps[vol_type] :
                                                       node->is_svg_volume() ? m_svg_volume_bmps[vol_type] : m_volume_bmps[vol_type]);
        }
        bmp = m_bitmap_cache->insert(scaled_bitmap_name, bmps);
    }
