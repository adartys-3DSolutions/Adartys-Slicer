...
void MsgDialog::on_dpi_changed(const wxRect& suggested_rect)
{
    if (m_buttons.size() > 0) {
        ...
        while (i != m_buttons.end()) {
            ...
            /* Adartys not required since all buttons has same size and Rescale re applies its style
            wxSize     bsize;
...
void MsgDialog::apply_style(long style)
{
    ...
    logo->SetBitmap(
        create_scaled_bitmap(style & wxAPPLY ?
                                 "completed" :
                                 style & wxICON_WARNING ?
                                 "exclamation" : // Adartys "exclamation" used for dialogs "obj_warning" used for 16x16 areas
                                     style & wxICON_INFORMATION ? "info" : style & wxICON_QUESTION ? "question" : "AdartysSlicer",
                             this, 64, style & wxICON_ERROR));
}
...
static void add_msg_content(
    wxWindow* parent, wxBoxSizer* content_sizer, wxString msg, bool monospaced_font = false, bool is_marked_msg = false)
{
    ...
    if (msg.Contains("<tr>")) {
        ...
        page_size = wxSize(std::min(msg_sz.GetX(), 68 * em), std::min(msg_sz.GetY(), 68 * em));
    }
    ...
    , msg(msg)
{
    ...
    logo->SetBitmap(create_scaled_bitmap("AdartysSlicer_192px_grayscale.png", this, monospaced_font ? 48 : /*1*/ 84));
...
wxBoxSizer* Newer3mfVersionDialog::get_msg_sizer()
{
    ...
    if (file_version_newer) {
        text1                  = new wxStaticText(this, wxID_ANY,
                                 _L("The 3mf file version is in Beta and it is newer than the current AdartysSlicer version."));
        wxStaticText*    text2 = new wxStaticText(this, wxID_ANY, _L("If you would like to try Adartys Slicer Beta, you may click to"));
        ...
        text1 = new wxStaticText(this, wxID_ANY, _L("The 3mf file version is newer than the current Adartys Slicer version."));
        wxStaticText* text2 = new wxStaticText(this, wxID_ANY,
                                               _L("Update your Adartys Slicer could enable all functionality in the 3mf file."));
        horizontal_sizer->Add(text2, 0, wxEXPAND, 0);
    }
