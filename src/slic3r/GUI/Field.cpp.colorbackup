...
void TextCtrl::BUILD()
{
    ...
    if (!m_opt.multiline) {
        ...
        // Adartys: adds logic that scrolls the parent if the text control doesn't have focus
...
void CheckBox::BUILD()
{
    ...
    bool check_value = m_opt.type == coBool ?
                           m_opt.default_value->getBool() :
                           m_opt.type == coBools ? m_opt.get_default_value<ConfigOptionBools>()->get_at(m_opt_idx) : false;
...
void Choice::BUILD()
{
    ...
    temp->Bind(
        ...
        temp->GetId());
    temp->Bind(
        wxEVT_COMBOBOX_DROPDOWN, [this](wxCommandEvent&) { m_is_dropped = true; }, temp->GetId());
    temp->Bind(
        wxEVT_COMBOBOX_CLOSEUP, [this](wxCommandEvent&) { m_is_dropped = false; }, temp->GetId());

    temp->Bind(
        wxEVT_COMBOBOX, [this](wxCommandEvent&) { on_change_field(); }, temp->GetId());
...
{
    auto size = wxSize(def_width_wider() * m_em_unit, -1); // ADARTYS match color picker width
    if (m_opt.height >= 0)
        size.SetHeight(m_opt.height * m_em_unit);
    if (m_opt.width >= 0)
        size.SetWidth(m_opt.width * m_em_unit);
    ...
    // ADARTYS reset value to default on right click. previously no way to switch back on windows
...
// ADARTYS match style with button on windows
void ColourPicker::draw_bmp_btn(wxColourPickerCtrl* field, wxColour color)
{
    ...
    if (!btn->GetBitmap().IsOk())
        return;
    btn->SetWindowStyle(wxBORDER_NONE); // ADARTYS just in case to prevent any overflow
...
void ColourPicker::msw_rescale()
{
    ...
    auto                size  = wxSize(def_width_wider() * m_em_unit, -1); // ADARTYS match color picker width with parameters
    if (m_opt.height >= 0)
        size.SetHeight(m_opt.height * m_em_unit);
    else if (parent_is_custom_ctrl && opt_height > 0)
        size.SetHeight(lround(opt_height * m_em_unit));
...
void PointCtrl::BUILD()
{
    ...
    const wxSize  field_size((m_opt.width >= 0 ? m_opt.width : def_width_wider()) * m_em_unit,
                            -1); // ADARTYS match width with other components
    ...
    //#endif
    // ADARTYS add icons to point control boxes instead of using text for X / Y
...
void PointCtrl::msw_rescale()
{
    ...
    wxSize field_size((m_opt.width >= 0 ? m_opt.width : def_width_wider()) * m_em_unit, -1); // ADARTYS match width with other components
