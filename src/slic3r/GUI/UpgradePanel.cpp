            ...
            for (std::map<std::string, Ams*>::iterator iter = obj->amsList.begin(); iter != obj->amsList.end(); iter++) {
                ...
                if (it == ver_list.end()) {
                    ...
                    if (m_obj->upgrade_display_state == (int) MachineObject::UpgradingDisplayState::UpgradingInProgress) {
                        ...
                        if (obj->new_ver_list.empty() && !obj->m_new_ver_list_exist) {
                            if (obj->upgrade_new_version && obj->ams_new_version_number.compare(it->second.sw_ver) != 0) {
...
void UpgradePanel::update(MachineObject* obj)
{
    ...
    // force upgrade
    // unlock hint
    if (m_obj && (m_obj->upgrade_display_state == (int) MachineObject::UpgradingDisplayState::UpgradingFinished) &&
        ...
        if (m_obj->upgrade_force_upgrade) {
            ...
            if (force_dlg == nullptr) {
                force_dlg = new SecondaryCheckDialog(this->GetParent(), wxID_ANY, _L("Update firmware"),
                                                     SecondaryCheckDialog::ButtonStyle::CONFIRM_AND_CANCEL, wxDefaultPosition,
                                                     wxDefaultSize);
    ...
    if (m_obj && m_show_consistency_hint) {
        if (m_obj->upgrade_consistency_request) {
            ...
            consistency_dlg->update_text(
                _L("The firmware version is abnormal. Repairing and updating are required before printing. Do you want to update now? You "
                   "can also update later on printer or update next time starting Adartys."));
