    ...
    , m_export_3mf_cancel(false)
{
    ...
    m_rename_button = new Button(m_rename_normal_panel, "", "rename_edit", wxBORDER_NONE,
                                 FromDIP(13)); // ADARTYS Match edit icon and its size
...
void SendToPrinterDialog::show_status(PrintDialogStatus status, std::vector<wxString> params)
{
    ...
    if (status == PrintDialogStatus::PrintStatusInit) {
        ...
        wxString msg_text = _L("The printer is required to be in the same LAN as Adartys Slicer.");
        update_print_status_msg(msg_text, true, true);
        Enable_Send_Button(false);
        Enable_Refresh_Button(true);
...
SendToPrinterDialog::~SendToPrinterDialog() { delete m_refresh_timer; }
