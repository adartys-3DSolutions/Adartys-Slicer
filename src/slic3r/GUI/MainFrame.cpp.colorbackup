...
#ifdef __APPLE__
class AdartysSlicerTaskBarIcon : public wxTaskBarIcon
{
public:
    AdartysSlicerTaskBarIcon(wxTaskBarIconType iconType = wxTBI_DEFAULT_TYPE) : wxTaskBarIcon(iconType) {}
...
#else  // _WIN32
    return wxIcon(Slic3r::var("AdartysSlicer_128px.png"), wxBITMAP_TYPE_PNG);
#endif // _WIN32
}
    ...
    , diff_dialog(this)
{
    ...
    */
    ...
    case GUI_App::EAppMode::Editor:
        m_taskbar_icon = std::make_unique<AdartysSlicerTaskBarIcon>(wxTBI_DOCK);
        m_taskbar_icon->SetIcon(wxIcon(Slic3r::var("AdartysSlicer-mac_256px.ico"), wxBITMAP_TYPE_ICO), "AdartysSlicer");
        break;
...
#endif
        {
            // Adartys: Use GUI_App::open_preferences instead of direct call so windows associations are updated on exit
...
// Adartys: Fix maximized window overlaps taskbar when taskbar auto hide is enabled (#8085)
...
// See https://github.com/SoftFever/AdartysSlicer/issues/8152
...
static wxMenu* generate_help_menu()
{
    ...
    // Report a bug
    // append_menu_item(helpMenu, wxID_ANY, _L("Report Bug(TODO)"), _L("Report a bug of AdartysSlicer"),
    //    [](wxCommandEvent&) {
    //        //TODO
    //    });
    // Check New Version
...
void MainFrame::init_menubar_as_editor()
{
    ...
    // auto preference_item = new wxMenuItem(parent_menu, AdartysSlicerMenuPreferences + bambu_studio_id_base, _L("Preferences") + "\t" + ctrl
    // + ",", "");
#else
...
#ifdef __APPLE__
    wxString about_title = wxString::Format(_L("&About %s"), SLIC3R_APP_FULL_NAME);
    // auto about_item = new wxMenuItem(parent_menu, AdartysSlicerMenuAbout + bambu_studio_id_base, about_title, "");
    // parent_menu->Bind(wxEVT_MENU, [this, bambu_studio_id_base](wxEvent& event) {
    //    switch (event.GetId() - bambu_studio_id_base) {
    //        case AdartysSlicerMenuAbout:
    //            Slic3r::GUI::about();
    //            break;
    //        case AdartysSlicerMenuPreferences:
    //            CallAfter([this] {
    //                PreferencesDialog dlg(this);
    //                dlg.ShowModal();
    //#if ENABLE_GCODE_LINES_ID_IN_H_SLIDER
    ...
    append_menu_item(
        flowrate_menu, wxID_ANY, _L("YOLO (Recommended)"), _L("Adartys YOLO flowrate calibration, 0.01 step"),
        [this](wxCommandEvent&) {
            if (m_plater)
        ...
        this);
    append_menu_item(
        flowrate_menu, wxID_ANY, _L("YOLO (perfectionist version)"), _L("Adartys YOLO flowrate calibration, 0.005 step"),
        [this](wxCommandEvent&) {
            if (m_plater)
...
void MainFrame::load_config_file()
{
    ...
    wxFileDialog  dlg(this, _L("Select profile to load:"),
                     ...
                     "Config files (*.json;*.zip;*.adartys_printer;*.adartys_filament)|*.json;*.zip;*.adartys_printer;*.adartys_filament",
        ...
        if (m_tabpanel->GetSelection() != (int) new_selection)
            ...
            /*if (wxGetApp().tabs_as_menu()) {
                if (Tab* cur_tab = dynamic_cast<Tab*>(m_tabpanel->GetPage(new_selection)))
                    update_marker_for_tabs_menu((m_layout == ESettingsLayout::Old ? m_menubar : m_settings_dialog.menubar()),
            cur_tab->title(), m_layout == ESettingsLayout::Old); else if (tab == 0 && m_layout == ESettingsLayout::Old)
...
#else
    SetIcon(wxIcon(var("AdartysSlicer_128px.png"), wxBITMAP_TYPE_PNG));
#endif // _WIN32
