...
bool is_associate_files(std::wstring extend)
{
    ...
    std::wstring prog_id       = L" Adartys.Slicer.1";
...
public:
          ...
          )
    {
        ...
        // ADARTYS scale all fonts with monitor scale
...
static void generic_exception_handle()
{
    ...
    try {
        ...
        flush_logs();
        wxString errmsg = wxString::Format(_L("AdartysSlicer will terminate because of running out of memory. "
        ...
        flush_logs();
        wxString errmsg = _L("AdartysSlicer will terminate because of a localization error. "
...
{
    // app config initializes early becasuse it is used in instance checking in AdartysSlicer.cpp
                ...
                if (obj) {
                    if (obj->is_lan_mode_printer()) {
                        if (state == ConnectStatus::ConnectStatusOk) {
                            ...
                            // Adartys: avoid showing same error message multiple times until next connection attempt.
...
static boost::optional<Semver> parse_semver_from_ini(std::string path)
{
    ...
    size_t      start = body.find("AdartysSlicer ");
    if (start == std::string::npos) {
        start = body.find("AdartysSlicer ");
        if (start == std::string::npos)
            return boost::none;
    }
...
void GUI_App::init_webview_runtime()
{
    ...
    if (!WebView::CheckWebViewRuntime()) {
        int nRet = wxMessageBox(
            _L("Adartys Slicer requires the Microsoft WebView2 Runtime to operate certain features.\nClick Yes to install it now."),
...
void GUI_App::init_app_config()
{
    ...
    if (data_dir().empty()) {
        // Adartys: check if data_dir folder exists in application folder use it if it exists
    ...
    if (m_app_conf_exists) {
        ...
        if (!error.empty()) {
            // Adartys: if the config file is corrupted, we will show a error dialog and create a default config file.
...
int GUI_App::OnExit()
{
    ...
    // Adartys: clean up encrypted bbl network log file if plugin is used
...
bool GUI_App::on_init_inner()
{
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("gui mode, Current AdartysSlicer Version %1%") % SoftFever_VERSION;
    ...
    if (is_editor()) {
        ...
        Bind(EVT_ENTER_FORCE_UPGRADE, [this](const wxCommandEvent& evt) {
            ...
            DownloadDialog dialog(this->mainframe, tips, _L("The Adartys Slicer needs an upgrade"), false, wxCENTER | wxICON_INFORMATION);
    ...
    // Adartys: select network plugin version
...
please delete installed plugin and try again!");
    ...
    if (m_config_corrupted) {
        ...
        show_error(
            nullptr,
            _u8L("The AdartysSlicer configuration file may be corrupted and cannot be parsed.\nAdartysSlicer has attempted to recreate the "
...
void GUI_App::init_label_colours()
{
    ...
    m_color_highlight_default = is_dark_mode ? wxColour("#36363B") : wxColour("#F1F1F1"); // ADARTYS row highlighting
            ...
            if (btn->GetId() == wxID_OK || btn->GetId() == wxID_CANCEL) {
                ...
                auto mark_button = [this, btn, highlited](const bool mark) {
                    btn->SetBackgroundColour(mark ? m_color_selected_btn_bg :
                                                    highlited ? m_color_highlight_default : m_color_window_default);
...
void GUI_App::on_http_error(wxCommandEvent& evt)
{
    ...
    if (code == HttpErrorVersionLimited) {
        MessageDialog msg_dlg(
            nullptr,
            _L("The version of Adartys Slicer is too low and needs to be updated to the latest version before it can be used normally."),
            "", wxAPPLY | wxOK);
        if (msg_dlg.ShowModal() == wxOK) {}
    }
...
{
    // Adartys: do not connect to default device during app startup, because some of the lan machines might not online yet
...
{
    // Adartys: alaways disable track event
    if (m_agent) {
...
{
    return; // adartys: not used, see check_new_version_sf
...
std::string detect_updater_os_info()
{
    ...
    if (description.empty())
        ...
        // Adartys: workaround: wxGetOsVersion can't recognize Windows 11
...
{
#if ADARTYS_UPDATER_SIG_KEY_AVAILABLE
    std::string key = ADARTYS_UPDATER_SIG_KEY_B64;
...
void maybe_attach_updater_signature(Http& http, const std::string& canonical_query, const std::string& request_url)
{
    ...
    http.header("X-Adartys-Ts", timestamp);
    http.header("X-Adartys-Sig", "v1:" + signature);
}
...
void GUI_App::start_sync_user_preset(bool with_progress_dlg)
{
    ...
    m_sync_update_thread = Slic3r::create_thread([this, progressFn, cancelFn, finishFn, t = std::weak_ptr<int>(m_user_sync_token)] {
        ...
        std::string version = preset_bundle->get_vendor_profile_version(PresetBundle::ADARTYS_DEFAULT_BUNDLE).to_string();
        if (!m_agent)
            return;
...
bool GUI_App::load_language(wxString language, bool initial)
{
    ...
    if (initial) {
        ...
        if (!language.empty())
            BOOST_LOG_TRIVIAL(info) << boost::format("language provided by AdartysSlicer.conf: %1%") % language;
        else {
    ...
    if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
        BOOST_LOG_TRIVIAL(trace)
            << boost::format("The following language code requires right to left layout, which is not supported by AdartysSlicer: %1%") %
...
void GUI_App::save_mode(const /*ConfigOptionMode*/ int mode)
{
    ...
    const std::string mode_str = mode == comAdvanced ? "advanced" :
                                                       mode == comSimple ? "simple" : mode == comDevelop ? "develop" : "simple";
...
void GUI_App::OSXStoreOpenFiles(const wxArrayString& fileNames)
{
    ...
    if (fileNames.size() == num_gcodes) {
        // Opening PrusaSlicer by drag & dropping a G-Code onto AdartysSlicer icon in Finder,
...
bool GUI_App::run_wizard(ConfigWizard::RunReason reason, ConfigWizard::StartPage start_page)
{
    ...
    auto       page = start_page == ConfigWizard::SP_WELCOME ?
                    GuideFrame::BBL_WELCOME :
                    start_page == ConfigWizard::SP_FILAMENTS ?
                    GuideFrame::BBL_FILAMENT_ONLY :
                    start_page == ConfigWizard::SP_PRINTERS ? GuideFrame::BBL_MODELS_ONLY : GuideFrame::BBL_MODELS;
...
void GUI_App::associate_files(std::wstring extend)
{
    ...
    std::wstring prog_id             = L" Adartys.Slicer.1";
    std::wstring prog_desc           = L"AdartysSlicer";
