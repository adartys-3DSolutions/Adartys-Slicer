                ...
                wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    ...
    m_gcode_editor = new wxTextCtrl(this, wxID_ANY, value, wxDefaultPosition, wxDefaultSize,
                                    wxTE_MULTILINE
#ifdef _WIN32
                                        | wxBORDER_SIMPLE
#endif
...
wxDataViewItem EditGCodeDialog::add_presets_placeholders()
{
    ...
    // Adartys: create subgroups from the pages of the tabs
    auto init_from_tab = [this, full_config](wxDataViewItem parent, Tab* tab, const set<string>& preset_keys) {
        ...
        for (const auto& page : tab->m_pages) {
            // ADARTYS: Pull icons from tabs for subgroups, icons are hidden on tabs
...
void EditGCodeDialog::selection_changed(wxDataViewEvent& evt)
{
    ...
    if (!opt_key.empty()) {
        ...
        // Adartys: move below checking for def in custom defined G-code placeholders
        ...
        if (def) {
            ...
            wxString               type_str    = scalar_type == coNone ?
                                    "none" :
                                    scalar_type == coFloat ?
                                    "float" :
                                    scalar_type == coInt ?
                                    "integer" :
                                    scalar_type == coString ?
                                    "string" :
                                    scalar_type == coPercent ?
                                    "percent" :
                                    scalar_type == coFloatOrPercent ?
                                    "float or percent" :
                                    scalar_type == coPoint ? "point" :
                                                             scalar_type == coBool ? "bool" : scalar_type == coEnum ? "enum" : "undef";
            if (!def->is_scalar())
                type_str += "[]";
...
wxDataViewItem ParamsModel::Delete(const wxDataViewItem& item)
{
    ...
    // Adartys: notify enabled item only, because disabled items have already been removed from UI,
