...
Bed_2D::Bed_2D(wxWindow* parent) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(32 * wxGetApp().em_unit(), -1), wxTAB_TRAVERSAL)
{
#ifdef __APPLE__
...
{
    // Adartys: use 500 x 500 bed size as baseline.
    ...
    const ExPolygon& poly, const BoundingBox& bb, const Vec2d& origin, const double& step, const double& scale)
{
    ...
    // ADARTYS draw grid lines relative to origin
...
void Bed_2D::repaint(const std::vector<Vec2d>& shape)
{
    ...
    dc.SetPen(wxPen(wxColour(encode_color(ColorRGB::X())), 2, wxPENSTYLE_SOLID)); // red // ADARTYS match axis colors
    auto x_end = Vec2d(origin_px(0) + axes_len, origin_px(1));
    dc.DrawLine(wxPoint(origin_px(0), origin_px(1)), wxPoint(x_end(0), x_end(1)));
    // for (auto angle : { -arrow_angle, arrow_angle }) {  // ADARTYS dont draw arrows
    ...
    dc.SetPen(wxPen(wxColour(encode_color(ColorRGB::Y())), 2, wxPENSTYLE_SOLID)); // green // ADARTYS match axis colors
    ...
    // draw origin
    dc.SetPen(wxPen(wxColour(encode_color(ColorRGB::Z())), 1, wxPENSTYLE_SOLID));    // ADARTYS match axis colors
    dc.SetBrush(wxBrush(wxColour(encode_color(ColorRGB::Z())), wxBRUSHSTYLE_SOLID)); // ADARTYS match axis colors
    ...
    const auto origin_label_x = origin_px(0) + 2; // ADARTYS always draw (0,0) text in axes bounding box
    ...
    dc.DrawRectangle(wxPoint(origin_label_x, origin_label_y),
                     extent); // ADARTYS draw a background to origin position text to improve readability when overlaps with grid
    dc.DrawText(origin_label, origin_label_x, origin_label_y);

    // ADARTYS add grid size value as information for large scale beds
