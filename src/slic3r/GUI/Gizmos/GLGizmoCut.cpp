...
void GLGizmoCut3D::render_cut_plane_grabbers()
{
    ...
    if (!no_xy_dragging && m_hover_id != CutPlaneZRotation && m_hover_id != CutPlaneXMove && m_hover_id != CutPlaneYMove) {
        ...
        color = m_hover_id == Y ? ColorRGBA::Y() :     // ADARTYS match axis colors
                    m_hover_id == X ? ColorRGBA::X() : // ADARTYS match axis colors
    ...
    if (no_xy_grabber_hovered || m_hover_id == X) {
        ...
        color = ColorRGBA::X(); // ADARTYS match axis colors
    ...
    if (no_xy_grabber_hovered || m_hover_id == Y) {
        ...
        color = ColorRGBA::Y(); // ADARTYS match axis colors
    ...
    if (CutMode(m_mode) == CutMode::cutTongueAndGroove) {
        ...
        if (no_xy_grabber_hovered || m_hover_id == CutPlaneZRotation) {
            ...
            color                    = ColorRGBA::Z(); // ADARTYS match axis colors
        ...
        if (no_xy_grabber_hovered || m_hover_id == CutPlaneXMove) {
            size  = (m_dragging ? get_dragging_half_size(mean_size) : get_half_size(mean_size));
            color = m_hover_id == CutPlaneXMove ? ColorRGBA::X() : m_plane.model.get_color(); // ADARTYS match axis colors
        ...
        if (m_groove.angle > 0.0f && (no_xy_grabber_hovered || m_hover_id == CutPlaneYMove)) {
            size  = (m_dragging ? get_dragging_half_size(mean_size) : get_half_size(mean_size));
            color = m_hover_id == CutPlaneYMove ? ColorRGBA::Y() : m_plane.model.get_color(); // ADARTYS match axis colors
...
void GLGizmoCut3D::dragging_grabber_move(const GLGizmoBase::UpdateData& data)
{
    ...
    Vec3d starting_vec = m_rotation_m *
                         (m_hover_id == CutPlaneXMove ? Vec3d::UnitX() : m_hover_id == CutPlaneYMove ? Vec3d::UnitY() : Vec3d::UnitZ());
...
void GLGizmoCut3D::render_build_size()
{
    ...
    wxString size   = "X: " + double_to_string(tbb_sz.x() * koef, 2) + unit_str + ",  Y: " + double_to_string(tbb_sz.y() * koef, 2) +
                    unit_str + ",  Z: " + double_to_string(tbb_sz.z() * koef, 2) + unit_str;
...
bool GLGizmoCut3D::is_outside_of_cut_contour(size_t idx, const CutConnectors& connectors, const Vec3d cur_pos)
{
    ...
    const int               sectorCount   = shape == CutConnectorShape::Triangle ?
                                3 :
                                shape == CutConnectorShape::Square ?
                                4 :
...
void GLGizmoCut3D::render_connectors()
{
    ...
    for (size_t i = 0; i < connectors.size(); ++i) {
        ...
        if (!m_connectors_editing)
            ...
            render_color = conflict_connector ?
                               HOVERED_ERR_COLOR :
