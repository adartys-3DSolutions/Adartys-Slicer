...
std::vector<size_t> GLGizmosManager::get_selectable_idxs() const
{
    ...
    if (m_parent.get_canvas_type() == GLCanvas3D::CanvasAssembleView) {
        for (size_t i = 0; i < m_gizmos.size(); ++i)
            if (m_gizmos[i]->get_sprite_id() == (unsigned int) Move || m_gizmos[i]->get_sprite_id() == (unsigned int) Rotate ||
                m_gizmos[i]->get_sprite_id() == (unsigned int) Measure || m_gizmos[i]->get_sprite_id() == (unsigned int) Assembly ||
...
void GLGizmosManager::switch_gizmos_icon_filename()
{
    ...
    if (!m_background_texture.metadata.filename.empty())
        m_background_texture.texture.load_from_file(resources_dir() + "/images/" + m_background_texture.metadata.filename, false,
                                                    GLTexture::SingleThreaded, false);

    for (auto& gizmo : m_gizmos) {
        gizmo->on_change_color_mode(m_is_dark);
        switch (gizmo->get_sprite_id()) {
        case (EType::Move): gizmo->set_icon_filename(m_is_dark ? "toolbar_move_dark.svg" : "toolbar_move.svg"); break;
        case (EType::Rotate): gizmo->set_icon_filename(m_is_dark ? "toolbar_rotate_dark.svg" : "toolbar_rotate.svg"); break;
        case (EType::Scale): gizmo->set_icon_filename(m_is_dark ? "toolbar_scale_dark.svg" : "toolbar_scale.svg"); break;
        case (EType::Flatten): gizmo->set_icon_filename(m_is_dark ? "toolbar_flatten_dark.svg" : "toolbar_flatten.svg"); break;
        case (EType::Cut): gizmo->set_icon_filename(m_is_dark ? "toolbar_cut_dark.svg" : "toolbar_cut.svg"); break;
        case (EType::FdmSupports): gizmo->set_icon_filename(m_is_dark ? "toolbar_support_dark.svg" : "toolbar_support.svg"); break;
        case (EType::Seam): gizmo->set_icon_filename(m_is_dark ? "toolbar_seam_dark.svg" : "toolbar_seam.svg"); break;
        case (EType::Emboss): gizmo->set_icon_filename(m_is_dark ? "toolbar_text_dark.svg" : "toolbar_text.svg"); break;
        case (EType::MmSegmentation): gizmo->set_icon_filename(m_is_dark ? "mmu_segmentation_dark.svg" : "mmu_segmentation.svg"); break;
        case (EType::FuzzySkin):
            gizmo->set_icon_filename(m_is_dark ? "toolbar_fuzzy_skin_paint_dark.svg" : "toolbar_fuzzy_skin_paint.svg");
            break;
        case (EType::MeshBoolean): gizmo->set_icon_filename(m_is_dark ? "toolbar_meshboolean_dark.svg" : "toolbar_meshboolean.svg"); break;
        case (EType::Measure): gizmo->set_icon_filename(m_is_dark ? "toolbar_measure_dark.svg" : "toolbar_measure.svg"); break;
        case (EType::Assembly): gizmo->set_icon_filename(m_is_dark ? "toolbar_assembly_dark.svg" : "toolbar_assembly.svg"); break;
        case (EType::BrimEars): gizmo->set_icon_filename(m_is_dark ? "toolbar_brimears_dark.svg" : "toolbar_brimears.svg"); break;
        }
    }
}
...
bool GLGizmosManager::init()
{
    ...
    if (!m_background_texture.metadata.filename.empty()) {
        if (!m_background_texture.texture.load_from_file(resources_dir() + "/images/" + m_background_texture.metadata.filename, false,
                                                         GLTexture::SingleThreaded, false))
            return false;
    }
    ...
    m_gizmos.emplace_back(
        new GLGizmoMove3D(m_parent, m_is_dark ? "toolbar_move_dark.svg" : "toolbar_move.svg", EType::Move, &m_object_manipulation));
    m_gizmos.emplace_back(
        new GLGizmoRotate3D(m_parent, m_is_dark ? "toolbar_rotate_dark.svg" : "toolbar_rotate.svg", EType::Rotate, &m_object_manipulation));
    m_gizmos.emplace_back(
    ...
    m_gizmos.emplace_back(
        new GLGizmoMeshBoolean(m_parent, m_is_dark ? "toolbar_meshboolean_dark.svg" : "toolbar_meshboolean.svg", EType::MeshBoolean));
    m_gizmos.emplace_back(
        new GLGizmoFdmSupports(m_parent, m_is_dark ? "toolbar_support_dark.svg" : "toolbar_support.svg", EType::FdmSupports));
    m_gizmos.emplace_back(new GLGizmoSeam(m_parent, m_is_dark ? "toolbar_seam_dark.svg" : "toolbar_seam.svg", EType::Seam));
    m_gizmos.emplace_back(
        new GLGizmoFuzzySkin(m_parent, m_is_dark ? "toolbar_fuzzy_skin_paint_dark.svg" : "toolbar_fuzzy_skin_paint.svg", EType::FuzzySkin));
    m_gizmos.emplace_back(
        new GLGizmoMmuSegmentation(m_parent, m_is_dark ? "mmu_segmentation_dark.svg" : "mmu_segmentation.svg", EType::MmSegmentation));
...
bool GLGizmosManager::init_icon_textures()
{
    ...
    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/images/toolbar_tooltip.svg", 25, 25,
                                      texture_id)) // ADARTYS: Use same resolution with gizmos to prevent blur on icon
        icon_list.insert(std::make_pair((int) IC_TOOLBAR_TOOLTIP, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/images/toolbar_tooltip_hover.svg", 25, 25,
                                      texture_id)) // ADARTYS: Use same resolution with gizmos to prevent blur on icon
...
float GLGizmosManager::get_layout_scale() { return m_layout.scale; }
...
void GLGizmosManager::reset_all_states()
{
    ...
    // Adartys: do not clear hover state, as Emboss gizmo can be used without selection
    // m_hover = Undefined;
}
...
bool GLGizmosManager::check_gizmos_closed_except(EType type) const
{
    if (get_current_type() != type && get_current_type() != Undefined) {
        wxGetApp().plater()->get_notification_manager()->push_notification(NotificationType::CustomSupportsAndSeamRemovedAfterRepair,
...
void GLGizmosManager::update_assemble_view_data()
{
    if (m_assemble_view_data) {
        if (!wxGetApp().plater()->get_assmeble_canvas3D()->get_wxglcanvas()->IsShown())
            ...
            m_assemble_view_data->update(
                AssembleViewDataID((int) AssembleViewDataID::ModelObjectsInfo | (int) AssembleViewDataID::ModelObjectsClipper));
    }
}
...
void GLGizmosManager::update_data()
{
    ...
    // Adartys: hack: Fix issue that flatten gizmo faces not updated after reload from disk
    if (m_current != Flatten && !m_gizmos.empty())
        m_gizmos[Flatten]->data_changed(m_serializing);
...
bool GLGizmosManager::gizmo_event(SLAGizmoEventType action, const Vec2d& mouse_position, bool shift_down, bool alt_down, bool control_down)
{
    ...
    /*if (m_current == SlaSupports)
        return dynamic_cast<GLGizmoSlaSupports*>(m_gizmos[SlaSupports].get())->gizmo_event(action, mouse_position, shift_down, alt_down,
    control_down); else if (m_current == Hollow) return dynamic_cast<GLGizmoHollow*>(m_gizmos[Hollow].get())->gizmo_event(action,
    mouse_position, shift_down, alt_down, control_down); else*/
    if (m_current == FdmSupports)
        ...
        return dynamic_cast<GLGizmoAssembly*>(m_gizmos[Assembly].get())
            ->gizmo_event(action, mouse_position, shift_down, alt_down, control_down);
        ...
        return dynamic_cast<GLGizmoMeshBoolean*>(m_gizmos[MeshBoolean].get())
            ->gizmo_event(action, mouse_position, shift_down, alt_down, control_down);
...
{
    return m_current == EType::FdmSupports || m_current == EType::MmSegmentation || m_current == EType::FuzzySkin ||
...
void GLGizmosManager::on_change_color_mode(bool is_dark) { m_is_dark = is_dark; }
...
bool GLGizmosManager::on_mouse_wheel(const wxMouseEvent& evt)
{
    ...
    if (/*m_current == SlaSupports || m_current == Hollow ||*/ m_current == FdmSupports || m_current == Seam ||
        m_current == MmSegmentation || m_current == FuzzySkin || m_current == BrimEars) {
        float rot = (float) evt.GetWheelRotation() / (float) evt.GetWheelDelta();
        if (gizmo_event((rot > 0.f ? SLAGizmoEventType::MouseWheelUp : SLAGizmoEventType::MouseWheelDown), Vec2d::Zero(), evt.ShiftDown(),
                        evt.AltDown()
        // BBS
#ifdef __WXOSX_MAC__
                            ,
...
bool GLGizmosManager::on_char(wxKeyEvent& evt)
{
    ...
    if ((evt.GetModifiers() & ctrlMask) != 0) {
        ...
        case 'a': {
            ...
            // MmSegmentation || m_current == FuzzySkin) && gizmo_event(SLAGizmoEventType::ResetClippingPlane))
            //    processed = true;
...
//            m_parent.set_cursor(GLCanvas3D::Cross);
        ...
        if ((m_current == BrimEars) && ((keyCode == WXK_SHIFT) || (keyCode == WXK_ALT))) {
            processed = true;
        } else if (m_current == Cut) {
...
void GLGizmosManager::render_background(float left, float top, float right, float bottom, float border_w, float border_h) const
{
    ...
    if (tex_id != 0 && tex_width > 0 && tex_height > 0) {
        ...
        GLTexture::render_sub_texture(tex_id, left, right, bottom, top,
        ...
        internal_right_uv, internal_bottom_uv }, { internal_right_uv, internal_top_uv }, { internal_left_uv, internal_top_uv } });

        // top edge
        GLTexture::render_sub_texture(tex_id, internal_left, internal_right, internal_top, top, { { internal_left_uv, internal_top_uv }, {
        internal_right_uv, internal_top_uv }, { internal_right_uv, top_uv }, { internal_left_uv, top_uv } });

        // top-right corner
        GLTexture::render_sub_texture(tex_id, internal_right, right, internal_top, top, { { internal_right_uv, internal_top_uv }, {
        right_uv, internal_top_uv }, { right_uv, top_uv }, { internal_right_uv, top_uv } });
        ...
        // center
        GLTexture::render_sub_texture(tex_id, internal_left, internal_right, internal_bottom, internal_top, { { internal_left_uv,
        internal_bottom_uv }, { internal_right_uv, internal_bottom_uv }, { internal_right_uv, internal_top_uv }, { internal_left_uv,
        internal_top_uv } });

        // center-right edge
        GLTexture::render_sub_texture(tex_id, internal_right, right, internal_bottom, internal_top, { { internal_right_uv,
        internal_bottom_uv }, { right_uv, internal_bottom_uv }, { right_uv, internal_top_uv }, { internal_right_uv, internal_top_uv } });
        ...
        internal_right_uv, bottom_uv }, { internal_right_uv, internal_bottom_uv }, { internal_left_uv, internal_bottom_uv } });
...
void GLGizmosManager::render_arrow(const GLCanvas3D& parent, EType highlighted_type) const
{
    ...
    for (size_t idx : selectable_idxs) {
        if (idx == highlighted_type) {
            ...
            GLTexture::render_sub_texture(tex_id, zoomed_top_x + zoomed_icons_size * 1.2f,
                                          zoomed_top_x + zoomed_icons_size * 1.2f + zoomed_icons_size * 2.2f * arrow_sides_ratio,
                                          zoomed_top_y - zoomed_icons_size * 1.6f, zoomed_top_y + zoomed_icons_size * 0.6f,
...
void GLGizmosManager::do_render_overlay() const
{
    ...
    for (size_t idx : selectable_idxs) {
        ...
        const int icon_idx = (m_highlight.first == idx ? (m_highlight.second ? 4 : 5) :
                                                         (m_current == idx) ? 2 : ((m_hover == idx) ? 1 : (gizmo->is_activable() ? 0 : 3)));
        ...
        if (idx == m_current
            // Adartys: Show Svg dialog at the same place as emboss gizmo
...
{
    switch (type) {
    case GLGizmosManager::EType::Move: return "Move";
    case GLGizmosManager::EType::Rotate: return "Rotate";
    case GLGizmosManager::EType::Scale: return "Scale";
    case GLGizmosManager::EType::Flatten: return "Flatten";
    case GLGizmosManager::EType::Cut: return "Cut";
    case GLGizmosManager::EType::MeshBoolean: return "MeshBoolean";
    case GLGizmosManager::EType::FdmSupports: return "FdmSupports";
    case GLGizmosManager::EType::Seam: return "Seam";
    case GLGizmosManager::EType::Emboss: return "Text";
    case GLGizmosManager::EType::MmSegmentation: return "Color Painting";
    case GLGizmosManager::EType::FuzzySkin: return "Fuzzy Skin Painting";
    default: return "";
    }
    return "";
}
