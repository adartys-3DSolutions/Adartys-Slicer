...
GLGizmoSVG::GLGizmoSVG(GLCanvas3D& parent)
    : GLGizmoBase(parent, M_ICON_FILENAME, -3), m_gui_cfg(nullptr), m_rotate_gizmo(parent, GLGizmoRotate::Axis::Z) // grab id = 2 (Z axis)
{
...
bool GLGizmoSVG::is_svg(const ModelVolume& volume) { return volume.emboss_shape.has_value() && volume.emboss_shape->svg_file.has_value(); }
...
void GLGizmoSVG::on_register_raycasters_for_picking() { m_rotate_gizmo.register_raycasters_for_picking(); }
void GLGizmoSVG::on_unregister_raycasters_for_picking() { m_rotate_gizmo.unregister_raycasters_for_picking(); }
...
IconManager::VIcons init_icons(IconManager& mng, const GuiCfg& cfg)
{
    ...
    std::vector<std::string> filenames{
        ...
        "burn.svg",         // bake
        "save.svg",         // save
        "obj_warning.svg",  // exclamation // ADARTYS: use obj_warning instead exclamation. exclamation is not compatible with low res
...
void GLGizmoSVG::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    GizmoImguiBegin(on_get_name(), ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoCollapse |
...
void draw_side_outline(
    const ExPolygons& shape, const std::array<unsigned char, N>& color, std::vector<unsigned char>& data, size_t data_width, double scale)
{
...
bool init_texture(Texture&                        texture,
                  const ExPolygonsWithIds&        shapes_with_ids,
                  unsigned                        max_size_px,
    ...
    for (NSVGshape* shape = image.shapes; shape != NULL; shape = shape->next, ++shape_index) {
        ...
        if (shape->strokeWidth <= minimal_width_in_mm * scale) {
            add_warning(shape_index * 2,
                        GUI::format(_L("Stroke of shape (%1%) is too thin (minimal width is %2% mm)."), shape->id, minimal_width_in_mm));
            continue;
        }
...
float GLGizmoSVG::get_scale_for_tolerance() { return std::max(m_scale_width.value_or(1.f), m_scale_height.value_or(1.f)); }
...
void GLGizmoSVG::draw_size()
{
    ...
    if (use_inch)
        height *= GizmoObjectManipulation::mm_to_in;
...
void GLGizmoSVG::draw_distance()
{
    ...
    if (use_inch) {
        ...
        if (m_distance.has_value())
            distance_inch = (*m_distance * GizmoObjectManipulation::mm_to_in);
...
void GLGizmoSVG::draw_rotation()
{
    ...
    if (m_imgui->slider_float("##angle", &angle_deg, limits.angle.min, limits.angle.max, u8"%.2f °", 1.f, false,
