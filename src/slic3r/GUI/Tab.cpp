...
void Tab::create_preset_tab()
{
    ...
    m_search_input->Bind(wxCUSTOMEVT_EXIT_SEARCH, [this](wxCommandEvent&) {
        ...
        m_btn_delete_preset->Show(); // ADARTYS: fixes delete preset button visible while search box focused
        m_undo_btn->Show();          // ADARTYS: fixes revert preset button visible while search box focused
        m_btn_search->Show();
        m_search_item->Hide();
    ...
    m_btn_search->Bind(wxEVT_BUTTON, [this](wxCommandEvent&) {
        ...
        m_btn_delete_preset->Hide(); // ADARTYS: fixes delete preset button visible while search box focused
        m_undo_btn->Hide();          // ADARTYS: fixes revert preset button visible while search box focused
        m_btn_search->Hide();
        m_search_item->Show();
    ...
    if (dynamic_cast<TabPrint*>(this) == nullptr && wxGetApp().is_admin_mode()) {
        ...
        // BBS: open this tab by select first
        m_static_title->Bind(wxEVT_LEFT_UP, [this](auto& e) { restore_last_select_item(); });
...
void Tab::on_value_change(const std::string& opt_key, const boost::any& value)
{
    ...
    if (opt_key == "print_sequence" && m_config->opt_enum<PrintSequence>("print_sequence") == PrintSequence::ByObject) {
        auto printer_structure_opt =
            wxGetApp().preset_bundle->printers.get_edited_preset().config.option<ConfigOptionEnum<PrinterStructure>>("printer_structure");
    ...
    // Adartys: do this only in simple mode
    ...
    if (opt_key == "sparse_infill_rotate_template") {
        // Adartys: show warning dialog if rotate template for solid infill if not support
    ...
    // Adartys: sync filament num if it's a multi tool printer
    ...
    // Adartys: disable purge_in_prime_tower if single_extruder_multi_material is disabled
...
void TabPrint::build()
{
    ...
    auto page     = add_options_page(L("Quality"), "custom-gcode_quality"); // ADARTYS: icon only visible on placeholders
    ...
    page     = add_options_page(L("Strength"), "custom-gcode_strength"); // ADARTYS: icon only visible on placeholders
    ...
    optgroup->append_single_option_line("internal_bridge_angle",
                                        "strength_settings_advanced#bridge-infill-direction"); // ADARTYS: Internal bridge angle override
    ...
    page     = add_options_page(L("Speed"), "custom-gcode_speed"); // ADARTYS: icon only visible on placeholders
    ...
    page     = add_options_page(L("Support"), "custom-gcode_support"); // ADARTYS: icon only visible on placeholders
    ...
    page     = add_options_page(L("Multimaterial"), "custom-gcode_multi_material"); // ADARTYS: icon only visible on placeholders
    ...
    page     = add_options_page(L("Others"), "custom-gcode_other"); // ADARTYS: icon only visible on placeholders
    ...
    // Adartys: hide the dependencies tab for process for now. The UI is not ready yet.
    ...
    for (auto p : m_pages) {
        for (auto g : p->m_optgroups) {
            ...
            for (auto& l : lines) {
                ...
                opts.erase(std::remove_if(opts.begin(), opts.end(), [this](auto& o) { return !has_key(o.opt.opt_key); }), opts.end());
                l.undo_to_sys = true;
            }
            lines.erase(std::remove_if(lines.begin(), lines.end(), [](auto& l) { return l.get_options().empty(); }), lines.end());
...
{
    // BBS
    PageShp page = add_options_page(L("Setting Overrides"), "custom-gcode_setting_override"); // ADARTYS: icon only visible on placeholders
    ...
    auto append_single_option_line = [optgroup, this](const std::string& opt_key, int opt_index) {
        ...
        line.near_label_widget = [this, optgroup_wk = ConfigOptionsGroupWkp(optgroup), opt_key, opt_index](wxWindow* parent) {
            auto check_box = new ::CheckBox(parent); // ADARTYS modernize checkboxes
            check_box->Bind(
                wxEVT_TOGGLEBUTTON,
...
void TabFilament::build()
{
    ...
    auto page = add_options_page(L("Filament"), "custom-gcode_filament"); // ADARTYS: icon only visible on placeholders
    // BBS
    auto optgroup = page->new_optgroup(L("Basic information"), L"param_information");
    optgroup->append_single_option_line("filament_type"); // ADARTYS use same width with other elements
    ...
    // Adartys: New section to focus on flow rate and PA to declutter general section
    ...
    // Adartys: adaptive pressure advance and calibration model
    ...
    page = add_options_page(L("Cooling"), "custom-gcode_cooling_fan"); // ADARTYS: icon only visible on placeholders
    ...
    optgroup->append_single_option_line("internal_bridge_fan_speed"); // ADARTYS: Add support for separate internal bridge fan speed control
    optgroup->append_single_option_line("support_material_interface_fan_speed");
    optgroup->append_single_option_line("ironing_fan_speed"); // ADARTYS: Add support for ironing fan speed control
    ...
    page                  = add_options_page(L("Advanced"), "custom-gcode_advanced"); // ADARTYS: icon only visible on placeholders
    ...
    create_line_with_widget(optgroup.get(), "filament_ramming_parameters", "", [this](wxWindow* parent) {
        // ADARTYS modernize button style
    ...
    page                  = add_options_page(L("Notes"), "custom-gcode_note"); // ADARTYS: icon only visible on placeholders
    optgroup              = page->new_optgroup(L("Notes"), "note", 0);
...
void TabFilament::toggle_options()
{
    ...
    if (m_active_page->title() == L("Cooling")) {
        ...
        for (auto el : {"overhang_fan_speed", "overhang_fan_threshold",
                        "internal_bridge_fan_speed"}) // ADARTYS: Add support for separate internal bridge fan speed control
            toggle_option(el, has_enable_overhang_bridge_fan);

        toggle_option("additional_cooling_fan_speed", cfg.opt_bool("auxiliary_fan"));

        // Adartys: toggle dont slow down for external perimeters if
    ...
    if (m_active_page->title() == L("Filament")) {
        ...
        // Adartys: Enable the plates that should be visible when multi bed support is enabled or a BBL printer is selected; otherwise,
        // enable only the plate visible for the selected bed type.
    ...
    if (m_active_page->title() == L("Multimaterial")) {
        // Adartys: hide specific settings for BBL printers
...
void TabPrinter::build_fff()
{
    ...
    auto page     = add_options_page(L("Basic information"), "custom-gcode_object-info"); // ADARTYS: icon only visible on placeholders
    ...
    const int gcode_field_height = 15;                                                   // 150
    const int notes_field_height = 25;                                                   // 250
    page                  = add_options_page(L("Machine G-code"), "custom-gcode_gcode"); // ADARTYS: icon only visible on placeholders
...
{
    auto page = add_options_page(L("Motion ability"), "custom-gcode_motion", true); // ADARTYS: icon only visible on placeholders
...
void TabPrinter::build_unregular_pages(bool from_initial_build /* = false*/)
{
    ...
    if (from_initial_build) {
        ...
        auto page = add_options_page(L("Multimaterial"), "custom-gcode_multi_material", true); // ADARTYS: icon only visible on placeholders
        ...
        // Adartys: rebuild missed extruder pages
...
// Adartys: we use a different logic here. If SEMM is enabled, we set extruder count to 1.
#if 1
    ...
    // Adartys: build missed extruder pages
    for (auto extruder_idx = m_extruders_count_old; extruder_idx < m_extruders_count; ++extruder_idx) {
        ...
        if (!page_exist) {
            //# build page
            // const wxString& page_name = wxString::Format(_L("Extruder %d"), int(extruder_idx + 1));
            auto page = add_options_page(page_name, "custom-gcode_extruder", true); // ADARTYS: icon only visible on placeholders
            ...
            optgroup->m_on_change = [this, extruder_idx](const t_config_option_key& opt_key, boost::any value) {
                ...
                if (is_SEMM && m_extruders_count > 1 && opt_key.find_first_of("nozzle_diameter") != std::string::npos) {
                    ...
                    std::vector<double> nozzle_diameters      = static_cast<const ConfigOptionFloats*>(m_config->option("nozzle_diameter"))
                                                               ->values;
...
#ifdef _MSW_DARK_MODE
                        if (!wxGetApp().tabs_as_menu()) {
                            std::string bmp_name = tab->type() == Slic3r::Preset::TYPE_FILAMENT ?
                                                       "spool" :
                        ...
                        bool               force_select)
{
    ...
    if (current_dirty && !may_discard_current_dirty_preset(nullptr, preset_name, no_transfer) && !force_select) {
        ...
        if (new_printer_technology == ptSLA && old_printer_technology == ptFFF &&
            ...
            for (PresetUpdate& pu : updates) {
                ...
                if (!canceled)
                    canceled = pu.old_preset_dirty && !may_discard_current_dirty_preset(pu.presets, preset_name) &&
    ...
    if (canceled) {
        ...
        auto update_compatible_type = [delete_current](bool technology_changed, bool on_page, bool show_incompatible_presets) {
            return (delete_current || technology_changed) ?
        ...
        // Adartys: update presets for the selected printer
...
void Tab::delete_preset()
{
    ...
    if (is_base_preset && (current_preset.type == Preset::Type::TYPE_FILAMENT) && action == _utf8(L("Delete"))) {
        msg += from_u8(_u8L("Are you sure to delete the selected preset? \nIf the preset corresponds to a filament currently in use on "
                                         ...
                                         bool            is_extruder_og /* false */)
{
    ...
    ConfigOptionsGroupShp optgroup = is_extruder_og ? std::make_shared<ExtruderOptionsGroup>(m_parent, title, icon, m_config,
                                                                                             true) // ADARTYS: add support for icons
                                                      :
...
ConfigManipulation Tab::get_config_manipulation()
{
    ...
    auto cb_toggle_line = [this](const t_config_option_key& opt_key, bool toggle) { return toggle_line(opt_key, toggle); };
