...
static const std::map<Preset::Type, std::string> type_icon_names = {
    {Preset::TYPE_PRINT, "cog"},       {Preset::TYPE_SLA_PRINT, "cog"},
    {Preset::TYPE_FILAMENT, "spool"},  {Preset::TYPE_SLA_MATERIAL, "blank_16"},
...
ModelNode::ModelNode(Preset::Type preset_type, wxWindow* parent_win, const wxString& text, const std::string& icon_name)
    : m_parent_win(parent_win), m_parent(nullptr), m_preset_type(preset_type), m_icon_name(icon_name), m_text(text)
{
    UpdateIcons();
}
...
ModelNode::ModelNode(ModelNode* parent, const wxString& text)
    : m_parent_win(parent->m_parent_win), m_parent(parent), m_icon_name("node_dot"), m_text(text)
{
    UpdateIcons();
}
...
ModelNode* DiffModel::AddOptionWithGroup(
...
ModelNode* DiffModel::AddOptionWithGroupAndCategory(ModelNode*        preset_node,
...
wxDataViewItem DiffModel::AddOption(Preset::Type      type,
                                    wxString          category_name,
                                    wxString          group_name,
                                    wxString          option_name,
                                    wxString          old_value,
                                    ...
                                    const std::string category_icon_name)
{
    ...
    for (std::unique_ptr<ModelNode>& preset : m_preset_nodes)
        if (preset->type() == type) {
            ...
            return wxDataViewItem((void*) AddOptionWithGroupAndCategory(preset.get(), category_name, group_name, option_name, old_value,
                                                                        new_value, category_icon_name));
        }
...
void DiffModel::GetValue(wxVariant& variant, const wxDataViewItem& item, unsigned int col) const
{
    ...
    ModelNode* node = static_cast<ModelNode*>(item.GetID());
    switch (col) {
    case colToggle: variant = node->m_toggle; break;
#ifdef __linux__
    case colIconText: variant << wxDataViewIconText(node->m_text, node->m_icon); break;
    case colOldValue: variant << wxDataViewIconText(node->m_old_value, node->m_old_color_bmp); break;
    case colNewValue: variant << wxDataViewIconText(node->m_new_value, node->m_new_color_bmp); break;
#else
    case colIconText: variant << DataViewBitmapText(node->m_text, node->m_icon); break;
    case colOldValue: variant << DataViewBitmapText(node->m_old_value, node->m_old_color_bmp); break;
    case colNewValue: variant << DataViewBitmapText(node->m_new_value, node->m_new_color_bmp); break;
#endif //__linux__

    default: wxLogError("DiffModel::GetValue: wrong column %d", col);
    }
}
...
bool DiffModel::SetValue(const wxVariant& variant, const wxDataViewItem& item, unsigned int col)
{
    ...
    ModelNode* node = static_cast<ModelNode*>(item.GetID());
    switch (col) {
    case colToggle: node->m_toggle = variant.GetBool(); return true;
#ifdef __linux__
    case colIconText: {
...
#endif //__linux__
    default: wxLogError("DiffModel::SetValue: wrong column");
    }
    return false;
}
...
{
    switch (col) {
    case colToggle: return "bool";
    case colIconText:
    case colOldValue:
    case colNewValue:
    default: return "DataViewBitmapText"; //"string";
    }
}
...
#else
    wxDataViewColumn* column = new wxDataViewColumn(label, new BitmapTextRenderer(true, wxDATAVIEW_CELL_INERT), model_column,
                                                    width * m_em_unit, wxALIGN_TOP, wxDATAVIEW_COL_RESIZABLE);
#endif //__linux__
...
void DiffViewCtrl::Append(const std::string& opt_key,
                          Preset::Type       type,
                          wxString           category_name,
                          wxString           group_name,
                          wxString           option_name,
...
void DiffViewCtrl::context_menu(wxDataViewEvent& event)
{
    ...
    FullCompareDialog(it->second.opt_name, it->second.old_val, it->second.new_val, old_value_header, new_value_header).ShowModal();
...
UnsavedChangesDialog::UnsavedChangesDialog(const wxString&    caption,
                                           const wxString&    header,
                                           const std::string& app_config_key,
                                           int                act_buttons)
...
void UnsavedChangesDialog::build(Preset::Type       type,
                                 PresetCollection*  dependent_presets,
                                 const std::string& new_selected_preset,
                                 const wxString&    header)
{
    ...
    auto        m_panel_temp     = new wxPanel(m_table_top, wxID_ANY, wxDefaultPosition, wxSize(UNSAVE_CHANGE_DIALOG_FIRST_VALUE_WIDTH, -1),
                                    wxTAB_TRAVERSAL);
    ...
    m_scrolledWindow = new wxScrolledWindow(m_panel_tab, wxID_ANY, wxDefaultPosition, UNSAVE_CHANGE_DIALOG_SCROLL_WINDOW_SIZE,
    ...
    // Add Buttons
    auto add_btn = [this, m_sizer_button, dependent_presets](Button** btn, int& btn_id, const std::string& icon_name, Action close_act,
                                                             const wxString& label, bool focus, bool process_enable = true) {
    ...
    if (ActionButtons::TRANSFER & m_buttons) {
        ...
        if (dependent_presets && switched_presets &&
            (type == dependent_presets->type() ? dependent_presets->get_edited_preset().printer_technology() ==
            ...
            add_btn(&m_transfer_btn, m_move_btn_id, "menu_paste", Action::Transfer,
                    switched_presets->get_edited_preset().name == new_selected_preset ? _L("Transfer") : _L("Transfer"), true);
    }
    ...
    { // "Don't save" / "Discard" button
        std::string btn_icon = (ActionButtons::DONT_SAVE & m_buttons) ?
                                   "" :
                                   (dependent_presets || (ActionButtons::KEEP & m_buttons)) ? "blank_16" : "exit";
    ...
    if (ActionButtons::SAVE & m_buttons)
        add_btn(&m_save_btn, m_save_btn_id, "save", Action::Save, _L("Save"), false);

    /* ScalableButton *cancel_btn = new ScalableButton(this, wxID_CANCEL, "cross", _L("Cancel"), wxDefaultSize, wxDefaultPosition,
      wxBORDER_DEFAULT, true, 24); buttons->Add(cancel_btn, 1, wxLEFT | wxRIGHT, 5); cancel_btn->SetFont(btn_font);*/
    ...
    if (action == Action::Undef && !m_has_long_strings)
        ...
        if (action == Action::Undef)
            ...
            if (preset_name.empty())
                ...
                text = format_wxstr(action == Action::Save ? _L("Save the selected options to preset \n\"%1%\".") :
                                                             _L("Transfer the selected options to the newly selected preset \n\"%1%\"."),
                                    preset_name);
...
static wxString get_string_value(std::string opt_key, const DynamicPrintConfig& config)
{
    ...
    switch (opt->type) {
    case coInt: return from_u8((boost::format("%1%") % config.opt_int(opt_key)).str());
    case coInts: {
        if (is_nullable) {
    ...
    case coBool: return config.opt_bool(opt_key) ? "true" : "false";
    case coBools: {
        if (is_nullable) {
    ...
    case coPercent: return from_u8((boost::format("%1%%%") % int(config.optptr(opt_key)->getFloat())).str());
    case coPercents: {
        if (is_nullable) {
    ...
    case coFloat: return double_to_string(config.opt_float(opt_key));
    case coFloats: {
        if (is_nullable) {
    ...
    case coString: return from_u8(config.opt_string(opt_key));
    case coStrings: {
                                  ...
                                  const wxString&    header)
{
    ...
    if (dependent_presets) {
        ...
        if (!m_transfer_btn) {
            ...
            action_msg += _L("\nYou can save or discard the preset values you have modified, or choose to transfer the values you have "
                             "modified to the new preset.");
        }
    } else {
...
void UnsavedChangesDialog::update_list()
{
    ...
    for (auto i = 0; i < m_presetitems.size(); i++) {
        if (class_c_list.count(m_presetitems[i].category_name) <= 0) {
            ...
            it = find(class_c_list[m_presetitems[i].category_name].begin(), class_c_list[m_presetitems[i].category_name].end(),
                      m_presetitems[i].group_name);
    ...
    for (auto iter = class_c_list.begin(); iter != class_c_list.end(); iter++) {
        // category
        auto panel_category = new wxPanel(m_scrolledWindow, wxID_ANY, wxDefaultPosition, wxSize(-1, UNSAVE_CHANGE_DIALOG_ITEM_HEIGHT),
                                          wxTAB_TRAVERSAL);
        ...
        for (auto i = 0; i < iter->second.size(); i++) {
            ...
            for (auto g = 0; g < class_g_list[gname].size(); g++) {
                ...
                if (g == 0) {
                    ...
                    auto panel_left = new wxPanel(panel_item, wxID_ANY, wxDefaultPosition,
                                                  wxSize(UNSAVE_CHANGE_DIALOG_FIRST_VALUE_WIDTH, -1), wxTAB_TRAVERSAL);
                ...
                auto panel_left = new wxPanel(panel_item, wxID_ANY, wxDefaultPosition, wxSize(UNSAVE_CHANGE_DIALOG_FIRST_VALUE_WIDTH, -1),
                                              wxTAB_TRAVERSAL);
                panel_left->SetBackgroundColour(GREY200);
                ...
                auto        panel_oldv  = new wxPanel(panel_item, wxID_ANY, wxDefaultPosition, wxSize(UNSAVE_CHANGE_DIALOG_VALUE_WIDTH, -1),
                                              wxTAB_TRAVERSAL);
                wxBoxSizer* sizer_old_v = new wxBoxSizer(wxVERTICAL);

                data.old_value = subreplace(data.old_value.ToStdString(), "\n", " ");
                auto text_oldv = new wxStaticText(panel_oldv, wxID_ANY, data.old_value, wxDefaultPosition, wxDefaultSize,
                                                  wxST_ELLIPSIZE_END);
                ...
                auto        panel_newv  = new wxPanel(panel_item, wxID_ANY, wxDefaultPosition, wxSize(UNSAVE_CHANGE_DIALOG_VALUE_WIDTH, -1),
                                              wxTAB_TRAVERSAL);
                wxBoxSizer* sizer_new_v = new wxBoxSizer(wxVERTICAL);

                data.new_value = subreplace(data.new_value.ToStdString(), "\n", " ");
                auto text_newv = new wxStaticText(panel_newv, wxID_ANY, data.new_value, wxDefaultPosition, wxDefaultSize,
                                                  wxST_ELLIPSIZE_END);
...
void UnsavedChangesDialog::update_tree(Preset::Type type, PresetCollection* presets_)
{
    ...
    for (PresetCollection* presets : presets_list) {
        ...
        if (type == Preset::TYPE_PRINTER && old_pt == ptFFF &&
            old_config.opt<ConfigOptionFloats>("nozzle_diameter")->values.size() !=
                new_config.opt<ConfigOptionFloats>("nozzle_diameter")->values.size()) {
            ...
            // m_tree->Append("extruders_count", type, _L("General"), _L("Capabilities"), local_label, old_val, new_val,
            // category_icon_map.at("Basic information")); m_tree->Append("extruders_count", type, _L("General"), _L("Capabilities"),
            // local_label, old_val, new_val, category_icon_map.at("General"));
...
bool UnsavedChangesDialog::check_option_valid() { return true; }

//------------------------------------------
//          FullCompareDialog
//------------------------------------------

FullCompareDialog::FullCompareDialog(const wxString& option_name,
    ...
    : wxDialog(nullptr, wxID_ANY, option_name, wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    ...
    std::set_difference(old_set.begin(), old_set.end(), new_set.begin(), new_set.end(),
                        std::inserter(old_new_diff_set, old_new_diff_set.begin()));
    ...
    auto add_value = [grid_sizer, border, this](wxString label, const std::set<wxString>& diff_set, bool is_colored = false) {
        ...
        for (const wxString& str : diff_set) {
            ...
            text->SetStyle(pos, pos + (int) str.Len(),
                           wxTextAttr(is_colored ? wxColour(orange) : wxNullColour, wxNullColour, this->GetFont().Bold()));
        }
...
static PresetCollection* get_preset_collection(Preset::Type type, PresetBundle* preset_bundle = nullptr)
{
    ...
    return type == Preset::Type::TYPE_PRINTER ?
               &preset_bundle->printers :
               type == Preset::Type::TYPE_FILAMENT ?
               &preset_bundle->filaments :
               type == Preset::Type::TYPE_SLA_MATERIAL ?
               &preset_bundle->sla_materials :
               type == Preset::Type::TYPE_PRINT ? &preset_bundle->prints :
                                                  type == Preset::Type::TYPE_SLA_PRINT ? &preset_bundle->sla_prints : nullptr;
}

//------------------------------------------
//          DiffPresetDialog
//------------------------------------------
static std::string get_selection(PresetComboBox* preset_combo) { return into_u8(preset_combo->GetString(preset_combo->GetSelection())); }
...
std::array<Preset::Type, 3> DiffPresetDialog::types_list() const { return PresetBundle::types_list(m_pr_technology); }
...
void DiffPresetDialog::update_controls_visibility(Preset::Type type /* = Preset::TYPE_INVALID*/)
{
    for (auto preset_combos : m_preset_combos) {
        ...
        bool         show    = type != Preset::TYPE_INVALID ?
                        type == cb_type :
                        cb_type == Preset::TYPE_PRINTER ?
                        true :
    ...
    for (auto preset_combos : m_preset_combos) {
        ...
        for (const std::string& opt_key : dirty_options) {
            ...
            if (option.opt_key() != opt_key) {
                ...
                m_tree->Append(opt_key, type, "Undef category", "Undef group", opt_key, left_val, right_val,
                               "undefined"); // ADARTYS: use low resolution compatible icon
                // When founded option isn't the correct one.
                // It can be for dirty_options: "default_print_profile", "printer_model", "printer_settings_id",
                // because of they don't exist in searcher
                continue;
            }
            m_tree->Append(opt_key, type, option.category_local, option.group_local, option.label_local, left_val, right_val,
...
void DiffPresetDialog::update_compatibility(const std::string& preset_name, Preset::Type type, PresetBundle* preset_bundle)
{
    ...
    if (technology_changed && m_preset_bundle_left.get()->printers.get_selected_preset().printer_technology() ==
