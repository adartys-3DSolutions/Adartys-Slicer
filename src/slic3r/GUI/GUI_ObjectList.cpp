...
public:
    ...
    { // ADARTYS draw selection background to improve consistency between platforms
    ...
    { // ADARTYS draw custom triangle to improve consistency between platforms
    ...
    { // ADARTYS draw custom text to improve consistency between platforms
...
ObjectList::ObjectList(wxWindow* parent)
    : wxDataViewCtrl(parent,
                     ...
                     wxDefaultPosition,
                     wxDefaultSize,
                     wxDV_MULTIPLE | wxNO_BORDER | wxDV_NO_HEADER) // ADARTYS: Remove border and header
{
    ...
    //    Bind(wxEVT_KEY_DOWN, &ObjectList::OnChar, this);
    {
        ...
        this->Bind(
            wxEVT_MENU, [this](wxCommandEvent& evt) { this->copy(); }, wxID_COPY);
        this->Bind(
            wxEVT_MENU, [this](wxCommandEvent& evt) { this->paste(); }, wxID_PASTE);
        this->Bind(
            wxEVT_MENU, [this](wxCommandEvent& evt) { this->select_item_all_children(); }, wxID_SELECTALL);
        this->Bind(
            wxEVT_MENU, [this](wxCommandEvent& evt) { this->remove(); }, wxID_DELETE);
        this->Bind(
            wxEVT_MENU, [this](wxCommandEvent& evt) { this->undo(); }, wxID_UNDO);
        this->Bind(
            wxEVT_MENU, [this](wxCommandEvent& evt) { this->redo(); }, wxID_REDO);
        this->Bind(
            wxEVT_MENU, [this](wxCommandEvent& evt) { this->cut(); }, wxID_CUT);
        this->Bind(
            wxEVT_MENU, [this](wxCommandEvent& evt) { this->clone(); }, wxID_DUPLICATE);
        ...
        if (item) {
            ...
            if (type & itVolume) {
                ...
                menu = volume->is_text() ? plater->text_part_menu() :
                                           volume->is_svg() ? plater->svg_part_menu() : // ADARTYS fixes missing "Edit SVG" item for
                                                                                        // Add/Negative/Modifier SVG objects in object list
                                               plater->part_menu();
            } else
                menu = type & itPlate ?
                           plater->plate_menu() :
                                                         ...
                                                         wxDataViewItem                                   item)
{
    ...
    const std::string snapshot_text = item_type & itLayer ? "Layer setting added" :
                                                            item_type & itVolume ? "Part setting added" : "Object setting added";
...
void ObjectList::add_category_to_settings_from_frequent(const std::vector<std::string>& options, wxDataViewItem item)
{
    ...
    const std::string snapshot_text = item_type & itLayer ? "Height range settings added" :
                                                            item_type & itVolume ? "Part settings added" : "Object settings added";
    take_snapshot(snapshot_text);
...
bool ObjectList::del_from_cut_object(bool is_cut_connector, bool is_model_part /* = false*/, bool is_negative_volume /* = false*/)
{
    ...
    const wxString title = is_cut_connector ?
                               _L("Delete connector from object which is a part of cut") :
...
void ObjectList::change_part_type()
{
    ...
    // ADARTYS: Fix crash when changing type of svg / text modifier
    wxArrayString names;
    names.Add(_L("Part"));
