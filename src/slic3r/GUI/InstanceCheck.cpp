...
static BOOL CALLBACK EnumWindowsProc(_In_ HWND hwnd, _In_ LPARAM lParam)
{
    ...
    if (wndTextString.find(L"AdartysSlicer") != std::wstring::npos && classNameString == L"wxWindowNR") {
...
static bool send_message(const std::string& message_text, const std::string& version)
{
    ...
    if ( !checker->IsAnotherRunning() ) */
    {
        ...
        std::string object_name = "/com/softfever3d/AdartysSlicer/InstanceCheck/Object" + version;
...
void OtherInstanceMessageHandler::handle_message(const std::string& message)
{
    ...
    boost::regex  re(R"(^(adartysslicer|orcaslicer|prusaslicer|cura|bambustudio):\/\/open[\/]?\?file=)", boost::regbase::icase);
...
// method AnotherInstance receives message from another AdartysSlicer instance
...
void OtherInstanceMessageHandler::listen()
{
    ...
    std::string          interface_name = "com.adartys.adartys-slicer.InstanceCheck.Object" + instance_hash;
    std::string          object_name    = "/com/adartys/AdartysSlicer/InstanceCheck/Object" + instance_hash;
    ...
    if (DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER != name_req_val) {
        BOOST_LOG_TRIVIAL(error) << "Not primary owner of DBus name - probably another AdartysSlicer instance is running.";
        BOOST_LOG_TRIVIAL(error) << "Dbus Messages listening terminating.";
        dbus_connection_unref(conn);
        return;
    }
