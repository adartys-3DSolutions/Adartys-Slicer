...
BundleMap BundleMap::load()
{
    ...
    auto adartys_bundle_path = (vendor_dir / PresetBundle::ADARTYS_DEFAULT_BUNDLE).replace_extension(".json");
    auto adartys_bundle_rsrc = false;
    if (!boost::filesystem::exists(adartys_bundle_path)) {
        adartys_bundle_path = (rsrc_vendor_dir / PresetBundle::ADARTYS_DEFAULT_BUNDLE).replace_extension(".json");
        adartys_bundle_rsrc = true;
    }
    {
        Bundle bbl_bundle;
        if (bbl_bundle.load(std::move(adartys_bundle_path), adartys_bundle_rsrc, true))
            res.emplace(PresetBundle::ADARTYS_DEFAULT_BUNDLE, std::move(bbl_bundle));
    }
...
{
    // Adartys: add custom as default
    auto it = find(PresetBundle::ADARTYS_DEFAULT_BUNDLE);
    if (it == end()) {
        throw Slic3r::RuntimeError("ConfigWizard: Internal error in BundleMap: ADARTYS_DEFAULT_BUNDLE not loaded");
    }
...
void PagePrinters::set_run_reason(ConfigWizard::RunReason run_reason)
{
    ...
    if (is_primary_printer_page && (run_reason == ConfigWizard::RR_DATA_EMPTY || run_reason == ConfigWizard::RR_DATA_LEGACY) &&
        printer_pickers.size() > 0 && printer_pickers[0]->vendor_id == PresetBundle::ADARTYS_DEFAULT_BUNDLE) {
...
ConfigWizardIndex::ConfigWizardIndex(wxWindow* parent)
    : wxPanel(parent)
    , bg(ScalableBitmap(parent, "AdartysSlicer_192px_transparent.png", 192))
...
void ConfigWizard::priv::create_3rdparty_pages()
{
    for (const auto& pair : bundles) {
        ...
        // Adartys: add custom as default
        if (vendor->id == PresetBundle::ADARTYS_DEFAULT_BUNDLE) {
            continue;
        }
