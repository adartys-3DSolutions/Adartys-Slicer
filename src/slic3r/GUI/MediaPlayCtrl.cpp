...
void MediaPlayCtrl::Play()
{
    ...
    if (agent) {
        ...
        agent->get_camera_url(m_machine + "|" + m_dev_ver + "|" + protocols[m_remote_proto], [this, m = m_machine, v = agent_version,
...
void MediaPlayCtrl::ToggleStream()
{
    ...
    if (!url.empty() && wxGetApp().app_config->get("not_show_vcamera_stop_prev") != "1") {
        MessageDialog dlg(this->GetParent(),
                          _L("Another virtual camera is running.\nAdartys Slicer supports only a single virtual camera.\nDo you want to "
                             "stop this virtual camera?"),
