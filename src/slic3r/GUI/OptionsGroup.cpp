...
bool OptionsGroup::activate(std::function<void()> throw_if_canceled /* = [](){}*/, int horiz_alignment /* = wxALIGN_LEFT*/)
{
    ...
    try {
        if (staticbox) {
            // ADARTYS match style of wxStaticBox between platforms
            ...
            stl->SetForegroundColour("#363636"); // ADARTYS Match Parameters title color with tab title color
...
boost::any ConfigOptionsGroup::get_config_value(const DynamicPrintConfig& config, const std::string& opt_key, int opt_index /*= -1*/)
{
    ...
    case coFloat: {
        double val = opt->type == coFloats ?
                         ...
                         opt->type == coFloat ? config.opt_float(opt_key) : config.option<ConfigOptionPercents>(opt_key)->get_at(idx);
        ret = double_to_string(val);
    } break;
...
wxString OptionsGroup::get_url(const std::string& path_end)
{
    ...
    // Adartys: point to sf wiki for seam parameters
