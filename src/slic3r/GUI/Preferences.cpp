...
wxBoxSizer* PreferencesDialog::create_item_autoflush(wxString title, wxString tooltip)
{
    ...
    std::vector<wxString>::iterator iter;
    for (iter = FlushOptions.begin(); iter != FlushOptions.end(); iter++) {
        combobox->Append(*iter);
    }
...
wxBoxSizer* PreferencesDialog::create_item_checkbox(wxString title, wxString tooltip, std::string param, const wxString secondary_title)
{
    ...
    checkbox->Bind(wxEVT_TOGGLEBUTTON, [this, checkbox, param](wxCommandEvent& e) {
        ...
        if (param == "installed_networking") {
            ...
            if (m_legacy_networking_ckeckbox != nullptr) {
                m_legacy_networking_ckeckbox->Enable(pbool);
            }
        }
...
wxBoxSizer* PreferencesDialog::create_item_link_association(wxString url_prefix, wxString website_name)
{
    ...
    wxString tooltip = _L("Associate") + " " + url_prefix + ":// " + _L("with AdartysSlicer so that Adartys can open models from") + " " +
                       website_name;
...
void PreferencesDialog::create_items()
{
...
#else
        _L("If this is enabled, when starting AdartysSlicer and another instance of the same AdartysSlicer is already running, that "
    ...
    auto item_remember_printer = create_item_checkbox(
        _L("Remember printer configuration"),
        _L("If enabled, Adartys will remember and switch filament/process configuration for each printer automatically."),
    ...
    auto item_auto_flush = create_item_autoflush(_L("Auto flush after changing ..."),
                                                 _L("Auto calculate flushing volumes when selected values changed"));
    ...
    auto item_legacy_network = create_item_checkbox(_L("Use legacy network plugin"),
                                                    _L("Disable to use latest network plugin that supports new BambuLab firmwares."),
                                                    "legacy_networking", _L("(Requires restart)"));
    ...
    g_sizer->Add(create_item_title(_L("Associate files to AdartysSlicer")), 1, wxEXPAND);

    auto item_associate_3mf = create_item_checkbox(_L("Associate .3mf files to AdartysSlicer"),
                                                   _L("If enabled, sets AdartysSlicer as default application to open .3mf files"),
                                                   "associate_3mf");
    g_sizer->Add(item_associate_3mf);

    auto item_associate_stl = create_item_checkbox(_L("Associate .stl files to AdartysSlicer"),
                                                   _L("If enabled, sets AdartysSlicer as default application to open .stl files"),
                                                   "associate_stl");
    g_sizer->Add(item_associate_stl);

    auto item_associate_step = create_item_checkbox(_L("Associate .step/.stp files to AdartysSlicer"),
                                                    _L("If enabled, sets AdartysSlicer as default application to open .step files"),
                                                    "associate_step");
    g_sizer->Add(item_associate_step);

    //// ASSOCIATE > WebLinks
    g_sizer->Add(create_item_title(_L("Associate web links to AdartysSlicer")), 1, wxEXPAND);
