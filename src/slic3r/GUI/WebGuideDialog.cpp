...
GuideFrame::GuideFrame(GUI_App* pGUI, long style)
    : DPIDialog((wxWindow*) (pGUI->mainframe), wxID_ANY, "AdartysSlicer", wxDefaultPosition, wxDefaultSize, style), m_appconfig_new()
{
...
bool GuideFrame::apply_config(AppConfig* app_config, PresetBundle* preset_bundle, const PresetUpdater* updater, bool& apply_keeped_changes)
{
    ...
    // Adartys "custom" printers are considered first, then 3rd party.
    if (preferred_model = get_preferred_printer_model(PresetBundle::ADARTYS_DEFAULT_BUNDLE, preferred_variant); preferred_model.empty()) {
        for (const auto& bundle : enabled_vendors) {
            if (bundle.first == PresetBundle::ADARTYS_DEFAULT_BUNDLE) {
                continue;
            }
...
bool GuideFrame::run()
{
    ...
    if (result == wxID_OK) {
        ...
        if (app.preset_bundle->printers.only_default_printers()) {
            ...
            app.app_config->set_variant(PresetBundle::ADARTYS_DEFAULT_BUNDLE, PresetBundle::ADARTYS_DEFAULT_PRINTER_MODEL,
                                        PresetBundle::ADARTYS_DEFAULT_PRINTER_VARIANT, "true");
            app.app_config->clear_section(AppConfig::SECTION_FILAMENTS);
            app.preset_bundle->load_selections(*app.app_config,
                                               {PresetBundle::ADARTYS_DEFAULT_PRINTER_MODEL, PresetBundle::ADARTYS_DEFAULT_PRINTER_VARIANT,
                                                PresetBundle::ADARTYS_DEFAULT_FILAMENT, std::string()});
        ...
        if (sVendor == "" || sType == "") {
            if (jLocal.contains("inherits")) {
                ...
                if (!boost::filesystem::exists(inherits_path))
                    inherits_path = (boost::filesystem::path(m_AdartysFilaLibPath) / boost::filesystem::path(FPath)).make_preferred();
...
int GuideFrame::LoadProfileData()
{
    try {
        ...
        // Adartys: add custom as default
        // Adartys: add json logic for vendor bundle
        adartys_bundle_rsrc = true;

        // search if there exists a .json file in vendor_dir folder, if exists, set adartys_bundle_rsrc to false
        for (const auto& entry : boost::filesystem::directory_iterator(vendor_dir)) {
            if (!boost::filesystem::is_directory(entry) && boost::iequals(entry.path().extension().string(), ".json") &&
                !boost::iequals(entry.path().stem().string(), PresetBundle::ADARTYS_FILAMENT_LIBRARY)) {
                adartys_bundle_rsrc = false;
                break;
            }
        }
        ...
        auto filament_library_name = boost::filesystem::path(PresetBundle::ADARTYS_FILAMENT_LIBRARY).replace_extension(".json");
        if (boost::filesystem::exists(vendor_dir / filament_library_name)) {
            m_AdartysFilaLibPath = (vendor_dir / PresetBundle::ADARTYS_FILAMENT_LIBRARY).string();
            LoadProfileFamily(PresetBundle::ADARTYS_FILAMENT_LIBRARY, (vendor_dir / filament_library_name).string());
        } else {
            m_AdartysFilaLibPath = (rsrc_vendor_dir / PresetBundle::ADARTYS_FILAMENT_LIBRARY).string();
            LoadProfileFamily(PresetBundle::ADARTYS_FILAMENT_LIBRARY, (rsrc_vendor_dir / filament_library_name).string());
        }
        loaded_vendors.insert(PresetBundle::ADARTYS_FILAMENT_LIBRARY);
...
int GuideFrame::LoadProfileFamily(std::string strVendor, std::string strFilePath)
{
    ...
    try {
        ...
        json tFilaList = m_AdartysFilaList;
        ...
        if (strVendor == PresetBundle::ADARTYS_FILAMENT_LIBRARY)
            m_AdartysFilaList = tFilaList;
