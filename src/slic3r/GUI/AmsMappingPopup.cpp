...
void MaterialItem::doRender(wxDC& dc)
{
    ...
    if (m_selected) {
        dc.SetPen(AMS_CONTROL_BRAND_COLOUR); // ADARTYS Highlight color for selected AMS in send job dialog
        dc.SetBrush(*wxTRANSPARENT_BRUSH);
        dc.DrawRoundedRectangle(1, 1, MATERIAL_ITEM_SIZE.x - 1, MATERIAL_ITEM_SIZE.y - 1, 5);
    }
#else
    ...
    if ((acolor.Red() > 160 && acolor.Green() > 160 && acolor.Blue() > 160) &&
        ...
        dc.DrawBitmap(m_arraw_bitmap_white.bmp(), size.x - m_arraw_bitmap_white.GetBmpSize().x - FromDIP(7),
                      size.y - m_arraw_bitmap_white.GetBmpSize().y);
    } else {
        dc.DrawBitmap(m_arraw_bitmap_gray.bmp(), size.x - m_arraw_bitmap_gray.GetBmpSize().x - FromDIP(7),
                      size.y - m_arraw_bitmap_gray.GetBmpSize().y);
    }
}

AmsMapingPopup::AmsMapingPopup(wxWindow* parent) : PopupWindow(parent, wxBORDER_NONE)
{
...
bool AmsMapingPopup::is_match_material(std::string material) { return m_tag_material == material ? true : false; }
...
void AmsMapingPopup::on_left_down(wxMouseEvent& evt)
{
    ...
    for (MappingItem* item : m_mapping_item_list) {
        ...
        if (pos.x > p_rect.x && pos.y > p_rect.y && pos.x < (p_rect.x + item->GetSize().x) && pos.y < (p_rect.y + item->GetSize().y)) {
            if (item->m_tray_data.type == TrayType::NORMAL && !is_match_material(item->m_tray_data.filament_type))
                return;
...
void AmsMapingPopup::update_ams_data_multi_machines()
{
    ...
    add_ams_mapping(tray_datas, m_amsmapping_container_list[m_amsmapping_container_list_index],
                    m_amsmapping_container_sizer_list[m_amsmapping_container_list_index]);
...
void AmsMapingPopup::add_ams_mapping(std::vector<TrayData> tray_data, wxWindow* container, wxBoxSizer* sizer)
{
    ...
    for (auto i = 0; i < tray_data.size(); i++) {
        // set number
        /* auto number = new wxStaticText(this, wxID_ANY, wxGetApp().transition_tridid(tray_data[i].id), wxDefaultPosition, wxDefaultSize,
         0); number->SetFont(::Label::Body_13); number->SetForegroundColour(wxColour(0X6B, 0X6B, 0X6B)); number->Wrap(-1);*/
...
void MappingItem::send_event(int fliament_id)
{
    ...
    wxString param = wxString::Format("%d|%d|%d|%d|%s|%d|%d|%d", m_coloul.Red(), m_coloul.Green(), m_coloul.Blue(), m_coloul.Alpha(),
                                      number, fliament_id, m_tray_data.ams_id, m_tray_data.slot_id);
    ...
    if (m_tray_data.material_cols.size() > 1) {
        ...
        if (m_tray_data.ctype == 0) {
            ...
            for (int i = 0; i < cols_size; i++) {
                ...
                dc.DrawRectangle(x, (size.y - MAPPING_ITEM_REAL_SIZE.y) / 2, (float) MAPPING_ITEM_REAL_SIZE.x / cols_size,
...
bool AmsHumidityTipPopup::ProcessLeftDown(wxMouseEvent& event) { return PopupWindow::ProcessLeftDown(event); }

void AmsHumidityTipPopup::set_humidity_level(int level)
{
    current_humidity_level = level;
    if (current_humidity_level <= 0) {
        return;
    }
...
MappingContainer::MappingContainer(wxWindow* parent) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize)
{
#ifdef __WINDOWS__
    SetDoubleBuffered(true);
#endif //__WINDOWS__
...
void MappingContainer::doRender(wxDC& dc) { dc.DrawBitmap(ams_mapping_item_container, 0, 0); }
...
void AmsReplaceMaterialDialog::update_machine_obj(MachineObject* obj)
{
    ...
    for (int filam : extder.filam_bak) {
        ...
        if (is_in_tray || m_tray_used.size() <= 0) {
            m_groups_sizer->Add(create_backup_group(wxString::Format("%s%d", _L("Group"), group_index + 1), group_info, group_material,
                                                    status_list),
                                0, wxALL, FromDIP(10));
            group_index++;
        }
    ...
    if (group_index > 0) {
        ...
        if (!obj->is_support_filament_backup) {
            ...
            label_txt->SetLabelText(
                _L("If there are two identical filaments in AMS, AMS filament backup will be enabled.\n"
...
AmsRMGroup* AmsReplaceMaterialDialog::create_backup_group(wxString                        gname,
                                                          std::map<std::string, wxColour> group_info,
                                                          wxString                        material,
...
void AmsRMGroup::doRender(wxDC& dc)
{
    ...
    for (auto iter = m_group_info.rbegin(); iter != m_group_info.rend(); ++iter) {
        ...
        if (tray_color == *wxWHITE)
            dc.SetPen(wxPen(wxColour(0xEEEEEE), 2));
    ...
    for (auto iter = m_group_info.rbegin(); iter != m_group_info.rend(); ++iter) {
        ...
        if (tray_color.Alpha() == 0) {
            dc.SetTextForeground(wxColour(0x262E30));
        }
        ...
        if (tray_color.Alpha() == 0) {
            dc.SetPen(wxPen(wxColour(0xCECECE), 2));
        }
        dc.SetBrush(*wxTRANSPARENT_BRUSH);
