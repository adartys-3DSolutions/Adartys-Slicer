    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    rename_editable       = new ScalableBitmap(this, "rename_edit", FromDIP(13)); // ADARTYS Match edit icon and its size
    rename_editable_light = new ScalableBitmap(this, "rename_edit", FromDIP(13)); // ADARTYS Match edit icon and its size
    ...
    rename_sizer_h->Add(m_rename_text, 0, wxALIGN_CENTER, 0);                     // ADARTYS align text with icon
    rename_sizer_h->Add(m_rename_button, 0, wxALIGN_CENTER | wxLEFT, FromDIP(3)); // ADARTYS add gap between text and icon
    ...
    m_bitmap_last_plate   = new wxStaticBitmap(m_basic_panel, wxID_ANY, create_scaled_bitmap("go_last_plate", this, 25), wxDefaultPosition,
    ...
    m_bitmap_next_plate   = new wxStaticBitmap(m_basic_panel, wxID_ANY, create_scaled_bitmap("go_next_plate", this, 25), wxDefaultPosition,
                                             wxSize(FromDIP(25), FromDIP(25)), 0);
    ...
    m_btn_bg_enable = StateColor(std::pair<wxColour, int>(wxColour(ADARTYS_PRIMARY_RGB), StateColor::Pressed),
    ...
    m_ams_backup_tip->SetForegroundColour(wxColour(ADARTYS_PRIMARY_HEX));
    m_ams_backup_tip->SetBackgroundColour(*wxWHITE);
    img_ams_backup = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("automatic_material_renewal", this, 16), wxDefaultPosition,
    ...
    m_statictext_finish->SetForegroundColour(wxColour(ADARTYS_PRIMARY_RGB));
...
wxWindow* SelectMachineDialog::create_ams_checkbox(wxString title, wxWindow* parent, wxString tooltip)
{
    ...
    img_use_ams_tip->Bind(wxEVT_LEAVE_WINDOW, [this](wxMouseEvent& e) { m_mapping_tip_popup.Dismiss(); });
    ...
    text->Bind(wxEVT_LEFT_DOWN, [this, check](wxMouseEvent& event) { check->SetValue(check->GetValue() ? false : true); });

    checkbox->Bind(wxEVT_LEFT_DOWN, [this, check](wxMouseEvent& event) { check->SetValue(check->GetValue() ? false : true); });
                ...
                if (m_ams_mapping_result[k].id == i) {
                    ...
                    try {
                        if (m_ams_mapping_result[k].ams_id.empty() || m_ams_mapping_result[k].slot_id.empty()) { // invalid case
                            mapping_item_v1["ams_id"]  = 255;                                                    // TODO: Adartys hack
...
bool SelectMachineDialog::build_nozzles_info(std::string& nozzles_info)
{
    ...
    for (size_t i = 0; i < opt_nozzle_diameters->size(); i++) {
        ...
        nozzle_item["flowSize"] = "standard_flow"; // TODO: Adartys hack
        //}
...
bool SelectMachineDialog::has_tips(MachineObject* obj)
{
    ...
    if (select_timelapse->IsShown() && m_checkbox_list["timelapse"]->GetValue()) {
    ...
    if (status == PrintDialogStatus::PrintStatusInit) {
        ...
        if (params.size() > 0)
            msg_text = wxString::Format(
                _L("Filament %s exceeds the number of AMS slots. Please update the printer firmware to support AMS slot assignment."),
                params[0]);
        else
...
void SelectMachineDialog::on_ok_btn(wxCommandEvent& event)
{
    ...
    if (!obj_->m_extder_data.extders[0].current_nozzle_type != ntUndefine && (m_print_type == PrintFromType::FROM_NORMAL)) {
        ...
        if (!is_same_nozzle_diameters(nozzle_diameter)) {
            ...
            confirm_text.push_back(
                ConfirmBeforeSendInfo(_L("Your nozzle diameter in sliced file is not consistent with memorized nozzle. If you changed your "
        ...
        if (!is_same_nozzle_type(obj_->m_extder_data.extders[0], filament_type)) {
            ...
            wxString nozzle_in_preset =
                wxString::Format(_L("Printing high temperature material (%s material) with %s may cause nozzle damage"), filament_type,
    ...
    if (has_slice_warnings) {
        ...
        if (is_printing_block) {
            ...
            confirm_text.push_back(ConfirmBeforeSendInfo(_L("Please fix the error above, otherwise printing cannot continue."),
                                                         ConfirmBeforeSendInfo::InfoLevel::Warning));
        } else {
            confirm_text.push_back(
...
void SelectMachineDialog::on_send_print()
{
    ...
    m_print_job->set_print_config(MachineBedTypeString[0], m_checkbox_list["bed_leveling"]->GetValue(),
                                  m_checkbox_list["flow_cali"]->GetValue(), false, timelapse_option, true,
                                  0, // TODO: Adartys hack
                                  0, 0);
...
void SelectMachineDialog::on_timer(wxTimerEvent& event)
{
    ...
    if (!obj_ || obj_->amsList.empty() || obj_->ams_exist_bits == 0 || !obj_->is_support_filament_backup ||
        !obj_->is_support_show_filament_backup || !obj_->ams_auto_switch_filament_flag || !m_checkbox_list["use_ams"]->GetValue()) {
...
void SelectMachineDialog::set_default_normal(const ThumbnailData& data)
{
    ...
    if (wxGetApp().app_config->get("use_inches") == "1") {
        ::sprintf(weight, "%.2f oz", aprint_stats.total_weight * 0.035274); // ADARTYS remove spacing before text
    } else {
        ::sprintf(weight, "%.2f g", aprint_stats.total_weight); // ADARTYS remove spacing before text
    }
...
void SelectMachineDialog::set_default_from_sdcard()
{
    ...
    try {
        ...
        char weight[64];
        ::sprintf(weight, "%.2f g", float_weight); // ADARTYS remove spacing before text
        m_stext_time->SetLabel(time);
        m_stext_weight->SetLabel(weight);
    } catch (...) {}
}
