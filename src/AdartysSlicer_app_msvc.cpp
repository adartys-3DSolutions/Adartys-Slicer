...
public:
    ...
    bool load_opengl_dll()
    {
        ...
        wc.lpszClassName = L"AdartysSlicer_opengl_version_check";
        wc.style         = CS_OWNDC;
        if (RegisterClass(&wc)) {
            HWND hwnd = CreateWindowW(wc.lpszClassName, L"AdartysSlicer_opengl_version_check", WS_OVERLAPPEDWINDOW, 0, 0, 640, 480, 0, 0,
...
Slic3rMainFunc adartysslicer_main = nullptr;
}
...
int wmain(int argc, wchar_t** argv)
{
    ...
    wcscat(path_to_slic3r, L"AdartysSlicer.dll");
    //	printf("Loading Slic3r library: %S\n", path_to_slic3r);
    HINSTANCE hInstance_Slic3r = LoadLibraryExW(path_to_slic3r, nullptr, 0);
    if (hInstance_Slic3r == nullptr) {
        printf("AdartysSlicer.dll was not loaded, error=%d\n", GetLastError());
        return -1;
    }

    // resolve function address here
    adartysslicer_main = (Slic3rMainFunc)
    ...
    if (adartysslicer_main == nullptr) {
        printf("could not locate the function adartysslicer_main in AdartysSlicer.dll\n");
        return -1;
    }
    // argc minus the trailing nullptr of the argv
    return adartysslicer_main((int) argv_extended.size() - 1, argv_extended.data());
}
}
