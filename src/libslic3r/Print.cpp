...
// BBS
// ADARTYS: Now this is a parameter
    ...
    for (const PrintObject* object : m_objects) {
        ...
        for (auto layer_range : mo->layer_config_ranges) {
            if (layer_range.second.has("extruder")) {
                ...
                // "extruder". Don't know why height range always save key "extruder" because of no change(should only save difference)... Add
                // protection here to avoid overflow
...
{
    // Adartys: unclear why (m_config.ooze_prevention && this->extruders().size() > 1) logic is here, removed.
    ...
    std::vector<struct print_instance_info> print_instance_with_bounding_box;
    {
        ...
        for (const PrintObject* print_object : print.objects()) {
            ...
            for (const PrintInstance& instance : print_object->instances()) {
                ...
                struct print_instance_info print_info
                {
                    &instance, convex_hull.bounding_box(), convex_hull
                };
...
// Adartys: this g92e0 regex is used copied from PrusaSlicer
                                      ...
                                      std::vector<std::pair<Polygon, float>>* height_polygons) const
{
    ...
    if (m_config.spiral_mode) {
        ...
        if (all_regions.size() > 1) {
            // Adartys: make sure regions are not compatible
    ...
    for (size_t print_object_idx = 0; print_object_idx < m_objects.size(); ++print_object_idx)
        if (const PrintObject& print_object = *m_objects[print_object_idx]; print_object.has_support_material() &&
                                                                            ...
                                                                            (print_object.config().support_style.value == smsTreeOrganic ||
                                                                             // Adartys: use organic as default
    ...
    // Adartys: G92 E0 is not supported when using absolute extruder addressing
    ...
    if (warning) {
        try {
            ...
            // check speed
            // Adartys: disable the speed check for now as we don't cap the speed
            // if (warning_key.empty()) {
            ...
            BOOST_LOG_TRIVIAL(warning) << "Adartys: validate motion ability failed: " << e.what() << std::endl;
        }
    }
...
Points Print::first_layer_wipe_tower_corners(bool check_wipe_tower_existance) const
{
        ...
        return corners;
    {
        ...
        for (Vec2d& pt : pts) {
            ...
            // Adartys: offset the wipe tower to the plate origin
...
const WipeTowerData& Print::wipe_tower_data(size_t filaments_cnt) const
{
    ...
    if (!is_step_done(psWipeTower) && filaments_cnt != 0) {
        ...
        if (m_config.purge_in_prime_tower && m_config.single_extruder_multi_material) {
            ...
            // Adartys: it's overshooting a bit, so let's reduce it a bit
            maximum *= 0.6;
...
void Print::_make_wipe_tower()
{
    ...
    // Adartys: itertate over wipe_volumes and change the non-zero values to the prime_volume
...
// Adartys: Implement prusa's filament shrink compensation approach
...
// Adartys: Implement prusa's filament shrink compensation approach, but amended so 100% from the user is the equivalent to 0 in adartys.
