...
void remove_bridges_from_contacts(
    const PrintConfig& print_config, const Layer& lower_layer, const LayerRegion& layerm, float fw, Polygons& contact_polygons)
{
        ...
        for (size_t support_layer_id = range.begin(); support_layer_id < range.end(); ++support_layer_id) {
            ...
            if (config.support_interface_top_layers == 0) {
                ...
                if (support_params.ironing && !top_contact_layer.empty()) {
                    // Adartys: save the top surface to be ironed later
            ...
            extrude_interface(top_contact_layer, raft_layer ?
                                                     InterfaceLayerType::RaftContact :
                                                     top_interfaces ? InterfaceLayerType::TopContact : InterfaceLayerType::InterfaceAsBase);
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(n_raft_layers, support_layers.size()),
                      [&support_layers, &layer_caches, &support_params, &bbox_object](const tbb::blocked_range<size_t>& range) {
                          for (size_t support_layer_id = range.begin(); support_layer_id < range.end(); ++support_layer_id) {
                              ...
                              // Adartys: Generate iron toolpath for contact layer
