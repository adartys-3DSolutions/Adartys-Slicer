...
inline bool is_adartys_open(const std::string& url) { return boost::starts_with(url, "orcaslicer://open"); }
...
{
    return is_adartys_open(url) || is_prusaslicer_open(url) || is_bambustudio_open(url) || is_cura_open(url);
}
