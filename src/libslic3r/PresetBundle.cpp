...
const char* PresetBundle::ADARTYS_DEFAULT_BUNDLE          = "Custom";
const char* PresetBundle::ADARTYS_DEFAULT_PRINTER_MODEL   = "MyKlipper 0.4 nozzle";
const char* PresetBundle::ADARTYS_DEFAULT_PRINTER_VARIANT = "0.4";
const char* PresetBundle::ADARTYS_DEFAULT_FILAMENT        = "Generic PLA @System";
const char* PresetBundle::ADARTYS_FILAMENT_LIBRARY        = "AdartysFilamentLibrary";
                                                        ...
                                                        ForwardCompatibilitySubstitutionRule   rule)
{
    ...
    for (auto& file : files) {
        ...
        if (boost::iends_with(file, ".adartys_printer") || boost::iends_with(file, ".adartys_filament") || boost::iends_with(file, ".zip")) {
...
void PresetBundle::remove_users_preset(AppConfig& config, std::map<std::string, std::map<std::string, std::string>>* my_presets)
{
    ...
    if (need_reset_printer_preset) {
        std::string default_printer_model = ADARTYS_DEFAULT_PRINTER_MODEL;
    ...
    ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Move ADARTYS_FILAMENT_LIBRARY to the beginning of the list
    for (size_t i = 0; i < vendor_names.size(); ++i) {
        if (vendor_names[i] == ADARTYS_FILAMENT_LIBRARY) {
    ...
    for (auto& vendor_name : vendor_names) {
        if (validation_mode && !vendor_to_validate.empty() && vendor_name != vendor_to_validate && vendor_name != ADARTYS_FILAMENT_LIBRARY)
            continue;
...
const std::string& PresetBundle::get_preset_name_by_alias(const Preset::Type& preset_type, const std::string& alias) const
{
    ...
    const PresetCollection& presets = preset_type == Preset::TYPE_PRINT ? prints :
                                                                          ...
                                                                          sla_prints :
                                                                          preset_type == Preset::TYPE_FILAMENT ? filaments : sla_materials;
...
void PresetBundle::update_selections(AppConfig& config)
{
    ...
    // Adartys: load from adartys_presets
    ...
    const PresetBundle*                  base_bundle)
{
    ...
    auto parse_subfile = [this, path, vendor_name, presets_loaded, current_vendor_profile,
        ...
        try {
            ...
            if (key_values.find(ADARTYS_JSON_KEY_RENAMED_FROM) != key_values.end()) {
                if (!unescape_strings_cstyle(key_values[ADARTYS_JSON_KEY_RENAMED_FROM], renamed_from)) {
        ...
        if (flags.has(LoadConfigBundleAttribute::LoadSystem)) {
            ...
            loaded.m_from_adartys_filament_lib = is_from_lib;
    ...
    configs.clear();
    filament_id_maps.clear();
    const auto is_adartys_lib = vendor_name == ADARTYS_FILAMENT_LIBRARY;
    for (auto& subfile : filament_subfiles) {
        std::string reason = parse_subfile(substitution_context, substitutions, flags, subfile, configs, filament_id_maps, presets,
                                           presets_loaded, is_adartys_lib);
        if (!reason.empty()) {
            ++m_errors;
            // parse error
            std::string subfile_path = path + "/" + vendor_name + "/" + subfile.second;
...
bool PresetBundle::has_errors() const
{
    ...
    // Adartys: check if all filament presets have compatible_printers setting
    for (auto& preset : filaments) {
        if (!preset.is_system)
            continue;
        // It's per design that the Adartys Filament Library can have the empty compatible_printers.
        if (preset.vendor->name == PresetBundle::ADARTYS_FILAMENT_LIBRARY)
            continue;
