...
std::string GCodeWriter::preamble()
{
    ...
    if (FLAVOR_IS(gcfRepRapSprinter) || FLAVOR_IS(gcfRepRapFirmware) || FLAVOR_IS(gcfMarlinLegacy) || FLAVOR_IS(gcfMarlinFirmware) ||
        FLAVOR_IS(gcfTeacup) || FLAVOR_IS(gcfRepetier) || FLAVOR_IS(gcfSmoothie) || FLAVOR_IS(gcfKlipper)) {
...
std::string GCodeWriter::set_chamber_temperature(int temperature, bool wait)
{
    ...
    if (wait) {
        // Adartys: should we let the M191 command to turn on the auxiliary fan?
...
std::string GCodeWriter::set_input_shaping(char axis, float damp, float freq, std::string type) const
{
    ...
    if (FLAVOR_IS(gcfKlipper)) {
        ...
        if (axis != 'A') {
            ...
            if (freq > 0.0f) {
                gcode << " SHAPER_FREQ_X=" << std::fixed << std::setprecision(2) << freq << " SHAPER_FREQ_Y=" << std::fixed
                      << std::setprecision(2) << freq;
            }
            if (damp > 0.0f) {
                gcode << " DAMPING_RATIO_X=" << std::fixed << std::setprecision(3) << damp << " DAMPING_RATIO_Y=" << std::fixed
                      << std::setprecision(3) << damp;
            }
        }
    } else if (FLAVOR_IS(gcfRepRapFirmware)) {
        gcode << "M593";
...
std::string GCodeWriter::toolchange_prefix() const
{
    return config.manual_filament_change ? ";" + GCodeProcessor::reserved_tag(GCodeProcessor::ETags::Manual_Tool_Change) + "T" :
                                           FLAVOR_IS(gcfMakerWare) ? "M135 T" : FLAVOR_IS(gcfSailfish) ? "M108 T" : "T";
}
...
std::string GCodeWriter::travel_to_xyz(const Vec3d& point, const std::string& comment, bool force_z)
{
    ...
    auto  travel_speed = m_is_first_layer ? this->config.get_abs_value("initial_layer_travel_speed") : this->config.travel_speed.value;
    ...
    if (std::abs(m_to_lift) > EPSILON) {
        ...
        if ((!this->is_current_position_clear() || m_pos != dest_point) && m_to_lift + m_pos(2) > point(2)) {
        ...
        if (delta(2) > 0 && delta_no_z.norm() != 0.0f) {
            ...
            // BBS: LazyLift
            else if (m_to_lift_type == LiftType::LazyLift && this->is_current_position_clear() &&
...
std::string GCodeWriter::_travel_to_z(double z, const std::string& comment)
{
    ...
    if (speed == 0.) {
        speed = m_is_first_layer ? this->config.get_abs_value("initial_layer_travel_speed") : this->config.travel_speed.value;
    }
...
std::string GCodeWriter::extrude_arc_to_xy(
    const Vec2d& point, const Vec2d& center_offset, double dE, const bool is_ccw, const std::string& comment, bool force_no_extrusion)
{
    m_pos(0) = point(0);
    m_pos(1) = point(1);
    if (!force_no_extrusion)
...
std::string GCodeWriter::retract(bool before_wipe, double retract_length)
{
    ...
    return this->_retract(retract_length > EPSILON ? retract_length : factor * m_extruder->retraction_length(),
                          factor * m_extruder->retract_restart_extra(), "retract");
}
...
std::string GCodeWriter::retract_for_toolchange(bool before_wipe, double retract_length)
{
    double factor = before_wipe ? m_extruder->retract_before_wipe() : 1.;
    assert(factor >= 0. && factor <= 1. + EPSILON);
    return this->_retract(retract_length > EPSILON ? retract_length : factor * m_extruder->retract_length_toolchange(),
                          factor * m_extruder->retract_restart_extra_toolchange(), "retract for toolchange");
}
...
std::string GCodeWriter::set_fan(const GCodeFlavor gcode_flavor, unsigned int speed)
{
    ...
    if (speed == 0) {
        switch (gcode_flavor) {
        case gcfTeacup: gcode << "M106 S0"; break;
        case gcfMakerWare:
        case gcfSailfish: gcode << "M127"; break;
        default: gcode << "M106 S0"; break;
        }
        if (GCodeWriter::full_gcode_comment)
            gcode << " ; disable fan";
        gcode << "\n";
        ...
        switch (gcode_flavor) {
        case gcfMakerWare:
        case gcfSailfish: gcode << "M126"; break;
        case gcfMach3:
        case gcfMachinekit: gcode << "M106 P" << static_cast<unsigned int>(255.5 * speed / 100.0); break;
        default: gcode << "M106 S" << static_cast<unsigned int>(255.5 * speed / 100.0); break;
        }
        if (GCodeWriter::full_gcode_comment)
            gcode << " ; enable fan";
        gcode << "\n";
...
void GCodeWriter::add_object_end_labels(std::string& gcode)
{
    if (!m_gcode_label_objects_end.empty()) {
        ...
        // Adartys: reset E so that e value remain correct after skipping the object
...
void GCodeFormatter::emit_axis(const char axis, const double v, size_t digits)
{
    ...
    *ptr_err.ptr++ = axis;
    ...
    if (writen_digits < digits) {
        ...
        for (char *from_ptr = this->ptr_err.ptr - 1, *to_ptr = from_ptr + remaining_digits; from_ptr >= this->ptr_err.ptr - writen_digits;
             --to_ptr, --from_ptr)
            *to_ptr = *from_ptr;
