                               ...
                               std::string&                        reason)
{
    ...
    try {
        ...
        for (auto it = j.begin(); it != j.end(); it++) {
            ...
            } else if (boost::iequals(it.key(), ADARTYS_JSON_KEY_RENAMED_FROM)) {
                key_values.emplace(ADARTYS_JSON_KEY_RENAMED_FROM, it.value());
            } else {
...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string& file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Look for Slic3r or AdartysSlicer header.
    ...
    {
        // BBS
        std::string bambuslicer_gcode_header = "; AdartysSlicer";

        std::string adartysslicer_gcode_header = std::string("; generated by ");
        adartysslicer_gcode_header += SLIC3R_APP_NAME;
        ...
        while (std::getline(ifs, header)) {
            ...
            if (strncmp(bambuslicer_gcode_header.c_str(), line_c, strlen(bambuslicer_gcode_header.c_str())) == 0 ||
                strncmp(adartysslicer_gcode_header.c_str(), line_c, strlen(adartysslicer_gcode_header.c_str())) == 0) {
                header_found = true;
                break;
            }
        }
...
void ConfigBase::save_to_json(const std::string& file,
                              ...
                              const std::string  is_custom) const
{
    json j;
    // record the headers
