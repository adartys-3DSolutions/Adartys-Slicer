...
void PrintConfigDef::init_common_params()
{
    ...
    def->tooltip = L("Adartys Slicer can upload G-code files to a printer host. This field should contain "
...
void PrintConfigDef::init_fff_params()
{
    ...
    // Adartys: make sure the order of the values is the same as the BedType enum
    ...
    // Adartys: allow profile maker to set default bed type in machine profile
    ...
    def->sidetext   = L("layers"); // ADARTYS add side text
    ...
    // ADARTYS: Internal bridge angle override
    ...
    def->tooltip = L(
        ...
        "set appropriately. AdartysSlicer ensures that adaptive_bed_mesh_min/adaptive_bed_mesh_max values do not exceed these min/max "
    ...
    // Adartys: Adaptive pressure advance option and calibration values
    ...
    */
    ...
    def->tooltip  = L("After a tool change, the exact position of the newly loaded filament inside "
                     ...
                     "Before purging the print head into an infill or a sacrificial object, Adartys Slicer will always prime "
    ...
    def->tooltip                     = L("Connect an infill line to an internal perimeter with a short segment of an additional perimeter. "
                     ...
                     "Adartys Slicer tries to connect two close infill lines to a short perimeter segment. If no such perimeter segment "
    ...
    def->sidetext = L("layer"); // ADARTYS add side text
    def->min      = 0;
    def->max      = 1000;
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionInts{0});
    ...
    // Adartys: may remove this option later
    ...
    // Adartys: max layer height for combined infill
    ...
    // ADARTYS: special flag for flow rate calibration
    ...
    def->tooltip       = L("Adartys Slicer can upload G-code files to a printer host. This field must contain "
    ...
    def->tooltip    = L("If you want to process the output G-code through custom scripts, "
                     ...
                     "and they can access the Adartys Slicer config settings by reading environment variables.");
    def->gui_flags  = "serialized";
    def->multiline  = true;
    def->full_width = true;
    def->height     = 6;
    ...
    def->tooltip = L(
        "To reduce the waiting time after tool change, Adartys can preheat the next tool while the current tool is still in use. "
        "This setting specifies the time in seconds to preheat the next tool. Adartys will insert a M104 command to preheat the "
        "tool in advance.");
    def->sidetext = "s"; // seconds, don't need translation
    def->min      = 0;
    def->max      = 120;
...
//    def->min = 0;
    ...
    for (const char* opt_key :
         {// floats
          "retraction_length", "z_hop", "z_hop_types", "retract_lift_above", "retract_lift_below", "retract_lift_enforce",
          ...
          // percents
          "retract_before_wipe", "long_retractions_when_cut", "retraction_distances_when_cut"}) {
        ...
        if ((strcmp(opt_key, "retraction_length") == 0) || (strcmp(opt_key, "z_hop") == 0) ||
            (strcmp(opt_key, "long_retractions_when_cut") == 0) || (strcmp(opt_key, "retraction_distances_when_cut") == 0))
            def->mode = comSimple;
        else
            def->mode = comAdvanced;
        switch (def->type) {
...
CLIMiscConfigDef::CLIMiscConfigDef()
{
    ...
    def->tooltip = L("This version of AdartysSlicer may not understand configurations produced by the newest AdartysSlicer versions. "
                     "For example, newer AdartysSlicer may extend the list of supported firmware flavors. One may decide to "
    ...
    def->tooltip = L("If enabled, the command line arguments are sent to an existing instance of GUI AdartysSlicer, "
                     "or an existing AdartysSlicer window is activated. "
...
ReadWriteSlicingStatesConfigDef::ReadWriteSlicingStatesConfigDef()
{
    ...
    def->tooltip = L("Position of the extruder at the beginning of the custom G-code block. If the custom G-code travels somewhere else, "
                     "it should write to this variable so AdartysSlicer knows where it travels from when it gets control back.");
    ...
    def->tooltip = L("Retraction state at the beginning of the custom G-code block. If the custom G-code moves the extruder axis, "
                     "it should write to this variable so AdartysSlicer de-retracts correctly when it gets control back.");
...
OtherSlicingStatesConfigDef::OtherSlicingStatesConfigDef()
{
    ...
    // Options from PS not used in Adartys
...
Points get_bed_shape(const PrintConfig& cfg) { return to_points(make_counter_clockwise(cfg.printable_area.values)); }
