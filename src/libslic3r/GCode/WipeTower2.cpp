// Adartys: WipeTower2 for all non bbl printers, support all MMU device and toolchanger.
...
public:
    ...
    {
        // ADARTYS: This class is only used by non BBL printers, so set the parameter appropriately.
                                   ...
                                   const int                         new_temperature)
{
    ...
    // Adartys: Do ramming when SEMM and ramming is enabled or when multi tool head when ramming is enabled on the multi tool.
...
void WipeTower2::plan_toolchange(float z_par, float layer_height_par, unsigned int old_tool, unsigned int new_tool, float wipe_volume)
{
    ...
    // Adartys: Set ramming depth to 0 if ramming is disabled.
...
{
    // Adartys: allow calculation of the required depth and wipe volume for soluable toolchanges as well
    ...
    WipeTowerWriter2& writer, const WipeTower::box_coordinates& wt_box, double feedrate, bool infill_cone, float spacing)
{
    ...
    for (int i = int(pts.size()) - 3; i > 0; --i)
        pts.emplace_back(Vec2f(pts[i].first.x(), 2 * center.y() - pts[i].first.y()),
                         i == int(pts.size()) - 3 ? ArcStart : i == 1 ? ArcEnd : Arc);
