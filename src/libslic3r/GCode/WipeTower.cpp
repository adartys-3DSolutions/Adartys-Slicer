...
{
public:
    WipeTowerWriter(float                                             layer_height,
                    float                                             line_width,
                    GCodeFlavor                                       flavor,
    ...
    {
        // ADARTYS: This class is only used by BBL printers, so set the parameter appropriately.
    ...
    {
        change_analyzer_line_width(0.f);
        m_preview_suppressed = true;
        return *this;
    }
    WipeTowerWriter& resume_preview()
    {
        change_analyzer_line_width(m_default_analyzer_line_width);
        m_preview_suppressed = false;
        return *this;
    }
#else
    ...
    WipeTowerWriter& extrude_explicit(
...
WipeTower::ToolChangeResult WipeTower::construct_tcr(
    WipeTowerWriter& writer, bool priming, size_t old_tool, bool is_finish, float purge_volume) const
{
...
// BBS
const std::map<float, float> WipeTower::min_depth_per_height = {{100.f, 20.f}, {250.f, 40.f}};

WipeTower::WipeTower(const PrintConfig& config,
                     int                plate_idx,
                     Vec3d              plate_origin,
                     const float        prime_volume,
                     size_t             initial_tool,
...
WipeTower::ToolChangeResult WipeTower::tool_change(size_t tool, bool extrude_perimeter, bool first_toolchange_to_nonsoluble)
{
    ...
    if (tool != (unsigned int) -1) { // This is not the last change.
        ...
        if (extrude_perimeter) {
            box_coordinates wt_box(Vec2f(0.f, (m_current_shape == SHAPE_REVERSED) ? m_layer_info->toolchanges_depth() - m_layer_info->depth :
                                                                                    0.f),
...
// Ram the hot material out of the melt zone, retract the filament into the cooling tubes and let it cool.
void WipeTower::toolchange_Unload(WipeTowerWriter&       writer,
...
void WipeTower::toolchange_Wipe(WipeTowerWriter& writer, const box_coordinates& cleaning_box, float wipe_length)
{
...
WipeTower::ToolChangeResult WipeTower::finish_layer(bool extrude_perimeter, bool extruder_fill)
{
    ...
    box_coordinates fill_box(Vec2f(m_perimeter_width, fill_box_y), m_wipe_tower_width - 2 * m_perimeter_width,
    ...
    // BBS
    box_coordinates wt_box(Vec2f(0.f, (m_current_shape == SHAPE_REVERSED ? m_layer_info->toolchanges_depth() : 0.f)), m_wipe_tower_width,
    ...
    float z_par, float layer_height_par, unsigned int old_tool, unsigned int new_tool, float wipe_volume, float purge_volume)
{
    ...
    m_plan.back().tool_changes.push_back(
        WipeTowerInfo::ToolChange(old_tool, new_tool, depth, 0.f, 0.f, wipe_volume, length_to_extrude, purge_volume));
#endif
}
    ...
    for (m_layer_info = m_plan.begin(); m_layer_info < m_plan.end(); ++m_layer_info) {
        ...
        for (int i = 0; i < int(m_layer_info->tool_changes.size()); ++i) {
            ...
            if (i == idx) {
                ...
                float length_to_wipe = volume_to_length(toolchange.wipe_volume, m_perimeter_width, m_layer_info->height) -
                                       toolchange.first_wipe_line - length_to_save;
...
int WipeTower::first_toolchange_to_nonsoluble_nonsupport(const std::vector<WipeTowerInfo::ToolChange>& tool_changes) const
{
