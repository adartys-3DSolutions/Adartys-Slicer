    ...
    case 'G': {
...
#ifdef PRESSURE_EQUALIZER_DEBUG
                    if (rate < 40.f) {
                        printf("Extremely low flow rate: %f. Line %d, Length: %f, extrusion: %f Old position: (%f, %f, %f), new position: "
                               "(%f, %f, %f)\n",
                               rate, int(line_idx), sqrt(len2), sqrt((diff[3] * diff[3]) / len2), m_current_pos[0], m_current_pos[1],
                               m_current_pos[2], new_pos[0], new_pos[1], new_pos[2]);
                    }
#endif
                }
            } else if (changed[0] || changed[1] || changed[2]) {
        ...
        case 92: {
            ...
            while (!is_eol(*line)) {
                ...
                case 'X':
                case 'Y':
                case 'Z': m_current_pos[axis - 'X'] = (!is_ws_or_eol(*line)) ? parse_float(line, line_end - line) : 0.f; break;
                case 'E': m_current_pos[3] = (!is_ws_or_eol(*line)) ? parse_float(line, line_end - line) : 0.f; break;
                default: break;
                }
                eatws(line);
            }
...
void PressureEqualizer::output_gcode_line(const size_t line_idx)
{
    ...
    // Adartys:
    // First, check if the change in volumetric extrusion rate is trivial (less than 10mm3/min -> 0.16mm3/sec (5mm/sec speed for a 0.25 mm
    // nozzle). Or if the line size is equal in length with the smallest segment. If so, then emit the line as a single extrusion, i.e. dont
    // split into segments.
    ...
    } else // The line needs to be split the line into segments and apply extrusion rate smoothing
    {
        ...
        float t_acc = 0.5f * (line.volumetric_extrusion_rate_start + line.volumetric_extrusion_rate_end) /
...
void PressureEqualizer::adjust_volumetric_rate(const size_t fist_line_idx, const size_t last_line_idx)
{
    ...
    while (line_idx != fist_line_idx) {
        ...
        for (size_t iRole = 1; iRole < size_t(ExtrusionRole::erCount); ++iRole) {
            ...
            // Adartys: Limit ERS to external perimeters and overhangs if option selected by user
            if (!line.adjustable_flow || line.extrusion_role == ExtrusionRole::erBridgeInfill ||
                line.extrusion_role == ExtrusionRole::erIroning ||
                (m_extrusion_rate_smoothing_external_perimeter_only && line.extrusion_role != ExtrusionRole::erOverhangPerimeter &&
                 line.extrusion_role != ExtrusionRole::erExternalPerimeter)) {
            ...
            if (line.adjustable_flow) {
                float rate_start = sqrt(rate_end * rate_end +
                                        2 * line.volumetric_extrusion_rate * line.dist_xyz() * rate_slope / line.feedrate());
            ...
            //            feedrate_per_extrusion_role[iRole] = (iRole == line.extrusion_role) ? line.volumetric_extrusion_rate_start :
            //            rate_start;
    ...
    while (line_idx != last_line_idx) {
        ...
        for (size_t iRole = 1; iRole < size_t(ExtrusionRole::erCount); ++iRole) {
            ...
            if (line.adjustable_flow) {
                float rate_end = sqrt(rate_start * rate_start +
                                      2 * line.volumetric_extrusion_rate * line.dist_xyz() * rate_slope / line.feedrate());
...
inline void PressureEqualizer::push_to_output(GCodeG1Formatter& formatter) { return this->push_to_output(formatter.string(), false); }
...
{
    // Adartys: sanity check, 1 mm/s is the minimum feedrate.
    if (new_feedrate < 60)
        new_feedrate = 60;
    ...
    if (line_idx > 0 && output_buffer_length > 0) {
        ...
        if (is_just_line_with_extrude_set_speed_tag(prev_line_str))
            this->output_buffer_length = this->output_buffer_prev_length; // Remove the last line because it only sets the speed for an
                                                                          // empty block of g-code lines, so it is useless.
        else
    ...
    extrusion_formatter.emit_axis('E', m_use_relative_e_distances ? (line.pos_end[3] - line.pos_start[3]) : line.pos_end[3],
