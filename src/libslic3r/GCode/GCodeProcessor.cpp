...
// Adartys: External device purge tag
...
float GCodeProcessor::Trapezoid::cruise_time() const { return (cruise_feedrate != 0.0f) ? cruise_distance() / cruise_feedrate : 0.0f; }
...
void GCodeProcessor::TimeBlock::calculate_trapezoid()
{
    ...
    float decelerate_distance = std::max(0.0f,
                                         estimated_acceleration_distance(feedrate_profile.cruise, feedrate_profile.exit, -acceleration));
...
void planner_reverse_pass_kernel(GCodeProcessor::TimeBlock& curr, GCodeProcessor::TimeBlock& next)
{
    ...
    if (curr.feedrate_profile.entry != curr.max_entry_speed) {
        ...
        if (!curr.flags.nominal_length && curr.max_entry_speed > next.feedrate_profile.entry)
            curr.feedrate_profile.entry = std::min(curr.max_entry_speed,
...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    // BBS: AdartysSlicer is also "bambu". Otherwise the time estimation didn't work.
    // FIXME: Workaround and should be handled when do removing-bambu
    {EProducer::AdartysSlicer, SLIC3R_APP_NAME},
    {EProducer::AdartysSlicer, "generated by AdartysSlicer"},
    {EProducer::AdartysSlicer, "generated by BambuStudio"},
    {EProducer::AdartysSlicer, "BambuStudio"}
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        // if the gcode was produced by AdartysSlicer,
        // extract the config from it
        if (m_producer == EProducer::AdartysSlicer || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from AdartysSlicer's own G-code.
...
void GCodeProcessor::finalize(bool post_process)
{
    ...
    auto it           = std::find_if(time_mode.roles_times.begin(), time_mode.roles_times.end(),
                           [](const std::pair<ExtrusionRole, float>& item) { return erCustom == item.first; });
    ...
    for (size_t i = 0; i < m_result.moves.size(); i++) {
        ...
        std::vector<float>& layer_times = m_result.print_statistics.modes[static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Normal)]
                                              .layers_times;
        if (layer_times.size() > layer_id - 1 && layer_id > 0)
            m_result.moves[i].layer_duration = layer_id == 1 ? std::max(0.f, layer_times[layer_id - 1] - prepare_time) :
...
std::string GCodeProcessor::get_time_dhm(PrintEstimatedStatistics::ETimeMode mode) const
{
    return (mode < PrintEstimatedStatistics::ETimeMode::Count) ?
               short_time(get_time_dhms(m_time_processor.machines[static_cast<size_t>(mode)].time)) :
               std::string("N/A");
}
...
{
    return (mode < PrintEstimatedStatistics::ETimeMode::Count) ? m_time_processor.machines[static_cast<size_t>(mode)].layers_time :
                                                                 std::vector<float>();
}
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    // Adartys: Integrate filament consumption for purging performed to an external device and controlled via macros
    ...
    if (!producers_enabled || m_producer == EProducer::AdartysSlicer) {
        // height tag
        ...
        // Adartys: manual tool change tag
...
void GCodeProcessor::process_G2_G3(const GCodeReader::GCodeLine& line)
{
        ...
        for (unsigned char a = X; a <= Z; ++a) {
            ...
            if (axis_acc[a] != 0.0f) {
                ...
                if (axis_max_acceleration != 0.0f && axis_acc[a] > axis_max_acceleration)
                    min_acc_factor = std::min<float>(min_acc_factor, axis_max_acceleration / axis_acc[a]);
            }
        }
    ...
    // Adartys: we now use spiral_vase_layers for proper layer detect when scarf joint is enabled,
...
void GCodeProcessor::process_T(const std::string_view command)
{
    ...
    if (command.length() > 1) {
        if (new_extruder < 0 || new_extruder > 254) {
            ...
            if ((m_flavor == gcfMarlinLegacy || m_flavor == gcfMarlinFirmware) &&
                (command == "Tx" || command == "Tc" || command == "T?" || new_extruder == 1000 || new_extruder == 1100 ||
                 new_extruder == 255))
                return;
...
void GCodeProcessor::run_post_process()
{
    ...
    public:
        ...
        {
            // Adartys: find start pos by seaching G28/G29/PRINT_START/START_PRINT commands
    ...
    auto process_placeholders = [&](std::string& gcode_line) {
        ...
        if (line.length() > 1) {
            ...
            if (true &&
                ...
                for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i) {
                    ...
                    if (mode == PrintEstimatedStatistics::ETimeMode::Normal || machine.enabled) {
                        char buf[128];
                        if (!s_IsBBLPrinter)
                            // Adartys: compatibility with klipper_estimator
            ...
            // Adartys: write total layer number, this is used by Bambu printers only as of now
    ...
    auto process_line_G1 = [this,
                            ...
                            time_in_minutes, format_time_float, format_line_M73_main, format_line_M73_stop_int, format_line_M73_stop_float,
                            time_in_last_minute,
                            // Caches, to be modified
                            &g1_times_cache_it, &last_exported_main, &last_exported_stop, &export_lines](const size_t g1_lines_counter) {
        if (true) {
            for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i) {
                const TimeMachine& machine = m_time_processor.machines[i];
                if (machine.enabled) {
                    ...
                    if (it != machine.g1_times_cache.end() && it->id == g1_lines_counter) {
                        ...
                        if (last_exported_main[i] != to_export_main) {
                            export_lines.append_line(format_line_M73_main(machine.line_m73_main_mask.c_str(), to_export_main.first,
                        ...
                        if (it_stop != machine.stop_times.end()) {
                            ...
                            if (last_exported_stop[i] != to_export_stop) {
                                if (to_export_stop > 0) {
                                    if (last_exported_stop[i] != to_export_stop) {
                                        export_lines.append_line(format_line_M73_stop_int(machine.line_m73_stop_mask.c_str(),
                                                                                          to_export_stop),
                                                                 true);
                                        last_exported_stop[i] = to_export_stop;
                                    }
            ...
            if (tool_number != -1) {
                ...
                export_lines.insert_lines(
                    ...
                    [tool_number, this](unsigned int id, const std::vector<float>& time_diffs) {
                        ...
                        // Adartys: M104.1 for XL printers, I can't find the documentation for this so I copied the C++ comments from
...
void GCodeProcessor::store_move_vertex(EMoveType type, EMovePathType path_type)
{
    ...
    if (path_type == EMovePathType::Arc_move_cw || path_type == EMovePathType::Arc_move_ccw) {
        for (size_t i = 0; i < m_interpolation_points.size(); i++)
            m_interpolation_points[i] = Vec3f(m_interpolation_points[i].x() + m_x_offset, m_interpolation_points[i].y() + m_y_offset,
