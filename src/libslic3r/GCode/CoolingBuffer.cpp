...
struct CoolingLine
{
    enum Type {
        ...
        // ADARTYS: Add support for separate internal bridge fan speed control
        TYPE_INTERNAL_BRIDGE_FAN_START = 1 << 17,
        TYPE_INTERNAL_BRIDGE_FAN_END   = 1 << 18,
        // ADARTYS: Add support for ironing fan speed control
        TYPE_IRONING_FAN_START = 1 << 19,
        TYPE_IRONING_FAN_END   = 1 << 20,
    };

    CoolingLine(unsigned int type, size_t line_start, size_t line_end)
        : type(type), line_start(line_start), line_end(line_end), length(0.f), feedrate(0.f), time(0.f), time_max(0.f), slowdown(false)
    {}

    bool adjustable(bool slowdown_external_perimeters) const
    {
        return (this->type & TYPE_ADJUSTABLE) && (!(this->type & TYPE_EXTERNAL_PERIMETER) || slowdown_external_perimeters) &&
        ...
        for (auto it = it_begin; it != it_end; ++it)
            for (size_t i = 0; i < (*it)->n_lines_adjustable; ++i) {
                ...
                if (line.feedrate > min_feedrate && line.feedrate < new_feedrate)
                    ...
                    // Re-run the calculation with a new min_feedrate limit, so that the segments with current feedrate lower than
                    // new_feedrate are not taken into account.
...
std::vector<PerExtruderAdjustments> CoolingBuffer::parse_layer_gcode(const std::string& gcode, std::vector<float>& current_pos) const
{
    ...
    for (size_t i = 0; i < m_extruder_ids.size(); ++i) {
        ...
        // ADARTYS: To enable dont slow down external perimeters feature per filament (extruder)
    ...
    // Adartys: Whether we had our first extrusion in this layer.
    ...
    for (; *line_start != 0; line_start = line_end) {
        ...
        if (line.type) {
            ...
            for (;;) {
                ...
                size_t axis = (*c >= 'X' && *c <= 'Z') ? (*c - 'X') :
                                                         (*c == 'E') ? 3 : (*c == 'F') ? 4 : (*c == 'I') ? 5 : (*c == 'J') ? 6 : size_t(-1);
                if (axis != size_t(-1)) {
            ...
            // Adartys: only slow down movements since the first extrusion
            if (boost::contains(sline, ";_EXTRUDE_SET_SPEED"))
                layer_had_extrusion = true;

            // ADARTYS: Dont slowdown external perimeters for layer time feature
            ...
            // ADARTYS: Dont slowdown external perimeters for layer time works by not marking the external perimeter as adjustable,
            ...
            if ((line.type & CoolingLine::TYPE_G92) == 0) {
                ...
                if ((line.type & CoolingLine::TYPE_ADJUSTABLE) || active_speed_modifier != size_t(-1))
                    line.time_max = (adjustment->slow_down_min_speed == 0.f) ?
                                        FLT_MAX :
        ...
        } else if (boost::starts_with(sline,
                                      ";_INTERNAL_BRIDGE_FAN_START")) { // ADARTYS: Add support for separate internal bridge fan speed control
            line.type = CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_START;
        } else if (boost::starts_with(sline,
                                      ";_INTERNAL_BRIDGE_FAN_END")) { // ADARTYS: Add support for separate internal bridge fan speed control
        ...
        } else if (boost::starts_with(sline, ";_IRONING_FAN_START")) { // ADARTYS: Add support for ironing fan speed control
            line.type = CoolingLine::TYPE_IRONING_FAN_START;
        } else if (boost::starts_with(sline, ";_IRONING_FAN_END")) { // ADARTYS: Add support for ironing fan speed control
            ...
            assert(is_decimal_separator_point()); // for atof
            line.time = line.time_max = float((pos_S > 0) ? atof(sline.c_str() + pos_S + 1) :
                                                            (pos_P > 0) ? atof(sline.c_str() + pos_P + 1) * 0.001 : 0.);
        } else if (boost::starts_with(sline, ";_FORCE_RESUME_FAN_SPEED")) {
            line.type = CoolingLine::TYPE_FORCE_RESUME_FAN;
        }

        // Adartys: For any movements before this layer's first ever extrusion, we exclude them from the layer time calculation.
                                                             ...
                                                             float                                          time_stretch)
{
    ...
    std::sort(by_min_print_speed.begin(), by_min_print_speed.end(), [](const PerExtruderAdjustments* p1, const PerExtruderAdjustments* p2) {
...
float CoolingBuffer::calculate_layer_slowdown(std::vector<PerExtruderAdjustments>& per_extruder_adjustments)
{
    ...
    std::sort(by_slowdown_time.begin(), by_slowdown_time.end(), [](const PerExtruderAdjustments* adj1, const PerExtruderAdjustments* adj2) {
    ...
    std::vector<PerExtruderAdjustments>& per_extruder_adjustments)
{
    ...
    bool internal_bridge_fan_control = false; // ADARTYS: Add support for separate internal bridge fan speed control
    int  internal_bridge_fan_speed   = 0;     // ADARTYS: Add support for separate internal bridge fan speed control
    bool supp_interface_fan_control  = false;
    int  supp_interface_fan_speed    = 0;
    bool ironing_fan_control         = false; // ADARTYS: Add support for ironing fan speed control
    int  ironing_fan_speed           = 0;     // ADARTYS: Add support for ironing fan speed control
    auto change_extruder_set_fan     = [this, layer_id, layer_time, &new_gcode, &overhang_fan_control, &overhang_fan_speed,
                                    &internal_bridge_fan_control, &internal_bridge_fan_speed, &supp_interface_fan_control,
                                    &supp_interface_fan_speed, &ironing_fan_control, &ironing_fan_speed](bool immediately_apply) {
#define EXTRUDER_CONFIG(OPT) m_config.OPT.get_at(m_current_extruder)
        ...
        if (close_fan_the_first_x_layers <= 0 && full_fan_speed_layer > 0) {
            // When ramping up fan speed from close_fan_the_first_x_layers to full_fan_speed_layer, force close_fan_the_first_x_layers above
            // zero, so there will be a zero fan speed at least at the 1st layer.
        ...
        if (int(layer_id) >= close_fan_the_first_x_layers) {
            ...
            if (int(layer_id) >= close_fan_the_first_x_layers && int(layer_id) + 1 < full_fan_speed_layer) {
                ...
                float factor = float(int(layer_id + 1) - close_fan_the_first_x_layers) /
            ...
            if (internal_bridge_fan_speed < 0) { // ADARTYS: Backwards compatibility setting for Adartys internal bridge fan speed setting -
                                                 // if set at -1 (which is the default) use the overhang fan speed settings.
...
#undef EXTRUDER_CONFIG
            ...
            internal_bridge_fan_control = false; // ADARTYS: Add support for separate internal bridge fan speed control
            internal_bridge_fan_speed   = 0;     // ADARTYS: Add support for separate internal bridge fan speed control
            ironing_fan_control         = false; // ADARTYS: Add support for ironing fan speed control
            ironing_fan_speed           = 0;     // ADARTYS: Add support for ironing fan speed control
        }
    ...
    // Adartys: Reduce set fan commands by deferring the GCodeWriter::set_fan calls. Inspired by SuperSlicer
    // define fan_speed_change_requests and initialize it with all possible types fan speed change requests
    std::unordered_map<int, bool> fan_speed_change_requests =
        {{CoolingLine::TYPE_OVERHANG_FAN_START, false},
         {CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_START, false}, // ADARTYS: Add support for separate internal bridge fan speed control
         {CoolingLine::TYPE_SUPPORT_INTERFACE_FAN_START, false},
         {CoolingLine::TYPE_IRONING_FAN_START, false}, // ADARTYS: Add support for ironing fan speed control
    ...
    for (const CoolingLine* line : lines) {
        ...
        if (line->type & CoolingLine::TYPE_SET_TOOL) {
            ...
            need_set_fan = true;
                   ...
                   CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_START) { // ADARTYS: Add support for separate internal bridge fan speed control
            if (internal_bridge_fan_control && !fan_speed_change_requests[CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_START]) {
                ...
                fan_speed_change_requests[CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_START] = true;
                   ...
                   CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_END) { // ADARTYS: Add support for separate internal bridge fan speed control
            if (internal_bridge_fan_control && fan_speed_change_requests[CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_START]) {
            ...
            if (supp_interface_fan_control && !fan_speed_change_requests[CoolingLine::TYPE_SUPPORT_INTERFACE_FAN_START]) {
                ...
                need_set_fan                                                             = true;
                   ...
                   fan_speed_change_requests[CoolingLine::TYPE_SUPPORT_INTERFACE_FAN_START]) {
            ...
            if (m_fan_speed != -1 && !std::any_of(fan_speed_change_requests.begin(), fan_speed_change_requests.end(),
        ...
        } else if (line->type & (CoolingLine::TYPE_ADJUSTABLE | CoolingLine::TYPE_EXTERNAL_PERIMETER | CoolingLine::TYPE_WIPE |
            ...
            if (new_feedrate == current_feedrate) {
                ...
                if ((line->type & (CoolingLine::TYPE_ADJUSTABLE | CoolingLine::TYPE_EXTERNAL_PERIMETER | CoolingLine::TYPE_WIPE)) ||
                    line->length == 0.)
                    // Feedrate does not change and this line does not move the print head. Skip the complete G-code line including the
                    // G-code comment.
                    end = line_end;
                else
        ...
        if (need_set_fan) {
            ...
            } else if (fan_speed_change_requests[CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_START]) { // ADARTYS: Add support for separate
                                                                                                 // internal bridge fan speed control
