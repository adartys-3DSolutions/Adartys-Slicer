...
void GCode::_do_export(Print& print, GCodeOutputStream& file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    // Adartys: Don't output Header block if BTT thumbnail is identified in the list
    ...
    if (!has_BTT_thumbnail) {
        ...
        // Adartys: extra check for bbl printer
        if (is_bbl_printers) {
    ...
    if (print.config().print_sequence == PrintSequence::ByObject) {
        ...
        if (tool_ordering.all_extruders().empty())
            ...
            throw Slic3r::SlicingError(_(L("No object can be printed. Maybe too small")));
        has_wipe_tower = print.has_wipe_tower() && tool_ordering.has_wipe_tower();
        // Adartys: support all extruder priming
    ...
    // Adartys: Initialise AdaptivePA processor filter
    ...
    // Adartys: set the key for compatibilty
    ...
    // Adartys: support max MAXIMUM_EXTRUDER_NUMBER extruders/filaments
    ...
    // Adartys: set chamber temperature at the beginning of gcode file
 ...
 */
    ...
    // Adartys: when activate_air_filtration is set on any extruder, find and set the highest during_print_exhaust_fan_speed
    ...
    // Adartys: support extruder priming
...
{
    // Adartys: Added CMake config option since debug is rarely used in current workflow.
                                                  ...
                                                  std::map<unsigned int, std::pair<size_t, size_t>>& skirt_loops_per_extruder_out)
{
    ...
    // ADARTYS: Always extrude skirt with first extruder, independantly of if the minimum skirt length is zero or not. The code below
...
// Adartys: Klipper can't parse object names with spaces and other spetical characters
                                 ...
                                 const bool prime_extruder)
{
    ...
    // BBS
    if (first_layer) {
        // Adartys: we don't need to optimize the Klipper as only set once
    ...
    if (!first_layer && !m_second_layer_things_done) {
        ...
        // Adartys: only set once, don't need to call set_accel_and_jerk
        if (m_config.default_acceleration.value > 0 && m_config.initial_layer_acceleration.value > 0) {
    ...
    for (unsigned int extruder_id : layer_tools.extruders) {
        ...
        for (int print_wipe_extrusions = is_anything_overridden; print_wipe_extrusions >= 0; --print_wipe_extrusions) {
            ...
            for (InstanceToPrint& instance_to_print : instances_to_print) {
                ...
                // Adartys(#7946): set current obj regardless of the `enable_overhang_speed` value, because
    ...
    ExtrusionLoop loop, std::string description, double speed, const ExtrusionEntitiesPtr& region_perimeters, const Point* start_point)
{
    ...
    // Adartys: Adaptive PA: calculate average mm3_per_mm value over the length of the loop.
    ...
    // Adartys: end of multipath average mm3_per_mm value calculation

    if (!enable_seam_slope) {
        for (ExtrusionPaths::iterator path = paths.begin(); path != paths.end(); ++path) {
            gcode += this->_extrude(*path, description, speed_for_path(*path));
            // Adartys: Adaptive PA - dont adapt PA after the first pultipath extrusion is completed
...
std::string GCode::extrude_multi_path(ExtrusionMultiPath multipath, std::string description, double speed)
{
    ...
    // Adartys: calculate multipath average mm3_per_mm value over the length of the path.
    // This is used for adaptive PA
...
{
    // Adartys: Reset average multipath flow as this is a single line, single extrude volumetric speed path
...
std::string GCode::_extrude(const ExtrusionPath& path, std::string description, double speed)
{
    ...
    if (!m_last_pos_defined || m_last_pos != path.first_point() || m_need_change_layer_lift_z || slope_need_z_travel) {
        ...
        // Adartys: ensure Z matches planned layer height
    ...
    // Adartys: optimize for Klipper, set acceleration and jerk in one command
    ...
    // ADARTYS: resonance‑avoidance on short external perimeters
    {
    ...
    // Adartys: End of dynamic PA trigger flag segment

    // Adartys: process custom gcode for extrusion role change
    ...
    auto check_overhang_fan = [&overhang_fan_threshold](float overlap, ExtrusionRole role) {
        if (role == erBridgeInfill ||
            role == erOverhangPerimeter) { // ADARTYS: Split out bridge infill to internal and external to apply separate fan settings
            return true;
        }
    ...
    if (!variable_speed) {
        ...
        if ((std::abs(writer().get_current_speed() - F) > EPSILON) || (std::abs(_mm3_per_mm - m_last_mm3_mm) > EPSILON)) {
            // ADARTYS: Adaptive PA code segment when adjusting PA within the same feature
            ...
            // ADARTYS: End of adaptive PA code segment
        }
                    ...
                    append_role_based_fan_marker(
                        ...
                        (overhang_fan_threshold == Overhang_threshold_none && is_external_perimeter(path.role())) ||
                            (path.role() == erBridgeInfill ||
                             path.role() == erOverhangPerimeter)); // ADARTYS: Add support for separate internal bridge fan speed control
        ...
        if (path.role() == erInternalBridgeInfill) // ADARTYS: Add support for separate internal bridge fan speed control
...
// Adartys: get string name of extrusion role. used for change_extruder_role_gcode
...
std::string GCode::travel_to(const Point& point, ExtrusionRole role, std::string comment, double z /* = DBL_MAX*/)
{
    ...
    if (needs_retraction) {
        // ADARTYS: Fix scenario where wipe is disabled when avoid crossing perimeters was enabled even though a retraction move was
    ...
    if (travel.size() >= 2) {
        // Adartys: use `travel_to_xyz` to ensure we start at the correct z, in case we moved z in custom/filament change gcode
...
std::string GCode::retract(bool toolchange, bool is_last_retraction, LiftType lift_type, ExtrusionRole role)
{
    ...
    // Adartys: check if should + can lift (roughly from SuperSlicer)
...
std::string GCode::set_extruder(unsigned int extruder_id, double print_z, bool by_object)
{
    ...
    if (m_writer.extruder() != nullptr || m_start_gcode_filament != -1) {
        ...
        // Adartys: always calculate wipe volume and hence provide correct flush_length, so that MMU devices with cutter and purge bin (e.g.
    ...
    // Adartys: Ignore change_filament_gcode if is the first call for a tool change and manual_filament_change is enabled
    ...
    // Adartys: tool changer or IDEX's firmware may change Z position, so we set it to unknown/undefined
...
std::string GCode::set_object_info(Print* print)
{
    ...
    std::ostringstream gcode;
    size_t             object_id = 0;
    // Adartys: check if we are in pa calib mode
    if (print->calib_mode() == CalibMode::Calib_PA_Line || print->calib_mode() == CalibMode::Calib_PA_Pattern) {
        BoundingBoxf bbox_bed(print->config().printable_area.values);
