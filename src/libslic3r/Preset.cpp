...
void Preset::save_info(std::string file)
{
    ...
    c << "sync_info"
      << " = " << sync_info_to_save << std::endl;
    c << "user_id"
      << " = " << this->user_id << std::endl;
    c << "setting_id"
      << " = " << this->setting_id << std::endl;
    c << "base_id"
      << " = " << this->base_id << std::endl;
    c << "updated_time"
      << " = " << std::to_string(this->updated_time) << std::endl;
    c.close();
}
...
{
    // Adartys: we allow cross vendor compatibility
                                ...
                                const DynamicPrintConfig*      extra_config)
{
    ...
    // Adartys: check excluded printers
    if (preset.vendor != nullptr && preset.preset.type == Preset::TYPE_FILAMENT) {
        ...
        const auto  excluded          = preset.vendor->name == PresetBundle::ADARTYS_FILAMENT_LIBRARY &&
...
static std::vector<std::string> s_Preset_filament_options{
    ...
    "filament_type", "filament_soluble", "filament_is_support", "filament_max_volumetric_speed", "filament_flow_ratio", "filament_density",
        ...
        if (Slic3r::is_json_file(file_name)) {
            ...
            try {
                ...
                try {
                    ...
                    // Adartys: find and use the inherit config as the base
                    ...
                    if (inherits_config) {
                        ...
                        // Adartys: try to find if the parent preset has been renamed
...
Preset& PresetCollection::load_preset(const std::string&        path,
    ...
    const std::string filament_id)
{
    ...
    if (!inherits.empty() && (different_settings_list.size() > 0)) {
            ...
            for (auto& opt : keys) {
                ...
                if (opt_src && opt_dst && (*opt_src != *opt_dst)) {
                    BOOST_LOG_TRIVIAL(debug)
                        << __FUNCTION__
                        << boost::format(" change key %1% from old_value %2% to inherit's value %3%, preset_name %4%, inherits_name %5%") %
        ...
        for (auto& opt : keys) {
            ...
            if (opt_src && opt_dst && (*opt_src != *opt_dst)) {
                BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                         << boost::format(" change key %1% from old_value %2% to new_value %3%, preset_name %4%") % opt %
                                                (opt_dst->serialize()) % (opt_src->serialize()) % original_name;
                opt_dst->set(opt_src);
            }
        }
    }
...
// BBS: add project embedded preset logic
void PresetCollection::save_current_preset(
    const std::string& new_name, bool detach, bool save_to_project, Preset* _curr_preset, const Preset* _current_printer)
{
    ...
    if (it != m_presets.end() && it->name == new_name) {
        ...
        // Adartys: check if compatible_printers exists and is not empty, set it to the current printer if it is empty
    ...
    for (const std::string& printer_name : compatible_printers->values) {
        ...
        if (m_printer_hold_alias.end() != printer_iter) {
            ...
            if (m_printer_hold_alias[printer_name].end() != alias_iter) {
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " "
                                        << " The alias already exists: " << alias << " and the preset name: " << preset->name;
                return true;
            }
        }
    }
...
Preset* PresetCollection::find_preset2(const std::string& name, bool auto_match)
{
    ...
    if (preset == nullptr) {
        ...
        if (auto_match && preset == nullptr) {
            // Adartys: one more try, find the most likely preset in AdartysFilamentLibrary
            if (name.find("Generic") != std::string::npos) {
                ...
                if (preset != nullptr) {
                    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << " "
                                             << "Failed to find: " << name << ". fallback to library preset: " << preset->file;
                }
            }
        }
    }
    ...
    for (const t_config_option_key& opt_key : config_this.keys()) {
        ...
        if (this_opt != nullptr && other_opt != nullptr && *this_opt != *other_opt) {
            ...
            if (opt_key == "printable_area" || opt_key == "bed_exclude_area" || opt_key == "compatible_prints" ||
                opt_key == "compatible_printers" || opt_key == "thumbnails") {
...
{
    // Adartys: Collect all filament presets that has empty compatible_printers and belongs to the Adartys Filament Library.
    std::map<std::string, std::set<std::string>*> excluded_froms;
    for (Preset& preset : m_presets) {
        if (preset.vendor != nullptr && preset.vendor->name == PresetBundle::ADARTYS_FILAMENT_LIBRARY) {
    ...
    // Check all presets that has the same alias as the filament presets with empty compatible_printers in Adartys Filament Library.
    for (const Preset& preset : m_presets) {
        if (preset.vendor == nullptr || preset.vendor->name == PresetBundle::ADARTYS_FILAMENT_LIBRARY)
            continue;
    ...
    for (const std::string& printer_name : compatible_printers->values) {
        ...
        if (m_printer_hold_alias.end() == printer_iter) {
            ...
            if (m_printer_hold_alias[printer_name].end() == alias_iter) {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " " << printer_name << "already has alias: " << alias
...
static std::vector<std::string> s_PhysicalPrinter_opts{"preset_name", // temporary option to compatibility with older Slicer
                                                       "preset_names", "printer_technology", "bbl_use_printhost", "host_type", "print_host",
                                                       "print_host_webui", "printhost_apikey", "printhost_cafile", "printhost_port",
                                                       "printhost_authorization_type",
