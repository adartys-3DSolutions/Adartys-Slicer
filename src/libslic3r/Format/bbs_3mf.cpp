...
class _BBS_3MF_Importer : public _BBS_3MF_Base
{
    ...
    // Semantic version of Adartys Slicer, that generated this 3MF.
                                              ...
                                              int                        plate_id)
{
    ...
    if (!m_is_bbl_3mf) {
        // if the 3mf was not produced by AdartysSlicer and there is more than one instance,
...
bool _BBS_3MF_Importer::_handle_end_model()
{
    ...
    if (!m_is_bbl_3mf) {
        // if the 3mf was not produced by AdartysSlicer and there is only one object,
...
bool _BBS_3MF_Importer::_handle_end_metadata()
{
    if ((m_curr_metadata_name == BBS_3MF_VERSION) || (m_curr_metadata_name == BBS_3MF_VERSION1)) {
        ...
        } else if (boost::starts_with(m_curr_characters, "AdartysSlicer-")) {
            m_is_bbl_3mf                    = true;
        ...
        /*} else if (m_curr_metadata_name == BBS_FDM_SUPPORTS_PAINTING_VERSION) {
            ...
            check_painting_version(m_fdm_supports_painting_version, FDM_SUPPORTS_PAINTING_VERSION,
                _(L("The selected 3MF contains FDM supports painted object using a newer version of AdartysSlicer and is not compatible.")));
        } else if (m_curr_metadata_name == BBS_SEAM_PAINTING_VERSION) {
            m_seam_painting_version = (unsigned int) atoi(m_curr_characters.c_str());
            check_painting_version(m_seam_painting_version, SEAM_PAINTING_VERSION,
                _(L("The selected 3MF contains seam painted object using a newer version of AdartysSlicer and is not compatible.")));
        } else if (m_curr_metadata_name == BBS_MM_PAINTING_VERSION) {
            m_mm_painting_version = (unsigned int) atoi(m_curr_characters.c_str());
            check_painting_version(m_mm_painting_version, MM_PAINTING_VERSION,
                _(L("The selected 3MF contains multi-material painted object using a newer version of AdartysSlicer and is not compatible.")));*/
...
bool _BBS_3MF_Importer::_handle_start_text_info_item(const char** attributes, unsigned int num_attributes)
{
    ...
    // TODO: Adartys: support legacy text info
    /*
    TextInfo text_info;
...
triangle mesh found"); return false;
        ...
        if (!m_is_bbl_3mf) {
            // if the 3mf was not produced by AdartysSlicer and there is only one instance,
            // bake the transformation into the geometry to allow the reload from disk command
            // to work properly
            if (object.instances.size() == 1) {
                triangle_mesh.transform(object.instances.front()->get_transformation().get_matrix(), false);
                                            ...
                                            int                                export_plate_idx)
{
    ...
    // The content of this file is the same for each AdartysSlicer 3mf.
                                                   ...
                                                   BBLProject*            project) const
{
...

    {
        ...
        if (!sub_model) {
            ...
            metadata_item_map[BBL_DESIGNER_USER_ID_TAG]    = ""; // Adartys: PRIVACY: do not store BBL user id in 3mf
            ...
            // Adartys: PRIVACY: do not store creation & modification date in 3mf
...
bool _BBS_3MF_Exporter::_add_mesh_to_object_stream(std::function<bool(std::string&, bool)> const& flush, ObjectData const& object_data) const
{
    ...
    for (unsigned int index = 0; index < object.volumes.size(); index++) {
        ...
        // This won't break anything because when loading the file Adartys (and Bambu) simply does not care about the actual object type at
        // all (as long as it's one of "model" & "other"); But PrusaSlicer requires the type to be "model".
    ...
    for (unsigned int i = 0; i < (unsigned int) plate_data_list.size(); ++i) {
        ...
        if (plate_data != nullptr) {
            ...
            for (int i = 0; i < filaments_count; ++i) {
                stream << "1"; // Adartys hack: for now, all filaments are mapped to extruder 1
                                                               ...
                                                               const DynamicPrintConfig&    config)
{
    ...
    stream << "    <" << SLICE_HEADER_ITEM_TAG << " " << KEY_ATTR << "=\""
           ...
           << "\" " << VALUE_ATTR << "=\"" << convert_to_full_version(SoftFever_VERSION) << "\"/>\n";
    stream << "  </" << SLICE_HEADER_TAG << ">\n";
    ...
    for (auto custom_gcodes : model.plates_custom_gcodes) {
        ...
        for (const CustomGCode::Item& code : custom_gcodes.second.gcodes) {
            ...
            std::string gcode = // code.type == CustomGCode::ColorChange ? config->opt_string("color_change_gcode")    :
                code.type == CustomGCode::PausePrint ?
                    config->opt_string("machine_pause_gcode") :
        ...
        mode_tree.put("<xmlattr>.value", custom_gcodes.second.mode == CustomGCode::Mode::SingleExtruder ?
                                             CustomGCode::SingleExtruderMode :
                                             custom_gcodes.second.mode == CustomGCode::Mode::MultiAsSingle ?
...
Transform3d create_fix(const std::optional<Transform3d>& prev, const ModelVolume& volume)
{
    ...
    // Adartys: do not bake volume transformation into meshes
