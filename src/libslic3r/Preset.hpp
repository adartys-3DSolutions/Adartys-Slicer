...
// Orca extension
#define ADARTYS_JSON_KEY_RENAMED_FROM "renamed_from"
...
public:
    ...
    bool m_from_adartys_filament_lib = false;
...
public:
    ...
    void save_current_preset(const std::string& new_name,
                             ...
                             const Preset*      _current_printer = nullptr);
    ...
    std::deque<Preset>::iterator find_preset_internal(const std::string& name, bool from_orca_lib_only = false)
    {
        ...
        if (it == m_presets.end() || it->name != name) {
            ...
            for (size_t i = 0; i < m_num_default_presets; ++i)
                if (m_presets[i].name == name && (!from_orca_lib_only || m_presets[i].m_from_adartys_filament_lib)) {
