...
Fill* Fill::new_from_type(const InfillPattern type)
{
    ...
    // Adartys: Replace BBS implementation with Prusa implementation
...
void Fill::fill_surface_extrusion(const Surface* surface, const FillParams& params, ExtrusionEntitiesPtr& out)
{
    ...
    if (!polylines.empty() || !thick_polylines.empty()) {
        ...
        // Adartys: run gap fill
        this->_create_gap_fill(surface, params, eec);
    }
}

// Adartys: Dedicated function to calculate gap fill lines for the provided surface, according to the print object parameters
...
{
    // Adartys: just to be safe, check against null pointer for the print object config and if NULL return.
    if (this->print_object_config == nullptr)
        return;

    // Adartys: Enable gap fill as per the user preference. Return early if gap fill is to not be applied.
...
std::pair<float, Point> Fill::_infill_direction(const Surface* surface) const
{
    ...
    if (surface->bridge_angle >= 0) {
        ...
        // Adartys: if template angle is not empty, don't apply layer angle
