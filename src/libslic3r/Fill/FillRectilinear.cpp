                                              ...
                                              Polylines&                              polylines_out)
{
    ...
    for (;;) {
        ...
        if (try_connect) {
            ...
            // Mark both the left and right connecting segment as consumed, because one cannot go to this intersection point as it has been
            // consumed. If there are any outer intersection points skipped (bypassed) by the contour, mark them as processed.
            if (going_up)
                                                 ...
                                                 Polylines&                                polylines_out)
{
    ...
    if (!fill_lines.empty()) {
        if (params.dont_connect()) {
            ...
            connect_infill(std::move(fill_lines), poly_with_offset_base.polygons_outer, get_extents(surface->expolygon.contour),
...
{
    Polylines polylines_out;
    // Adartys Todo: fow now don't use fill_surface_by_multilines for zipzag infill
...
// Adartys: Introduced FillMonotonicLines from Prusa slicer, inhereting from FillRectilinear
...
// Adartys: Replaced with FillMonotonicLines from Prusa slicer. Moved gap fill algorithm to
...
/*void FillMonotonicLineWGapFill::fill_surface_extrusion(const Surface* surface, const FillParams& params, ExtrusionEntitiesPtr& out)
{
    ...
    // Adartys: The above causes pockmarks in top layer surfaces with a properly calibrated printer with PA and EM tuned.
