...
struct SurfaceFillParams
{
    ...
    float angle = 0.f;
    // Adartys: is_using_template_angle
...
#define RETURN_COMPARE_NON_EQUAL_TYPED(TYPE, KEY) \
    ...
    bool operator==(const SurfaceFillParams& rhs) const
    {
        return this->extruder == rhs.extruder && this->pattern == rhs.pattern && this->spacing == rhs.spacing &&
               this->overlap == rhs.overlap && this->angle == rhs.angle && this->is_using_template_angle == rhs.is_using_template_angle &&
    ...
    for (size_t region_id = 0; region_id < layer.regions().size(); ++region_id) {
        ...
        for (const Surface& surface : layerm.fill_surfaces.surfaces)
            if (surface.surface_type == stInternalVoid)
                ...
                // Adartys: apply fill multiline only for sparse infill
                ...
                params.flow                = params.bridge ?
                                  // Adartys: enable thick bridge based on config
    ...
    struct IroningParams
    {
        ...
        bool operator==(const IroningParams& rhs) const
        {
            return this->extruder == rhs.extruder && this->just_infill == rhs.just_infill && this->line_spacing == rhs.line_spacing &&
                   ...
                   this->is_using_template_angle == rhs.is_using_template_angle && this->pattern == rhs.pattern && this->inset == rhs.inset;
        }
    ...
    for (LayerRegion* layerm : m_regions)
        if (!layerm->slices.empty()) {
            ...
            if (ironing_params.extruder != -1) {
                ...
                ironing_params.angle = calculate_infill_rotation_angle(this->object(), this->id(), config.solid_infill_direction.value,
