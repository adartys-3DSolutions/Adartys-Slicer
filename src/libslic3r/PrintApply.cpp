...
// Adartys: Updated to include XYZ filament shrinkage compensation
                                                                                 ...
                                                                                 const Vec3d&       shrinkage_compensation)
{
    ...
    for (ModelInstance* model_instance : model_object.instances) {
        if (model_instance->is_printable()) {
            // Adartys: Updated with XYZ filament shrinkage compensation
            ...
            // BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << boost::format(", Line %1%: found object %2%'s instance %3% not
            // printable")%__LINE__ %model_object.name %index; BOOST_LOG_TRIVIAL(debug) << boost::format(" object printable %1%, instance
            // printable %2%, print_volume_state %3%")%model_object.printable %model_instance->printable %model_instance->print_volume_state;
        }
        index++;
    }
        ...
        if (opt_new_filament != nullptr && !opt_new_filament->is_nil()) {
            ...
            if (overriden || *opt_old != *opt_new) {
                ...
                if (!((opt_key == "long_retractions_when_cut" || opt_key == "retraction_distances_when_cut") &&
                      new_full_config.option<ConfigOptionInt>("enable_long_retraction_when_cut")->value !=
                          LongRectrationLevel::EnableFilament)) // ugly code, remove it later if firmware supports
...
Print::ApplyStatus Print::apply(const Model& model, DynamicPrintConfig new_full_config)
{
    ...
    // 4) Generate PrintObjects from ModelObjects and their instances.
    {
        ...
        for (ModelObject* model_object : m_model.objects) {
            ModelObjectStatus& model_object_status = const_cast<ModelObjectStatus&>(model_object_status_db.reuse(*model_object));
            // Adartys: Updated for XYZ filament shrink compensation
