...
// Call IMGUI_CHECKVERSION() from your .cpp files to verify that the data structures your files are using are matching the ones imgui.cpp is
// using.
//-----------------------------------------------------------------------------
...
// forward ABI compatibility. DLL users: heaps and globals are not shared across DLL boundaries! You will need to call SetCurrentContext() +
// SetAllocatorFunctions() for each static/DLL boundary you are calling from. Read "Context and Memory Allocators" section of imgui.cpp for
// more details.
//#define IMGUI_API __declspec( dllexport )
//#define IMGUI_API __declspec( dllimport )

//---- Don't define obsolete functions/enums/behaviors. Consider enabling from time to time after updating to avoid using soon-to-be
//obsolete function/names. #define IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
//recommended. #define IMGUI_DISABLE_METRICS_WINDOW                      // Disable metrics/debugger window: ShowMetricsWindow() will be empty.
...
//OpenClipboard/GetClipboardData/CloseClipboard etc. (user32.lib/.a, kernel32.lib/.a) #define IMGUI_DISABLE_WIN32_DEFAULT_IME_FUNCTIONS //
//[Win32] Don't implement default IME handler. Won't use and link with ImmGetContext/ImmSetCompositionWindow. (imm32.lib/.a) #define
//IMGUI_DISABLE_WIN32_FUNCTIONS                     // [Win32] Won't use and link with any Win32 function (clipboard, ime). #define
//IMGUI_ENABLE_OSX_DEFAULT_CLIPBOARD_FUNCTIONS      // [OSX] Implement default OSX clipboard handler (need to link with '-framework
//ApplicationServices', this is why this is not the default). #define IMGUI_DISABLE_DEFAULT_FORMAT_FUNCTIONS            // Don't implement
...
//them yourself. #define IMGUI_DISABLE_FILE_FUNCTIONS                      // Don't implement ImFileOpen/ImFileClose/ImFileRead/ImFileWrite
...
//---- Use stb_printf's faster implementation of vsnprintf instead of the one from libc (unless IMGUI_DISABLE_DEFAULT_FORMAT_FUNCTIONS is
//defined)
// Requires 'stb_sprintf.h' to be available in the include path. Compatibility checks of arguments and formats done by clang and GCC will be
// disabled in order to support the extra formats provided by STB sprintf. #define IMGUI_USE_STB_SPRINTF
...
// (in this repository) + the FreeType library (not provided). On Windows you may use vcpkg with 'vcpkg install freetype' + 'vcpkg integrate
// install'.
...
const wchar_t gCodeButtonIcon    = 0x0819; // ADARTYS
const wchar_t VisibleIcon        = 0x0820; // ADARTYS
const wchar_t HiddenIcon         = 0x0821; // ADARTYS
