...
static bool InputTextFilterCharacter(
    unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source);
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    // AllowOverlap mode (rarely used) requires previous frame HoveredId to be null or to match. This allows using patterns where a later
    ...
    if (g.NavActivateDownId == id) {
        ...
        bool nav_activated_by_inputs = IsNavInputTest(ImGuiNavInput_Activate, (flags & ImGuiButtonFlags_Repeat) ?
...
bool ImGui::BBLButton(const char* label, const ImVec2& size_arg) { return BBLButtonEx(label, size_arg, ImGuiButtonFlags_None); }
...
bool ImGui::ScrollbarEx(
    const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawFlags flags)
{
    ImGuiContext& g      = *GImGui;
    ImGuiWindow*  window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f) {
        ...
        const float scroll_v_norm = ImSaturate((clicked_v_norm - g.ScrollbarClickDeltaToGrabCenter - grab_h_norm * 0.5f) /
                                               (1.0f - grab_h_norm));
    ...
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels,
                           bb.Max.y);
    else
...
void ImGui::Image(
    ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
...
bool ImGui::ImageButtonEx2(ImGuiID       id,
                           ImTextureID   texture_id,
                           const ImVec2& size,
                           const ImVec2& uv0,
                           const ImVec2& uv1,
                           ...
                           const ImVec2& margin)
{
    ...
    if (border_size > 0.0f) {
        // window->DrawList->AddRect(bb.Min + ImVec2(1, 1), bb.Max + ImVec2(1, 1), col, g.Style.FrameRounding, 0, border_size); // ADARTYS
        // adds a secondary border without reason that causes wrong framesize
...
bool ImGui::ImageButtonEx3(ImGuiID       id,
                           ImTextureID   texture_id,
                           ImTextureID   texture_id_hover,
                           const ImVec2& size,
                           const ImVec2& uv0,
                           const ImVec2& uv1,
                           const ImVec2& padding,
...
bool ImGui::BBLImageButtonEx(ImGuiID       id,
                             ImTextureID   texture_id,
                             ImTextureID   texture_id_hover,
                             ImTextureID   texture_id_press,
                             const ImVec2& size,
                             bool&         value,
                             const ImVec2& uv0,
                             const ImVec2& uv1,
                             ...
                             const ImVec2& margin)
{
    ...
    if (value)
        window->DrawList->AddImage(texture_id_press, bb.Min + padding + margin, bb.Max - padding - margin, uv0, uv1, GetColorU32(tint_col));
...
bool ImGui::ImageButton2(ImTextureID   user_texture_id,
                         const ImVec2& size,
                         const ImVec2& uv0,
                         const ImVec2& uv1,
                         int           frame_padding,
...
bool ImGui::ImageButton3(ImTextureID   user_texture_id,
                         ImTextureID   user_texture_id_hover,
                         const ImVec2& size,
                         const ImVec2& uv0,
                         const ImVec2& uv1,
                         int           frame_padding,
...
bool ImGui::BBLImageButton(ImTextureID   user_texture_id,
                           ImTextureID   user_texture_id_hover,
                           ImTextureID   user_texture_id_press,
                           const ImVec2& size,
                           bool&         value,
                           const ImVec2& uv0,
                           const ImVec2& uv1,
                           ...
                           const ImVec2& margin)
{
    ...
    const ImVec2 padding = (frame_padding >= 0) ? ImVec2((float) frame_padding, (float) frame_padding) : g.Style.FramePadding;
    return BBLImageButtonEx(id, user_texture_id, user_texture_id_hover, user_texture_id_press, size, value, uv0, uv1, padding, bg_col,
                            tint_col, margin);
}

bool ImGui::ImageTextButton(const ImVec2& button_size,
                            const char*   text,
                            ...
                            const ImVec2& margin)
{
    ...
    window->DrawList->AddImage(user_texture_id, bb.Min + padding + calc_image_offset, bb.Min + padding + image_size + calc_image_offset,
...
bool ImGui::BBLCheckbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(ImVec2(pos.x, pos.y + style.ItemInnerSpacing.x * 0.5),
                          pos + ImVec2(square_sz, square_sz + style.ItemInnerSpacing.x * 0.5));
    RenderNavHighlight(total_bb, id);
    RenderFrame(check_bb.Min, check_bb.Max,
                GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true,
                style.FrameRounding);
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label,
                                window->DC.LastItemStatusFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));
    return pressed;
}
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    if (overlay_size.x > 0.0f)
        RenderTextClipped(ImVec2(ImClamp(fill_br.x + style.ItemSpacing.x, bb.Min.x, bb.Max.x - overlay_size.x - style.ItemInnerSpacing.x),
                                 bb.Min.y),
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    if (flags & ImGuiSeparatorFlags_Vertical) {
        // Vertical separator, for menu bars (use current line height). Not exposed because it is misleading and it doesn't have an effect
        // on regular layout.
...
bool ImGui::Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items)
{
    ...
    bool value_changed = Combo(label, current_item, Items_SingleStringGetter, (void*) items_separated_by_zeros, items_count,
                               height_in_items);
    return value_changed;
}
...
// FIXME-LEGACY: Prior to 1.61 our DragInt() function internally used floats and because of this the compile-time default value for format
// was "%.0f". Even though we changed the compile-time default, we expect users to have carried %f around, which would break the display of
// DragInt() calls. To honor backward compatibility we are rewriting the format string, unless IMGUI_DISABLE_OBSOLETE_FUNCTIONS is enabled.
...
bool ImGui::DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* p_data, const char* format)
{
    ...
    if (data_type == ImGuiDataType_S32) {
        ...
        if (op && sscanf(initial_value_buf, format, &arg0i) < 1)
            return false;
        // Store operand in a float so we can use fractional value for multipliers (*1.1), but constant always parsed as integer so we can
        ...
        // We don't bother handling support for legacy operators since they are a little too crappy. Instead we will later implement a
                       ...
                       ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active) {
        ...
        // FIXME: Currently polling ImGuiConfigFlags_IsTouchScreen, may either poll an hypothetical ImGuiBackendFlags_HasKeyboard and/or an
        // explicit drag settings.
...
bool ImGui::BBLDragScalar(const char*      label,
                          ImGuiDataType    data_type,
                          void*            p_data,
                          float            v_speed,
                          const void*      p_min,
                          ...
                          ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active) {
        ...
        if (focus_requested || clicked || double_clicked || g.NavActivateId == id || g.NavInputId == id) {
            ...
            FocusWindow(window);
            g.ActiveIdUsingNavDirMask = (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
            if (temp_input_allowed && (focus_requested || (clicked && g.IO.KeyCtrl) || clicked || g.NavInputId == id))
                temp_input_is_active = true;
        }
        // Experimental: simple click (without moving) turns Drag into an InputText
        // FIXME: Currently polling ImGuiConfigFlags_IsTouchScreen, may either poll an hypothetical ImGuiBackendFlags_HasKeyboard and/or an
    ...
    if (temp_input_is_active) {
        ...
        const bool is_clamp_input = (flags & ImGuiSliderFlags_AlwaysClamp) != 0 &&
                                    (p_min == NULL || p_max == NULL || DataTypeCompare(data_type, p_min, p_max) < 0);
        ImGui::PushStyleColor(ImGuiCol_Border, GetColorU32(ImGuiCol_BorderActive));
        bool b_input_sclar = TempInputScalar(frame_bb, id, label, data_type, p_data, format, is_clamp_input ? p_min : NULL,
                            ...
                            ImGuiSliderFlags flags)
{
    ...
    value_changed |= DragScalar("##max", ImGuiDataType_Float, v_current_max, v_speed, &max_min, &max_max, format_max ? format_max : format,
                                max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    if (g.ActiveId == id) {
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
            if (!g.IO.MouseDown[0]) {
                ...
                clicked_t = (slider_usable_sz > 0.0f) ? ImClamp((mouse_abs_pos - slider_usable_pos_min) / slider_usable_sz, 0.0f, 1.0f) :
                                                        0.0f;
                if (axis == ImGuiAxis_Y)
                           ...
                           ImRect*          out_grab_bb)
{
    ...
    case ImGuiDataType_S8: {
        ImS32 v32 = (ImS32) * (ImS8*) p_v;
        bool  r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS8*) p_min, *(const ImS8*) p_max, format,
                                                      flags, out_grab_bb);
        if (r)
            *(ImS8*) p_v = (ImS8) v32;
        return r;
    ...
    case ImGuiDataType_U8: {
        ImU32 v32 = (ImU32) * (ImU8*) p_v;
        bool  r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU8*) p_min, *(const ImU8*) p_max, format,
                                                      flags, out_grab_bb);
        if (r)
            *(ImU8*) p_v = (ImU8) v32;
        return r;
    ...
    case ImGuiDataType_Double:
        ...
        return SliderBehaviorT<double, double, double>(bb, id, data_type, (double*) p_v, *(const double*) p_min, *(const double*) p_max,
...
bool ImGui::BBLSliderScalarIn(const char*      label,
                              ImGuiDataType    data_type,
                              void*            p_data,
                              const void*      p_min,
                              const void*      p_max,
                              ...
                              ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        for (int i = 1; i < 5; i++) {
            window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x) * 0.2 * i, center.y), radius - pad,
                                              frame_col, style.GrabRounding);
        }
        window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x, center.y), radius, GetColorU32(ImGuiCol_SliderGrab), style.GrabRounding);
        window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x, center.y), radius - pad - 1, GetColorU32(ImGuiCol_CheckMark),
                                          style.GrabRounding);
    }
    ...
    RenderTextClipped(ImVec2(frame_bb.Min.x + end_x, frame_bb.Min.y), ImVec2(frame_bb.Max.x + value_size, frame_bb.Max.y), value_buf,
...
bool ImGui::BBLSliderScalar(const char*      label,
                            ImGuiDataType    data_type,
                            void*            p_data,
                            const void*      p_min,
                            const void*      p_max,
...
bool ImGui::SliderScalar(
    const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
...
bool ImGui::BBLInputDouble(const char* label, double* v, double step, double step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ImGui::PushStyleColor(ImGuiCol_BorderActive, ImVec4(0.00f, 0.59f, 0.53f, 1.00f));
    flags |= ImGuiInputTextFlags_CharsScientific;
    bool bbl_input_scalar = BBLInputScalar(label, ImGuiDataType_Double, (void*) v, (void*) (step > 0.0 ? &step : NULL),
                                           (void*) (step_fast > 0.0 ? &step_fast : NULL), format, flags);
    ImGui::PopStyleColor(1);
...
void ImGuiInputTextCallbackData::InsertChars(int pos, const char* new_text, const char* new_text_end)
{
    ...
    if (new_text_len + BufTextLen >= BufSize) {
        ...
        // Contrary to STB_TEXTEDIT_INSERTCHARS() this is working in the UTF8 buffer, hence the mildly similar code (until we remove the U16
        // buffer altogether!)
        ImGuiContext&        g          = *GImGui;
    ...
    unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source)
{
    ...
    if (flags & (ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase |
        ...
        // We don't really intend to provide widespread support for it, but out of empathy for people stuck with using odd API, we support
        // the bare minimum aka overriding the decimal point. Change the default decimal_point with:
...
// (FIXME: Rather confusing and messy function, among the worse part of our codebase, expecting to rewrite a V2 at some point.. Partly
// because we are
                        ...
                        void*                  callback_user_data)
{
    ...
    const bool init_make_active   = (user_clicked || user_scroll_finish || user_nav_input_start || focus_requested_by_code ||
                                   focus_requested_by_tabbing);
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs) {
        ...
        // FIXME: For non-readonly widgets we might be able to require that TextAIsValid && TextA == buf ? (untested) and discard undo stack
        // if user buffer has changed.
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
        ...
        IM_ASSERT(io.KeyMods == GetMergedKeyModFlags() &&
                  "Mismatching io.KeyCtrl/io.KeyShift/io.KeyAlt/io.KeySuper vs io.KeyMods"); // We rarely do this check, but if anything
                                                                                             // let's do it here.
    ...
    if (g.ActiveId == id) {
        ...
        // When using 'ImGuiInputTextFlags_EnterReturnsTrue' as a special case we reapply the live buffer back to the input buffer before
        // clearing ActiveId, even though strictly speaking it wasn't modified on this frame. If we didn't do that, code like InputInt()
        // with ImGuiInputTextFlags_EnterReturnsTrue would fail. This also allows the user to use InputText() with
        // ImGuiInputTextFlags_EnterReturnsTrue without maintaining any user-side storage (please note that if you use this property along
        // ImGuiInputTextFlags_CallbackResize you can end up with your temporary string object unnecessarily allocating once a frame, either
        // store your string data, either if you don't then don't use ImGuiInputTextFlags_CallbackResize).
        ...
        if (apply_edit_back_to_user_buffer) {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit |
                ...
                if (event_flag) {
                    ...
                    // We have to convert from wchar-positions to UTF-8-positions, which can be pretty slow (an incentive to ditch the
                    // ImWchar buffer, see https://github.com/nothings/stb/issues/188)
    ...
    if (render_cursor || render_selection) {
        ...
        // We test for 'buf_display_max_length' as a way to avoid some pathological cases (e.g. single-line 1 MB string) which would make
        // ImDrawList crash.
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel) {
        ...
        // Render SV Square
        draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), col_white, hue_color32,
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    {
        off = -2.5f; // The border (using Col_FrameBg) tends to look off when color is near-opaque and rounding is enabled. This offset
...
#ifdef __APPLE__
            window->DrawList->AddRect(bb.Min - ImVec2(3, 3), bb.Max + ImVec2(3, 3), GetColorU32(ImGuiCol_FrameBg), rounding * 2, 0,
                                      4.0f); // Color button are often in need of some sort of border
#else
            window->DrawList->AddRect(bb.Min - ImVec2(2, 2), bb.Max + ImVec2(2, 2), GetColorU32(ImGuiCol_FrameBg), rounding * 2, 0,
                                      3.0f); // Color button are often in need of some sort of border
#endif
    }
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    if ((flags & ImGuiColorEditFlags_InputRGB) || !(flags & ImGuiColorEditFlags__InputMask)) {
        if (flags & ImGuiColorEditFlags_NoAlpha)
            ...
            Text("#%02X%02X%02X%02X\nR:%d, G:%d, B:%d, A:%d\n(%.3f, %.3f, %.3f, %.3f)", cr, cg, cb, ca, cr, cg, cb, ca, col[0], col[1],
                 col[2], col[3]);
    } else if (flags & ImGuiColorEditFlags_InputHSV) {
        if (flags & ImGuiColorEditFlags_NoAlpha)
...
bool ImGui::BBLImageSelectable(ImTextureID   user_texture_id,
                               const ImVec2& size_arg,
                               const ImVec2& font_size,
                               int           font_line,
                               const ImVec4& tint_col,
                               const ImVec2& uv0,
                               const ImVec2& uv1,
                               bool          selected)
{
    ...
    if (hovered || selected) {
        ...
        if (arrow_size == 0) {
            ...
            RenderFrame(ImVec2(bb.Min.x + style.WindowPadding.x, bb.Min.y), ImVec2(bb.Max.x - style.WindowPadding.x, bb.Max.y), col, false,
                        0.0f);
        }
        RenderNavHighlight(bb, id, ImGuiNavHighlightFlags_TypeThin | ImGuiNavHighlightFlags_NoRounding);
    }
    ...
    window->DrawList->AddImage(user_texture_id, p_min, p_max, uv0, uv1,
                               selected || (held && hovered) ? GetColorU32(ImVec4(1.f, 1.f, 1.f, 1.f)) : GetColorU32(tint_col));
...
// Tip: If your vertical size is calculated from an item count (e.g. 10 * item_height) consider adding a fractional part to facilitate
// seeing scrolling boundaries (e.g. 10.25 * item_height).
...
bool ImGui::ListBox(
    const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
    ImGuiContext& g = *GImGui;
                  ...
                  ImVec2      frame_size)
{
    ...
    if (values_count >= values_count_min) {
        ...
        float  histogram_zero_line_t = (scale_min * scale_max < 0.0f) ? (-scale_min * inv_scale) :
...
void ImGui::EndMenuBar()
{
    ...
    if (NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right) &&
        ...
        if (nav_earliest_child->ParentWindow == window && nav_earliest_child->DC.ParentLayoutType == ImGuiLayoutType_Horizontal &&
            ...
            // multiple window (probably not worth the hassle/cost)
...
bool ImGui::BBLMenuItem(const char* label, const char* shortcut, bool selected, bool enabled, float size_arg_y)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        float extra_w = ImMax(0.0f, GetContentRegionAvail().x - min_w);
        pressed       = BBLSelectable(label, false, flags | ImGuiSelectableFlags_SpanAvailWidth | ImGuiComboFlags_NoArrowButton,
                                ImVec2(min_w, size_arg_y));
        if (shortcut_w > 0.0f) {
            PushStyleColor(ImGuiCol_Text, g.Style.Colors[ImGuiCol_TextDisabled]);
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImU32 col)
{
    ...
    const float rounding = ImMax(0.0f, ImMin((flags & ImGuiTabItemFlags_Button) ? g.Style.FrameRounding : g.Style.TabRounding,
                                             width * 0.5f - 1.0f));
