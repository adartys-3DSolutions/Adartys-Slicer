...
public:
    Renderer(wxGLCanvas* c) : m_canvas{c}
    {
        ...
        if (!ctx || !ctx->IsOK()) {
            wxMessageBox("Could not create OpenGL context.", "Error", wxOK | wxICON_ERROR);
            return;
        }
...
public:
    template<class... Args> Canvas(Args&&... args) : wxGLCanvas(std::forward<Args>(args)...) {}
...
public:
    ...
    {
        if (m_recording)
            m_events.emplace_back(LCLK_D);
        if (!m_playing)
            MouseInput::left_click_down();
    }
    void left_click_up() override
    {
        if (m_recording)
            m_events.emplace_back(LCLK_U);
        if (!m_playing)
            MouseInput::left_click_up();
    }
    void right_click_down() override
    {
        if (m_recording)
            m_events.emplace_back(RCLK_D);
        if (!m_playing)
            MouseInput::right_click_down();
    }
    void right_click_up() override
    {
        if (m_recording)
            m_events.emplace_back(RCLK_U);
        if (!m_playing)
            MouseInput::right_click_up();
    }
    void double_click() override
    {
        if (m_recording)
            m_events.emplace_back(DDCLK);
        if (!m_playing)
            MouseInput::double_click();
    }
    void scroll(long v, long d, WheelAxis wa) override
    {
        if (m_recording)
            m_events.emplace_back(SCRL, v, d);
        if (!m_playing)
            MouseInput::scroll(v, d, wa);
    }
    void move_to(long x, long y) override
    {
        if (m_recording)
            m_events.emplace_back(MV, x, y);
        if (!m_playing)
            MouseInput::move_to(x, y);
    }
...
class MyFrame : public wxFrame
{
    ...
    public:
        SLAJob(MyFrame* frame, const std::string& fname) : Slic3r::GUI::Job{frame->m_stbar}, m_parent{frame}, m_fname{fname} {}
    ...
    protected:
        ...
        void finalize() override
        {
            ...
            m_parent->m_stbar->set_status_text(wxString::Format("Model %s loaded.", m_fname));
        }
    };
...
public:
    MyFrame(const wxString& title, const wxPoint& pos, const wxSize& size, const wxCmdLineParser& parser);
...
public:
    bool OnInit() override
    {
        ...
        parser.AddOption("o", "optimization", "OpenCSG optimization strategy [Default|ForceOn|On|Off]", wxCMD_LINE_VAL_STRING,
                         wxCMD_LINE_PARAM_OPTIONAL);
        parser.AddOption("c", "convexity", "OpenCSG convexity parameter for generic meshes", wxCMD_LINE_VAL_NUMBER,
                         wxCMD_LINE_PARAM_OPTIONAL);
        ...
        m_frame = new MyFrame("AdartysSlicer OpenCSG Demo", wxDefaultPosition, wxSize(1024, 768), parser);
...
void MyFrame::set_renderer_algorithm(const wxString& alg)
{
    ...
    if (alg_idx < 0 || alg_idx >= long(CSG_ALGS.size()))
        return;

    // If there is a valid display in place, save its camera.
    auto cam = m_canvas->get_display() ? m_canvas->get_display()->get_camera() : nullptr;
    ...
    if (alg == "EnricoShader") {
        ...
        if (m_csg_settings.get_algo() > 0)
            m_depth_select->Enable(true);
...
MyFrame::MyFrame(const wxString& title, const wxPoint& pos, const wxSize& size, const wxCmdLineParser& parser)
    : wxFrame(nullptr, wxID_ANY, title, pos, size)
{
    ...
    int attribList[] = {WX_GL_RGBA, WX_GL_DOUBLEBUFFER,
                        ...
                        WX_GL_MIN_ALPHA, 8, WX_GL_DEPTH_SIZE, 8, WX_GL_STENCIL_SIZE, 8, WX_GL_SAMPLE_BUFFERS, GL_TRUE, WX_GL_SAMPLES, 4, 0};

    m_scene = std::make_shared<Scene>();
    m_ctl   = std::make_shared<Controller>();
    m_ctl->set_scene(m_scene);

    m_canvas = std::make_shared<Canvas>(this, wxID_ANY, attribList, wxDefaultPosition, wxDefaultSize,
    ...
    auto add_combobox = [control_panel, console_sizer](const wxString& label, const std::vector<wxString>& list) {
        auto widget = new wxComboBox(control_panel, wxID_ANY, list[0], wxDefaultPosition, wxDefaultSize, int(list.size()), list.data());
    ...
    auto add_spinctl = [control_panel, console_sizer](const wxString& label, int initial, int min, int max) {
        auto widget = new wxSpinCtrl(control_panel, wxID_ANY, wxString::Format("%d", initial), wxDefaultPosition, wxDefaultSize,
    ...
    Bind(
        wxEVT_MENU,
        [this](wxCommandEvent&) {
            wxFileDialog dlg(this, "Select project file", wxEmptyString, wxEmptyString, "*.3mf", wxFD_OPEN | wxFD_FILE_MUST_EXIST);

            if (dlg.ShowModal() == wxID_OK)
                load_model(dlg.GetPath().ToStdString());
        },
        wxID_OPEN);
    ...
    Bind(wxEVT_SHOW, [this](wxShowEvent&) { activate_canvas_display(); });

    Bind(wxEVT_SLIDER, [this, slider](wxCommandEvent&) { m_ctl->move_clip_plane(double(slider->GetValue())); });
    ...
    m_record_btn->Bind(wxEVT_TOGGLEBUTTON, [this](wxCommandEvent&) {
        ...
        if (m_record_btn->GetValue()) {
            if (auto c = m_canvas->get_display()->get_camera())
                reset(*c);
            m_ctl->on_scene_updated(*m_scene);
            m_mouse.record(true);
        } else {
            m_mouse.record(false);
            wxFileDialog dlg(this, "Select output file", wxEmptyString, wxEmptyString, "*.events", wxFD_SAVE | wxFD_OVERWRITE_PROMPT);

            if (dlg.ShowModal() == wxID_OK) {
                std::fstream stream(dlg.GetPath().ToStdString(), std::fstream::out);
...
void MyFrame::bind_canvas_events(MouseInput& ms)
{
    m_canvas->Bind(wxEVT_MOUSEWHEEL, [&ms](wxMouseEvent& evt) {
        ms.scroll(evt.GetWheelRotation(), evt.GetWheelDelta(),
                  evt.GetWheelAxis() == wxMOUSE_WHEEL_VERTICAL ? Slic3r::GL::MouseInput::waVertical : Slic3r::GL::MouseInput::waHorizontal);
    });

    m_canvas->Bind(wxEVT_MOTION, [&ms](wxMouseEvent& evt) { ms.move_to(evt.GetPosition().x, evt.GetPosition().y); });

    m_canvas->Bind(wxEVT_RIGHT_DOWN, [&ms](wxMouseEvent& /*evt*/) { ms.right_click_down(); });

    m_canvas->Bind(wxEVT_RIGHT_UP, [&ms](wxMouseEvent& /*evt*/) { ms.right_click_up(); });

    m_canvas->Bind(wxEVT_LEFT_DOWN, [&ms](wxMouseEvent& /*evt*/) { ms.left_click_down(); });

    m_canvas->Bind(wxEVT_LEFT_UP, [&ms](wxMouseEvent& /*evt*/) { ms.left_click_up(); });
...
void MyFrame::SLAJob::process()
{
    ...
    m_print->set_status_callback([this](const SlStatus& status) { update_status(status.percent, status.text); });
